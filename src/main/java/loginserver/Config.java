...
import loginserver.utils.LoginSettings;
import loginserver.utils.net.AuthHostSettings;
...
public class Config
{
	...
	public static void load()
	{
		...
		try
		{
			...
			AuthHostSettings.getInstance().loadLoginSettings();
	...
	public static void loadConfiguration(InputStream is)
	{
		try
		{
			...
			DATABASE_CONNECTION_TIMEOUT = serverSettings.getProperty("MaxConnectionTimeout", 2);
			...
			IP_BAN_TIME = serverSettings.getProperty("IpBanTime", 300);
			...
			ALLOW_ENCODE_PASSWORD = serverSettings.getProperty("AllowEncodePasswords", true);
			SHOW_LICENCE = serverSettings.getProperty("ShowLicence", true);
			LOGIN_SERVER_SCHEDULE_RESTART = serverSettings.getProperty("LoginRestartSchedule", true);
			LOGIN_SERVER_SCHEDULE_RESTART_TIME = serverSettings.getProperty("LoginRestartTime", 24);
			FAKE_ONLINE = serverSettings.getProperty("FakeOnline", 1.0);
			FAKE_ONLINE_MULTIPLIER = serverSettings.getProperty("FakeOnlineMultiplier", 0);
