...
import loginserver.network.communication.loginserverpackets.AuthResponse;
import loginserver.network.communication.loginserverpackets.LoginServerFail;
...
public class AuthRequest extends ReceivablePacket
{
	...
	protected void readImpl()
	{
		try
		{
			...
			for (int i = 0; i < hostsCount; i++)
			{
				...
				final var host = new HostInfo(id, address, port, key, allowProxy == 1);
	...
	protected void runImpl()
	{
		...
		_log.info("Gameserver ID[" + gs.getHostId() + "] registration successful.");
