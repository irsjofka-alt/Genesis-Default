...
import loginserver.network.ProxyServer;
...
public class GameServerManager
{
	...
	private static final Logger _log = LogManager.getLogger(GameServerManager.class);
	...
	public int registerGameServer(HostInfo host, GameServer gs)
	{
		...
		try
		{
			final var pgs = _gameServers.get(host.getId());
			if(pgs != null)
			{
				final var phost = pgs.getHost();
	...
	{
		for (final var psc : Config.PROXY_SERVERS_CONFIGS)
		{
			if (psc != null)
			{
				...
				{
					final var ps = new ProxyServer(psc.getOrigServerId(), psc.getProxyId(), psc.getMinAccessLevel(), psc.getMaxAccessLevel(), psc.isHideMain());
					try
					{
						final var inetAddress = InetAddress.getByName(psc.getPorxyHost());
					...
					var proxyList = _gameServerProxys.get(ps.getOrigServerId());
					if (proxyList == null)
					{
	...
	{
		final var result = _gameServerProxys.get(gameServerId);
