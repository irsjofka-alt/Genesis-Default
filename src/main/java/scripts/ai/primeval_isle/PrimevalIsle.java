 ...
 */
package scripts.ai.primeval_isle;
...
public final class PrimevalIsle extends AbstractNpcAI
{
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon)
	{
		if (npc.getId() == EGG)
		{
			if ((getRandom(100) <= 80) && npc.isScriptValue(0))
			{
				...
				for (final Npc npcs : World.getAroundNpc(npc, 500, 200))
				{
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if ((npc.getId() == SAILREN) || (getRandom(100) < 3))
		{
			...
			{
				final var item = player.addItem("PrimevalIsle", DEINONYCHUS, 1, player, true);
				if (item != null)
				{
					final var handler = ItemHandler.getInstance().getHandler(item.getEtcItem());
					if ((handler != null) && !player.hasPet())
					{
						handler.useItem(player, item, true);
					}
