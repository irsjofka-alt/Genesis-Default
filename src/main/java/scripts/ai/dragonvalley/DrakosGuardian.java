	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		if (attacker != null && (actor.getCurrentHp() < (actor.getMaxHp() / 2)) && (Rnd.chance(chance)) && actor.isScriptValue(0))
		{
			...
			for (int i = 0; i < rnd; i++)
			{
				try
				{
					final var npc = new MonsterInstance(IdFactory.getInstance().getNextId(), NpcsParser.getInstance().getTemplate(22823));
					final var loc = Location.findPointToStay(actor, actor.getTemplate().getMinionRange(), actor.getTemplate().getMinionRange(), false);
					if (loc != null)
					{
