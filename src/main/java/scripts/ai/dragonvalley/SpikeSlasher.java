...
public class SpikeSlasher extends Fighter
{
	...
	private void spawnMinions()
	{
		...
		final var oldTarget = getAttackTarget();
		actor.setTarget(actor);
		actor.doCast(SkillsParser.getInstance().getInfo(6841, 1));
		for (int i = 0; i < count; i++)
		{
			final var minion = NpcUtils.spawnSingle(25733, Location.findPointToStay(actor, 250, true));
			if (oldTarget != null)
			{
				minion.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, oldTarget, 5000);
			}
		}
	}
