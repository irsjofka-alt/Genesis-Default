	...
	protected void spawnMobs()
	{
		...
		for (final int[] run : guard_run)
		{
			try
			{
				...
				final ArrayList<Integer> chars = new ArrayList<>();
				for (final var info : actor.getAggroList().getCharMap().values())
				{
					if (info == null)
					{
						continue;
					}
					chars.add(info.getAttackerId());
				}
				...
				{
					hated = GameObjectsStorage.getPlayer(Rnd.get(chars));
				}
				...
				if (hated != null)
				{
					...
					guard.getAI().setIntention(CtrlIntention.ATTACK, hated);
				}
			}
