...
public class ValakasManager extends AbstractNpcAI
{
	...
	private ValakasManager()
	{
		...
		final var info = manager.getStatsSet(29028);
		if (info != null)
		{
			final int status = manager.getBossStatus(29028);
			if (status == 3)
			{
				...
				{
					manager.setBossStatus(29028, 0, false);
				}
			}
			else
			{
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon)
	{
		...
		{
			final var skill = SkillsParser.getInstance().getInfo(4258, 1);
		...
		public void run()
		{
			switch (_taskId)
			{
				case 1 :
					...
					_lastAttackTime = System.currentTimeMillis();
					final var task = _valakasSpawnTask;
					if (task != null)
					{
						task.cancel(false);
				...
				case 19 :
					for (final var pc : _players)
					{
						...
						if (buff != null)
						{
							buff.getEffects(pc, pc, false, true);
						}
					}
					break;
			}
