...
public class Core extends AbstractNpcAI
{
	...
	private Core()
	{
		...
		final var info = manager.getStatsSet(29006);
		if (info != null)
		{
			final int status = manager.getBossStatus(29006);
			if (status == 3)
			{
				...
				else
				{
					...
					manager.setBossStatus(29006, 0, false);
					spawnBoss(_boss);
				}
			}
			else
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final var mob = (Attackable) addSpawn(npc.getId(), npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false, 0);
			mob.setIsRaidMinion(true);
			_minions.add(mob);
		}
