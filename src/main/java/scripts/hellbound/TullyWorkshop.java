...
import gameserver.model.zone.type.DamageZone;
...
public class TullyWorkshop extends Quest
{
	...
	private boolean isTyllyDead = false;
	
	private ScheduledFuture<?> _countdown = null;
	
	private final List<Npc> postMortemSpawn = new ArrayList<>();
	private final Set<Integer> brokenContraptions = ConcurrentHashMap.newKeySet();
	private final Set<Integer> rewardedContraptions = new HashSet<>();
	private final Set<Integer> talkedContraptions = new HashSet<>();
	...
	{
		final var classId = player.getClassId();
	...
	public String onTalk(Npc npc, Player player)
	{
		if (npc.getId() == TOMBSTONE)
		{
			...
			for (final var pl : party.getMembers())
			{
				if (pl == null)
				{
					continue;
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final var dmgZone = (DamageZone) ZoneManager.getInstance().getZoneById(200011);
			if (dmgZone != null)
			{
		...
		{
			final var spawnedNpc = addSpawn(CUBE_68, 12527, 279714, -11622, 16384, false, 0, false);
		...
		{
			World.getAroundPlayers(npc, 300, 200).stream().filter(p -> p != null).forEach(p -> p.teleToLocation(-12176, 279696, -10492, true, p.getReflection()));
		...
		{
			World.getAroundPlayers(npc, 500, 200).stream().filter(p -> p != null).forEach(p -> p.teleToLocation(-12176, 279696, -10492, true, p.getReflection()));
			npc.deleteMe();
		...
		if (event.equalsIgnoreCase("enter") && (npcId == DORIAN))
		{
			...
			if ((party != null) && (party.getLeaderObjectId() == player.getObjectId()))
			{
				...
				party.getMembers().stream().filter(p -> p != null).forEach(p -> p.teleToLocation(-13400, 272827, -15300, true, p.getReflection()));
				htmltext = null;
		...
		else if (npcId == AGENT)
		{
			...
			else if (event.equalsIgnoreCase("refuse") && (allowAgentSpawn_7th == false))
			{
				...
				for (final var monster : spawnedFollowers)
				{
					if ((monster != null) && !monster.isDead())
					{
						...
						final var target = player.getParty() == null ? player : player.getParty().getMembers().get(getRandom(player.getParty().getMembers().size()));
						
						if ((target != null) && !target.isDead())
						{
							monster.addDamageHate(target, 0, 999);
							monster.getAI().setIntention(CtrlIntention.ATTACK, target);
						}
					}
				}
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if (npcId == TULLY)
		{
			...
			{
				final var spawnedNpc = addSpawn(i[0], i[1], i[2], i[3], i[4], false, i[5], false);
			...
			if ((roomData[0] >= 0) && (roomData[1] >= 0) && allowAgentSpawn)
			{
				...
				if (nextServantIdx == 2)
				{
					...
					if (RaidBossSpawnManager.getInstance().getRaidBossStatusId(DARION) == StatusEnum.ALIVE)
					{
						...
						final var spawnedNpc = addSpawn(AGENT, coords[0], coords[1], coords[2], 0, false, 0, false);
	...
	public final String onSpawn(Npc npc)
	{
		...
		{
			isTyllyDead = false;
			postMortemSpawn.stream().filter(n -> n != null).forEach(n -> n.deleteMe());
	...
	private int[] getRoomData(Npc npc)
	{
		...
		if (npc != null)
		{
			...
			for (final var zone : ZoneManager.getInstance().getZones(x, y, z))
			{
				for (int i = 0; i < 2; i++)
				{
					for (int j = 0; j < 4; j++)
	...
	private void do7thFloorSpawn()
	{
		...
		{
			final var monster = (MonsterInstance) addSpawn(data[0], data[1], data[2], data[3], data[4], false, 0, false);
	...
	private void do7thFloorDespawn()
	{
		...
		spawnedFollowers.stream().filter(n -> n != null && !n.isDead()).forEach(n -> n.deleteMe());
		spawnedFollowerMinions.stream().filter(n -> n != null && !n.isDead()).forEach(n -> n.deleteMe());
		...
		public void run()
		{
			...
			if (countdownTime > 60000)
			{
				if ((countdownTime % 60000) == 0)
				{
					...
					{
						final var ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.S1_MINUTES_REMAINING);
						ns.addStringParameter(Integer.toString((countdownTime / 60000)));
						npc.broadcastPacketToOthers(2000, ns);
					}
				}
			...
			else if (countdownTime <= 0)
			{
				...
				postMortemSpawn.stream().filter(n -> n != null && (n.getId() == INGENIOUS_CONTRAPTION || n.getId() == TIMETWISTER_GOLEM)).forEach(n -> n.deleteMe());
			...
			else
			{
				if ((npc != null) && (npc.getId() == INGENIOUS_CONTRAPTION))
				{
					final var ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.S1_SECONDS_REMAINING);
