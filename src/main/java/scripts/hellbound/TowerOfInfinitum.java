		...
		if (event.equalsIgnoreCase("enter") && (npcId == JERIAN))
		{
			if (HellboundManager.getInstance().getLevel() >= 11)
			{
				...
				if ((party != null) && (party.getLeaderObjectId() == player.getObjectId()))
				{
					...
					if (isValid)
					{
						party.getMembers().stream().filter(p -> p != null).forEach(p -> p.teleToLocation(-22204, 277056, -15023, true, p.getReflection()));
						htmltext = null;
					}
				}
				else
		...
		else if ((event.equalsIgnoreCase("up") || event.equalsIgnoreCase("down")) && (npcId >= GK_FIRST) && (npcId <= GK_LAST))
		{
			...
			else
			{
				...
				if (isValid)
				{
					...
					if (tele != null)
					{
						party.getMembers().stream().filter(p -> p != null).forEach(p -> p.teleToLocation(tele, true, p.getReflection()));
					}
					htmltext = null;
				}
			}
