...
import static l2e.commons.util.Utils.formatAdena;
...
import gameserver.model.actor.stat.SummonStat;
import gameserver.model.actor.status.PcStatus;
import gameserver.model.actor.status.SummonStatus;
...
import gameserver.network.serverpackets.SetSummonRemainTime;
			...
			case "cast" :
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					for (int i = 0; i < buffs.size(); ++i)
					{
						if (!getSummonbuff)
						{
							SkillsParser.getInstance().getInfo(buffs.get(i), levels.get(i)).getEffects(player, player, false, false);
						}
						else
						{
							if (player.getSummon() != null)
							{
								SkillsParser.getInstance().getInfo(buffs.get(i), levels.get(i)).getEffects(player.getSummon(), player.getSummon(), false, false);
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
			...
			case "giveBuffs" :
			{
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (!getSummonbuff)
					{
						...
						{
							SkillsParser.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)).getEffects(player, player, false, true);
						}
					}
					else
					{
						...
						else
						{
							if (player.getSummon() != null)
							{
								SkillsParser.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)).getEffects(player.getSummon(), player.getSummon(), false, true);
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
			...
			case "castBuffSet" :
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (!getSummonbuff)
					{
						...
						{
							SkillsParser.getInstance().getInfo(i[0], i[1]).getEffects(player, player, false, false);
						}
						player.updateEffectIcons();
					}
					else
					{
						if (player.getSummon() != null)
						{
							...
							for (final int[] i : buff_sets)
							{
								SkillsParser.getInstance().getInfo(i[0], i[1]).getEffects(player.getSummon(), player.getSummon(), false, false);
							}
							player.getSummon().updateEffectIcons();
						}
						else
