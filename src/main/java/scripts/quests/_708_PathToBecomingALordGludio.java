...
public class _708_PathToBecomingALordGludio extends Quest
{
	...
	private _708_PathToBecomingALordGludio()
	{
		super(708);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		final var castle = CastleManager.getInstance().getCastleById(GLUDIO_CASTLE);
		switch (event)
		{
			case "35100-02.htm":
			case "30298-04.htm" :
			...
			case "35100-23.htm" :
			{
				if (st.isCond(9))
				{
					...
					for (final var fort : FortManager.getInstance().getFortsById(101, 102))
					{
						if (fort != null)
						{
							...
							else if (fort.getContractedCastleId() != GLUDIO_CASTLE)
							{
								return "35100-22b.htm";
							}
						}
					}
					final var packet = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.S1_HAS_BECOME_LORD_OF_THE_TOWN_OF_GLUDIO_LONG_MAY_HE_REIGN);
