...
{
	private _10275_ContainingTheAttributePower()
	{
		super(10275);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			case "32325-09.htm" :
				if (st.isCond(4))
				{
					SkillsParser.getInstance().getInfo(2635, 1).getEffects(player, player, false, true);
					st.giveItems(13845, 1, Elementals.FIRE, 10);
					st.setCond(5, true);
				}
				break;
			case "32326-09.htm" :
				if (st.isCond(9))
				{
					SkillsParser.getInstance().getInfo(2636, 1).getEffects(player, player, false, true);
					st.giveItems(13881, 1, Elementals.EARTH, 10);
					st.setCond(10, true);
				}
				break;
