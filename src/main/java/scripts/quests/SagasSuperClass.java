		...
		if (st != null && !st.isCompleted())
		{
			switch (event)
			{
				...
				case "Mob_3 Timer 1":
					...
					if (Mob_2 != null && World.getAroundNpc(npc).contains(Mob_2))
					{
						((Attackable) npc).addDamageHate(Mob_2, 0, 99999);
						npc.getAI().setIntention(CtrlIntention.ATTACK, Mob_2);
						Mob_2.getAI().setIntention(CtrlIntention.ATTACK, npc);
