...
{
	private final Map<Integer, ReflectionHolder> _reflections = new HashMap<>();
	
	private static class ReflectionHolder
	{
		List<Npc> _mobs = new ArrayList<>();
		boolean _spawned = false;
	}
	
	private _10292_SevenSignsGirlofDoubt()
	{
		super(10292);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		final int refId = npc.getReflectionId();
		var holder = _reflections.get(refId);
		if (holder == null)
		{
			holder = new ReflectionHolder();
			_reflections.put(refId, holder);
		}
		
		if (event.equalsIgnoreCase("evil_despawn"))
		{
			holder._spawned = false;
			for (final Npc h : holder._mobs)
			{
				if (h != null)
				{
					h.deleteMe();
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null && st.isCond(5) && npc.getId() == 27422)
		{
			...
			final var holder = _reflections.get(instanceid);
			if (holder == null)
			{
				return null;
			}
			for (final var h : holder._mobs)
			{
				if (h != null)
				{
					h.deleteMe();
			...
			holder._spawned = false;
			holder._mobs.clear();
			_reflections.remove(instanceid);
			st.setCond(6, true);
		}
		return super.onKill(npc, player, isSummon);
	}
