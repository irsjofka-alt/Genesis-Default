...
import gameserver.instancemanager.TerritoryWarManager.TerritoryNPCSpawn;
...
public class TerritoryWarSuperClass extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("setNextTWDate"))
		{
			...
			final String siegeDate = TerritoryWarManager.getInstance().getSiegeDateTime();
			...
			_log.info("TerritoryWarManager: Next battle " + cal.getTime());
		}
	...
	public static void main(String[] args)
	{
		...
		final var gludio = new _717_FortheSakeoftheTerritoryGludio();
		_forTheSakeScripts.put(gludio.TERRITORY_ID, gludio);
		final var dion = new _718_FortheSakeoftheTerritoryDion();
		_forTheSakeScripts.put(dion.TERRITORY_ID, dion);
		final var giran = new _719_FortheSakeoftheTerritoryGiran();
		_forTheSakeScripts.put(giran.TERRITORY_ID, giran);
		final var oren = new _720_FortheSakeoftheTerritoryOren();
		_forTheSakeScripts.put(oren.TERRITORY_ID, oren);
		final var aden = new _721_FortheSakeoftheTerritoryAden();
		_forTheSakeScripts.put(aden.TERRITORY_ID, aden);
		final var innadril = new _722_FortheSakeoftheTerritoryInnadril();
		_forTheSakeScripts.put(innadril.TERRITORY_ID, innadril);
		final var goddard = new _723_FortheSakeoftheTerritoryGoddard();
		_forTheSakeScripts.put(goddard.TERRITORY_ID, goddard);
		final var rune = new _724_FortheSakeoftheTerritoryRune();
		_forTheSakeScripts.put(rune.TERRITORY_ID, rune);
		final var schuttgart = new _725_FortheSakeoftheTerritorySchuttgart();
		_forTheSakeScripts.put(schuttgart.TERRITORY_ID, schuttgart);
		final var catapult = new _729_Protecttheterritorycatapult();
		_protectTheScripts.put(catapult.getId(), catapult);
		final var military = new _731_ProtecttheMilitaryAssociationLeader();
		_protectTheScripts.put(military.getId(), military);
		final var religious = new _732_ProtecttheReligiousAssociationLeader();
		_protectTheScripts.put(religious.getId(), religious);
		final var supplies = new _730_ProtecttheSuppliesSafe();
		_protectTheScripts.put(supplies.getId(), supplies);
		final var knights = new _734_Piercethroughashield();
		for (final int i : knights.CLASS_IDS)
		{
			_killTheScripts.put(i, knights);
		}
		final var warriors = new _735_Makespearsdull();
		for (final int i : warriors.CLASS_IDS)
		{
			_killTheScripts.put(i, warriors);
		}
		final var wizards = new _736_Weakenmagic();
		for (final int i : wizards.CLASS_IDS)
		{
			_killTheScripts.put(i, wizards);
		}
		final var priests = new _737_DenyBlessings();
		for (final int i : priests.CLASS_IDS)
		{
			_killTheScripts.put(i, priests);
		}
		final var keys = new _738_DestroyKeyTargets();
