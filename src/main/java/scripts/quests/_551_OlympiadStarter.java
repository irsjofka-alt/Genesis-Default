...
public class _551_OlympiadStarter extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final long cert3 = st.getQuestItemsCount(CERT_3);
			final long cert5 = st.getQuestItemsCount(CERT_5);
			final long cert10 = st.getQuestItemsCount(CERT_10);
			if (cert3 > 0 && cert5 > 0)
			{
			...
			else if (cert5 > 0 && cert10 > 0)
			{
				st.giveItems(OLY_CHEST, 6);
				st.giveItems(MEDAL_OF_GLORY, 8);
				st.playSound("ItemSound.quest_finish");
				st.exitQuest(QuestType.DAILY);
			}
			else if (cert3 > 0)
