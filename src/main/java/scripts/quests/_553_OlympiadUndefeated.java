...
public class _553_OlympiadUndefeated extends Quest
{
	...
	private _553_OlympiadUndefeated()
	{
		super(553);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final var win2 = st.getQuestItemsCount(WIN_CONF_2);
			final var win5 = st.getQuestItemsCount(WIN_CONF_5);
			final var win10 = st.getQuestItemsCount(WIN_CONF_10);
			if (win2 > 0 && win5 > 0)
			{
			...
			else if (win2 > 0 && win10 > 0)
			{
				st.giveItems(OLY_CHEST, 7);
				st.giveItems(MEDAL_OF_GLORY, 10);
				st.playSound("ItemSound.quest_finish");
				st.exitQuest(QuestType.DAILY);
			}
			else if (win5 > 0 && win10 > 0)
			{
				st.giveItems(OLY_CHEST, 11);
				st.giveItems(MEDAL_OF_GLORY, 15);
				st.playSound("ItemSound.quest_finish");
				st.exitQuest(QuestType.DAILY);
			}
			else if (win2 > 0)
