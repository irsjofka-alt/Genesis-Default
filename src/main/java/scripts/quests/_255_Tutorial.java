...
import gameserver.network.serverpackets.ExTutorialShowId;
...
public final class _255_Tutorial extends Quest
{
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.startsWith("TE"))
		{
			...
			switch (eventId)
			{
				...
				case 3 :
					...
					break;
				case 4 :
					if (Config.ENABLE_SPECIAL_TUTORIAL && player.getLevel() >= 6 && st.getInt("firstexp") <= 1 && player.getClassId().level() == 0)
					{
						st.set("firstexp", "2");
						htmltext = "Level6.htm";
					}
		...
		else if (event.startsWith("CE"))
		{
			...
			switch (eventId)
			{
				...
				case 40 :
					...
					else
					{
						switch (playerLevel)
						{
							...
							case 28 :
								...
								break;
							case 32 :
								if (Config.ENABLE_SPECIAL_TUTORIAL && st.getInt("lvl") < 32)
								{
									st.set("lvl", "32");
									htmltext = "Level32.htm";
								}
								break;
							case 35 :
								if (st.getInt("lvl") < 35)
							...
							case 48 :
								...
								break;
							case 52 :
								if (Config.ENABLE_SPECIAL_TUTORIAL && st.getInt("lvl") < 52)
								{
									st.set("lvl", "52");
									htmltext = "Level52.htm";
								}
							case 58 :
								if (st.getInt("lvl") < 58)
								{
					...
					break;
				case 41 :
					if (Config.ENABLE_SPECIAL_TUTORIAL && st.getInt("firstexp") == 3 && player.getLevel() > 7)
					{
						st.set("firstexp", "4");
						htmltext = "Level8.htm";
					}
					break;
				case 42 :
					if (Config.ENABLE_SPECIAL_TUTORIAL && st.getInt("lvl") >= 40 && st.getInt("lvl") < 42)
					{
						st.set("lvl", "42");
						htmltext = "Level41Ready.htm";
					}
					break;
				case 45 :
					if (playerLevel < 10)
		...
		else if (event.startsWith("ShowProff"))
		{
			...
			if (player.getClassId().level() == 1 && st.getInt("lvl") < 21)
			{
				st.set("lvl", "21");
				htmltext = "Level21.htm";
			}
			else if (player.getClassId().level() == 2 && st.getInt("lvl") < 41)
			{
				// st.set("lvl", "41");
				/// htmltext = "Level41.htm";
			}
		}
		else if (event.startsWith("ShowBuffer") && st.getInt("firstexp") == 2)
		{
			...
			st.set("firstexp", "3");
			final var handler = CommunityBoardHandler.getInstance().getHandler("_bbsbuffer");
			if (handler != null)
			{
		...
		else if (event.startsWith("GetWeaponD ") && st.getInt("weapon") < 1)
		{
			if (!Config.ENABLE_SPECIAL_TUTORIAL)
			{
				return "";
			...
			final var tokenizer = new StringTokenizer(event, " ");
			tokenizer.nextToken();
			final int itemId = Integer.parseInt(tokenizer.nextToken());
			
			final var createditem = player.addItem("SpecialTutorial", itemId, 1, player, true);
			if (createditem == null)
			{
				st.closeTutorialHtml();
				return null;
			...
			{
				if (((Weapon) createditem.getItem()).getItemType() == WeaponType.BOW)
				{
					player.addItem("SpecialTutorial", 1341, 300, player, true);
				}
				else if (((Weapon) createditem.getItem()).getItemType() == WeaponType.CROSSBOW)
				{
					player.addItem("SpecialTutorial", 9633, 300, player, true);
				}
			}
			
			if (player.getRace() == Race.KAMAEL)
			{
				htmltext = "Level21ArmorKamael.htm";
			}
			else
			{
				htmltext = "Level21Armors.htm";
			}
		}
		else if (event.startsWith("GetArmorD ") && st.getInt("armor") < 1)
		{
			if (!Config.ENABLE_SPECIAL_TUTORIAL)
			{
				return "";
			...
			{
				final int itemId = Integer.parseInt(tokenizer.nextToken());
				final var createditem = player.getInventory().addItem("SpecialTutorial", itemId, 1, player, null);
				if (createditem == null)
				{
					st.closeTutorialHtml();
					return null;
				}
				final var unequipped = player.getInventory().unEquipItemInBodySlotAndRecord(createditem.getItem().getBodyPart(), true);
				...
				player.getInventory().equipItem(createditem, true, false);
				
				final var u = new InventoryUpdate();
				u.addItem(createditem);
				player.sendInventoryUpdate(u);
			}
			htmltext = "Level21Soulshots.htm";
		}
		else if (event.startsWith("GetShotsD ") && st.getInt("shots") < 1)
		{
			if (!Config.ENABLE_SPECIAL_TUTORIAL)
			{
				return "";
			...
			final var createditem = player.addItem("SpecialTutorial", itemId, itemCount, player, true);
			if (createditem == null)
			{
				st.closeTutorialHtml();
				return null;
			...
			{
				final var shortCut = new ShortCutTemplate(11, 0, ShortcutType.ITEM, createditem.getObjectId(), -1, 1);
				player.sendPacket(new ShortCutRegister(shortCut));
				player.registerShortCut(shortCut);
			}
			htmltext = "Level21Cruma.htm";
		}
		...
		else if (event.startsWith("ShowLocation "))
		{
			...
			final var npcLoc = GameObjectsStorage.getByNpcId(npcId);
			if (npcLoc != null)
			{
				player.sendPacket(new RadarControl(2, 2, 0, 0, 0));
				player.sendPacket(new RadarControl(0, 2, npcLoc.getLocation().getX(), npcLoc.getLocation().getY(), npcLoc.getLocation().getZ()));
			}
			st.closeTutorialHtml();
		}
		else if (event.startsWith("PortToLoc "))
		{
			if (!Config.ENABLE_SPECIAL_TUTORIAL)
			{
				return "";
			...
			final int z = Integer.parseInt(tokenizer.nextToken());
			player.teleToLocation(x, y, z, true, player.getReflection());
			st.closeTutorialHtml();
		}
