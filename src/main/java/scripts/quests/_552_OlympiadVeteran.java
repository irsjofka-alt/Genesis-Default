...
public class _552_OlympiadVeteran extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final long team = st.getQuestItemsCount(Team_Event_Certificate);
			final long free = st.getQuestItemsCount(Class_Free_Battle_Certificate);
			final long classes = st.getQuestItemsCount(Class_Battle_Certificate);
			
			if (team > 0 && free > 0 && classes > 0)
			{
				st.giveItems(OLY_CHEST, 6);
				st.playSound("ItemSound.quest_finish");
				st.exitQuest(QuestType.DAILY);
			}
			else if (team > 0 && free > 0)
			{
				st.giveItems(OLY_CHEST, 5);
				st.playSound("ItemSound.quest_finish");
				st.exitQuest(QuestType.DAILY);
			}
			else if (team > 0 && classes > 0)
			{
				st.giveItems(OLY_CHEST, 4);
				st.playSound("ItemSound.quest_finish");
				st.exitQuest(QuestType.DAILY);
			}
			else if (free > 0 && classes > 0)
			{
