...
public class _503_PursuitClanAmbition extends Quest
{
	...
	private _503_PursuitClanAmbition()
	{
		super(503);
	...
	public final String onAttack(Npc npc, Player player, int damage, boolean isSummon, Skill skill)
	{
		if ((npc.getMaxHp() / 2) > npc.getCurrentHp())
		{
			if (getRandom(100) < 4)
			{
				...
				{
					final Collection<Player> plrs = World.getAroundPlayers(npc);
					if (!plrs.isEmpty())
					{
	...
	private void exit(boolean complete, QuestState st)
	{
		...
		try
		{
			...
			for (final Player i : members)
			{
				...
				final QuestState qs = i.getQuestState(getName());
				if (qs != null)
				{
					qs.exitQuest(true);
				}
	...
	private int getLeaderVar(QuestState st, String var)
	{
		...
		{
			IO.println("Pursuit of Clan Ambition: cannot read quest states offline clan leader");
		}
		return val;
	}
