...
public class FreyaBattleEasy extends AbstractReflection
{
			...
			if (isInReflection(r))
			{
				...
				switch (event)
				{
					case "openDoor" :
					{
						if (npc.isScriptValue(0))
						{
							...
							if (knightStatues != null)
							{
								...
								{
									final Npc statue = addSpawn(18919, loc, false, 0, false, r);
									knightStatues.add(statue);
								}
							}
							
							for (final var players : r.getReflectionPlayers())
							{
								if (!players.isDead())
								{
									final var qs = players.getQuestState("_10286_ReunionWithSirra");
					...
					case "STAGE_2_GLAKIAS" :
					{
						...
						addSpawn(25699, new Location(114707, -114799, -11199, 15956), false, 0, true, r);
						break;
					}
					case "STAGE_3_MOVIE" :
					{
					...
					case "STAGE_3_START" :
					{
						...
						final var freya = (GrandBossInstance) addSpawn(29179, new Location(114720, -117085, -11088, 15956), false, 0, true, r);
						r.setParam("freya", freya);
						r.setStatus(4);
						notifyEvent("START_SPAWN", controller, null);
						if (!freya.isInCombat())
					...
					case "SPAWN_KNIGHT" :
					{
						...
						if (canSpawnMobs)
						{
							...
							if (spawnedMobs != null)
							{
								...
								final var knight = (Attackable) addSpawn(18855, npc.getLocation(), false, 0, false, r);
	...
	{
		if (enterReflection(player, npc, 139))
		{
	...
	protected void onTeleportEnter(Player player, ReflectionTemplate template, Reflection r, boolean firstEntrance)
	{
		...
		{
			final Location teleLoc = r.isStatus(4) ? new Location(114694, -113700, -11200) : template.getTeleportCoord();
			player.getAI().setIntention(CtrlIntention.IDLE);
			player.teleToLocation(teleLoc, true, r);
			if (player.hasSummon())
			{
	...
	private void guardSpawn(int mode, Reflection r)
	{
		if (r != null)
		{
			...
			if (canSpawn)
			{
				...
				{
					final var knight = (Attackable) addSpawn(18855, SpawnTerritory.getRandomLoc(CENTRAL_ROOM, false), false, 0, false, r);
					if (spawnedMobs != null)
					{
						spawnedMobs.add(knight);
					}
