...
public class Kamaloka extends AbstractReflection
{
	...
	{
		if (enterReflection(player, npc, INSTANCE_IDS[index]))
		{
			final var r = player.getReflection();
			if (isInReflection(r))
			{
				r.setParam("index", index);
				spawnKama(r);
			}
		}
	}
	...
	private final void spawnKama(Reflection r)
	{
		int[] npcs;
		int[][] spawns;
		Npc npc;
		final int index = r.getParams().getInteger("index");

		npcs = FIRST_ROOM[index];
		spawns = FIRST_ROOM_SPAWNS[index];
		if (npcs != null)
		{
			final List<Spawner> firstRoom = new ArrayList<>(spawns.length - 1);
			...
			for (int i = 0; i < spawns.length; i++)
			{
				if (i == shaman)
				{
					npc = addSpawn(STEALTH_SHAMAN ? npcs[1] : npcs[0], spawns[i][0], spawns[i][1], spawns[i][2], 0, false, 0, false, r);
					r.setParam("shaman", npc.getObjectId());
				}
				else
				{
					npc = addSpawn(npcs[1], spawns[i][0], spawns[i][1], spawns[i][2], 0, false, 0, false, r);
		...
		if (npcs != null)
		{
			...
			{
				npc = addSpawn(npcs[0], spawn[0], spawn[1], spawn[2], 0, false, 0, false, r);
				npc.setIsNoRndWalk(true);
		...
		{
			npc = addSpawn(MINIBOSS[index][0], MINIBOSS[index][1], MINIBOSS[index][2], MINIBOSS[index][3], 0, false, 0, false, r);
			npc.setIsNoRndWalk(true);
			r.setParam("miniBoss", npc.getObjectId());
		}

		if (TELEPORTERS[index] != null)
		{
			addSpawn(32496, TELEPORTERS[index][0], TELEPORTERS[index][1], TELEPORTERS[index][2], 0, false, 0, false, r);
		}

		npc = addSpawn(BOSS[index][0], BOSS[index][1], BOSS[index][2], BOSS[index][3], 0, false, 0, false, r);
	...
	public final String onTalk(Npc npc, Player player)
	{
		...
		if (npcId == 32496 || npcId == 4314)
		{
			...
			if (isInReflection(r))
			{
				...
				final var template = ReflectionParser.getInstance().getReflectionId(INSTANCE_IDS[r.getParams().getInteger("index")]);
				if (template != null)
				{
					foundTemplate = true;
				...
				if ((!foundTemplate && player.isInParty() && player.getParty().isLeader(player)) || (foundTemplate && canTalk))
				{
					if (r.isAllowed(player.getObjectId()))
					{
						...
						if (party != null)
						{
							...
							{
								if ((member != null) && (member.getReflectionId() == r.getId()))
								{
									teleportPlayer(member, r.getReturnLoc(), ReflectionManager.DEFAULT);
								}
							}
						}
						else
	...
	public final String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (isInReflection(r))
		{
			...
			final var index = r.getParams().getInteger("index");
			final var firstRoom = r.getParams().getList("firstRoom", Spawner.class);
			if (firstRoom != null)
			{
				if ((r.getParams().getInteger("shaman", 0) != 0) && (r.getParams().getInteger("shaman", 0) == objectId))
				{
					r.setParam("shaman", 0);
					for (final var spawn : firstRoom)
					{
						if (spawn != null)
						{
							spawn.stopRespawn();
					...
					r.getParams().remove("firstRoom");

					final var boss = r.getParams().getObject("boss", Npc.class);
					if (boss != null)
					{
						final int skillId = FIRST_ROOM[index][2];
						final int skillLvl = FIRST_ROOM[index][3];
						if ((skillId != 0) && (skillLvl != 0))
						{
							final Skill skill = SkillsParser.getInstance().getInfo(skillId, skillLvl);
							if (skill != null)
							{
								skill.getEffects(boss, boss, false, true);
							}
						}
					}
			...
			final var secondRoom = r.getParams().getList("secondRoom", Integer.class);
			if (secondRoom != null)
			{
				boolean all = true;
				for (int i = 0; i < secondRoom.size(); i++)
				{
					if (secondRoom.get(i) == objectId)
					{
						secondRoom.set(i, 0);
					}
					else if (secondRoom.get(i) != 0)
					{
						all = false;
					}
				}
				...
				if (all)
				{
					r.getParams().remove("secondRoom");
					final var boss = r.getParams().getObject("boss", Npc.class);
					if (boss != null)
					{
						final int skillId = SECOND_ROOM[index][1];
						final int skillLvl = SECOND_ROOM[index][2];
						if ((skillId != 0) && (skillLvl != 0))
						{
							final Skill skill = SkillsParser.getInstance().getInfo(skillId, skillLvl);
							if (skill != null)
			...
			final var miniBoss = r.getParams().getInteger("miniBoss", 0);
			if ((miniBoss != 0) && (miniBoss == objectId))
			{
				r.setParam("miniBoss", 0);
				final var boss = r.getParams().getObject("boss", Npc.class);
				if (boss != null)
				{
					final int skillId = MINIBOSS[index][4];
					final int skillLvl = MINIBOSS[index][5];
					if ((skillId != 0) && (skillLvl != 0))
					{
						final Skill skill = SkillsParser.getInstance().getInfo(skillId, skillLvl);
						if (skill != null)
			...
			{
				r.getParams().remove("boss");
				addSpawn(4314, BOSS[index][1], BOSS[index][2], BOSS[index][3], 0, false, 0, false, r);
