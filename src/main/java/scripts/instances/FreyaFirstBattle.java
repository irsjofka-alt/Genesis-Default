...
public final class FreyaFirstBattle extends AbstractReflection
{
	...
	{
		if (enterReflection(player, npc, 137))
		{
			final var r = player.getReflection();
			if (isInReflection(r))
			{
				r.setParam("refPlayer", player);
				r.openDoor(23140101);
			}
		}
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			...
			case "TIMER_PC_LEAVE" :
			{
				final var qs = player.getQuestState("_10285_MeetingSirra");
				if ((qs != null))
				{
					qs.setMemoState(3);
					qs.setCond(10, true);
	...
	public String onSpellFinished(Npc npc, Player player, Skill skill)
	{
		...
		if (isInReflection(r))
		{
			final var refPlayer = r.getParams().getObject("refPlayer", Player.class);
			if (skill == new SkillHolder(6276, 1).getSkill() && refPlayer != null)
			{
				startQuestTimer("TIMER_SCENE_21", 1000, npc, refPlayer);
			}
		}
