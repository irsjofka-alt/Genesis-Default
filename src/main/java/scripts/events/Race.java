...
public class Race extends AbstractWorldEvent
{
	...
	public boolean eventStart(long totalTime, boolean force)
	{
		...
		final var msg1 = new ServerMessage("EventRace.START_MSG_1", true);
		Announcements.getInstance().announceToAll(msg1);
		
		final var msg2 = new ServerMessage("EventRace.START_MSG_2", true);
	...
	protected void startRace(long totalTime)
	{
		...
		if (_players.isEmpty())
		{
			final var msg = new ServerMessage("EventRace.ABORTED", true);
		...
		final var msg = new ServerMessage("EventRace.RACE_START", true);
		Announcements.getInstance().announceToAll(msg);
		
		final var loc = _template.getLocations().get(Rnd.get(_template.getLocations().size()));
	...
	public boolean eventStop()
	{
		...
		final var msg = new ServerMessage("EventRace.STOP", true);
		Announcements.getInstance().announceToAll(msg);
	...
	private void transformPlayer(Player player)
	{
		...
		for (final var e : player.getAllEffects())
		{
			if (e.getAbnormalType().equalsIgnoreCase("SPEED_UP") || ((e.getSkill() != null) && ((e.getSkill().getId() == 268) || (e.getSkill().getId() == 298))))
			{
				e.exit(false);
			}
		}
		SkillsParser.getInstance().getInfo(_skill, 1).getEffects(player, player, false, true);
	}
	...
	{
		final var msg = new ServerMessage("EventRace.TIME_UP", true);
		Announcements.getInstance().announceToAll(msg);
		eventStop();
	}
	...
	private void winRace(Player player)
	{
		if (_isActive)
		{
			...
			final var msg = new ServerMessage("EventRace.WINNER", true);
