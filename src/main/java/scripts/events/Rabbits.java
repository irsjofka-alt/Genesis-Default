...
public class Rabbits extends AbstractWorldEvent
{
	...
	public boolean eventStart(long totalTime, boolean force)
	{
		...
		final var npcLoc = _template.getLocations().get(rndSelect).getLocation();
		final var territory = _template.getTerritories().get(rndSelect).getTerritory();
		...
		final var msg1 = new ServerMessage("EventRabbits.START_MSG_1", true);
		Announcements.getInstance().announceToAll(msg1);

		final var msg2 = new ServerMessage("EventRabbits.START_MSG_2", true);
		Announcements.getInstance().announceToAll(msg2);

		final var msg3 = new ServerMessage("EventRabbits.START_MSG_3", true);
	...
	public boolean eventStop()
	{
		...
		final var msg = new ServerMessage("EventRabbits.STOP", true);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("transform"))
		{
			...
			SkillsParser.getInstance().getInfo(2428, 1).getEffects(npc, player, false, true);
	...
	public String onSkillSee(Npc npc, Player caster, Skill skill, GameObject[] targets, boolean isSummon)
	{
		if (skill.getId() == _skill_tornado && npc.isVisible() && caster.getTarget() == npc)
		{
			...
			if (_chest_count <= 0)
			{
				final var msg = new ServerMessage("EventRabbits.NO_MORE", true);
				Announcements.getInstance().announceToAll(msg);
				eventStop();
			}
		}
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon, Skill skill)
	{
		...
		{
			SkillsParser.getInstance().getInfo(4515, 1).getEffects(npc, attacker, false, true);
		}
