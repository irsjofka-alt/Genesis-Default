...
public class HeavyMedal extends AbstractWorldEvent
{
	...
	public boolean eventStart(long totalTime, boolean force)
	{
		...
		final var msg = new ServerMessage("EventHeavyMedal.START", true);
	...
	public boolean eventStop()
	{
		...
		final var msg = new ServerMessage("EventHeavyMedal.STOP", true);
		Announcements.getInstance().announceToAll(msg);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("game"))
		{
			...
			final var requestItems = _template.getVariantRequests().get(level);
			if (requestItems != null && !requestItems.isEmpty())
			{
				for (final var request : requestItems)
				{
		...
		else if (event.equalsIgnoreCase("heads") || event.equalsIgnoreCase("tails"))
		{
			...
			else
			{
				...
				else
				{
					if (level > 0)
					{
						final var rewards = _template.getVariantRewards().get(level - 1);
