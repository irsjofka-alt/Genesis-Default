...
public class GrandWedding extends AbstractWorldEvent
{
	...
	public boolean eventStart(long totalTime, boolean force)
	{
		...
		final var msg = new ServerMessage("GrandWedding.START", true);
	...
	public boolean eventStop()
	{
		...
		final var msg = new ServerMessage("GrandWedding.STOP", true);
		Announcements.getInstance().announceToAll(msg);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
					...
					if (HUSBAND_OK == false)
					{
						if ((_husbandCoupleId == 0) && (_wifeCoupleId == 0))
						{
							if (player.getQuestState(getName()).getQuestItemsCount(57) >= Config.WEDDING_PRICE)
							{
								...
								{
									_players = World.getAroundPlayers(i);
		...
		{
			final var msg = new ServerMessage("GrandWedding.MSG_05", true);
		...
		if (event.equals("WeddingTeleport"))
		{
			...
			final var msg = new ServerMessage("GrandWedding.MSG_07", _husband.getLang());
		...
		{
			var witness = addSpawn(102508, -50034, -54068, -2688, 48643, false, 0, false, getReflection());
			guards.add(witness);
		...
		if (event.equals("PixiesSpawn"))
		{
			for (int i = 0; i < 45; i++)
			{
				...
				final var pixy = addSpawn(PIXY_ID, xx, yy, -2824, 0, false, 0, false, getReflection());
				pixy.setRunning();
				_pixies.add(pixy);
			}
		...
		if (event.equals("entertainmentSpawn"))
		{
			for (int i = 0; i < 24; i++)
			{
				...
				final var show = addSpawn(entertainmentId[rr], xx, yy, -2674, 0, false, 0, false, getReflection());
				show.setRunning();
		...
		{
			for (final var i : _entertainment)
			{
		...
		if (event.equals("entertainmentMove2"))
		{
			...
			for (final var show : _entertainment)
			{
				show.setWalking();
			}
		...
		if (event.equals("SpecialGuestsSpawn"))
		{
			for (int i = 0; i < specialGuests.length; i++)
			{
				...
				final var guard = addSpawn(specialGuests[i], x1, y1, -2808, 58609, false, 0, false, getReflection());
				_guests.add(guard);
			}
		...
		{
			final var msg = new ServerMessage("GrandWedding.MSG_17", _husband.getLang());
			msg.add(_wife.getName(null));
			msg.add(_husband.getName(null));
		...
		{
			final var msg = new ServerMessage("GrandWedding.MSG_18", _husband.getLang());
			msg.add(_husband.getName(null));
			msg.add(_wife.getName(null));
			_anakim.broadcastPacketToOthers(new CreatureSay(_anakim.getId(), 0, "Anakim", msg.toString()));
			_weddingStep = 1;
		...
		{
			final var msg = new ServerMessage("GrandWedding.MSG_19", _husband.getLang());
			msg.add(_wife.getName(null));
			msg.add(_husband.getName(null));
			_anakim.broadcastPacketToOthers(new CreatureSay(_anakim.getId(), 0, "Anakim", msg.toString()));
			_weddingStep = 2;
		...
		if (event.equals("AnakimSpeak9"))
		{
			...
			_husband.setRunning();
			_wife.setRunning();
			final var couple = CoupleManager.getInstance().getCouple(_husbandCoupleId);
			couple.marry();
			if (_husband != null)
			{
		...
		if (event.equals("WeddingFinale"))
		{
			...
			for (int i = 0; i < 25; i++)
			{
				...
				xx = -51228 + (Rnd.get(1200) - 600);
				yy = -54178 + (Rnd.get(1200) - 600);
				final var gourd = addSpawn(Gourd[rr], xx, yy, -2824, 0, false, 0, false, getReflection());
				_gourds.add(gourd);
			}
		...
		if (event.equals("WeddingFinale3"))
		{
			...
			for (final var i : _entertainment2)
			{
				xx = -51867;
				yy = -54120;
				i.getAI().setIntention(CtrlIntention.MOVING, new Location(xx, yy, -2825, 0), 0);
		...
		if (event.equals("weddingDespawn"))
		{
			...
			_pet1.deleteMe();
			_pet2.deleteMe();
			for (final var s : _entertainment2)
			{
				s.deleteMe();
			}
			for (final var s : _entertainment)
			{
				s.deleteMe();
			}
			for (final var n : _pixies)
			{
				n.deleteMe();
			}
			for (final var i : guards)
	...
	protected boolean wearsFormalWear(Player player)
	{
		...
		{
			final var fw1 = player.getChestArmorInstance();
