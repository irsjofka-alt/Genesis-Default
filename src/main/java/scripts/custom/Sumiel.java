...
import gameserver.listener.actor.OnMagicUseListener;
...
public class Sumiel extends AbstractNpcAI implements OnMagicUseListener
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			case "timer" :
			{
				...
				{
					final var b = room.getBurners()[room.getOrder()[room.getCurrentPot()]];
	...
	public String onFirstTalk(Npc npc, Player talker)
	{
		...
		final var room = getRoomByManager(npc);
		final boolean miniGameStarted = room.getStarted();
	...
	public void onMagicUse(Creature actor, Skill skill, GameObject[] targets, boolean alt)
	{
		...
		final var room = getRoomByParticipant((Player) actor);
		final boolean miniGameStarted = room.getStarted();
					...
					if (npc.getId() == BURNER)
					{
						...
						if (pos == room.getOrder()[room.getCurrentPot()])
						{
							...
							else
							{
								...
								addSpawn(TREASURE_BOX, Location.findAroundPosition(actor, 100), true, 0);
