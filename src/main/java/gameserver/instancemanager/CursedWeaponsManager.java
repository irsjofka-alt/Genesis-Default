...
public class CursedWeaponsManager extends LoggerObject
{
	...
	private final void load()
	{
		try
		{
			...
			final var file = new File(Config.DATAPACK_ROOT + "/data/stats/items/cursedWeapons.xml");
			if (!file.exists())
			{
				...
				if ("list".equalsIgnoreCase(n.getNodeName()))
				{
					for (var d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						if ("item".equalsIgnoreCase(d.getNodeName()))
						{
							...
							final var cw = new CursedWeapon(id, skillId, name);
							int val;
	...
	private final void restore()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				final var cw = _cursedWeapons.get(itemId);
	...
	public final void controlRepairPlayer(int objId)
	{
		Connection con = null;
		PreparedStatement statement = null;
		ResultSet rset = null;
		try
		{
			...
			for (final var cw : _cursedWeapons.values())
			{
				if (cw.isActivated())
				{
					continue;
	...
	{
		final var cw = _cursedWeapons.get(item.getId());
		if (player.isCursedWeaponEquipped())
		{
			final var cw2 = _cursedWeapons.get(player.getCursedWeaponEquippedId());
	...
	public void checkPlayer(Player player)
	{
		...
		for (final var cw : _cursedWeapons.values())
		{
			if (cw.isActivated() && (player.getObjectId() == cw.getPlayerId()))
			{
				...
				final var sm = SystemMessage.getSystemMessage(SystemMessageId.S2_MINUTE_OF_USAGE_TIME_ARE_LEFT_FOR_S1);
