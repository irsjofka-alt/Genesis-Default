...
public final class CastleManager extends LoggerObject
{
	...
	private final List<Castle> _castles = new LinkedList<>();
			...
			if (player != null)
			{
				try
				{
					...
					if (circlet != null)
					{
						...
						{
							player.getInventory().unEquipItemInSlot(circlet.getLocationSlot(), true);
						}
	...
	{
		_castles.stream().filter(c -> c != null).forEach(c -> c.activateInstance());
	}
