...
import gameserver.model.zone.form.ZoneCuboid;
import gameserver.model.zone.form.ZoneCylinder;
import gameserver.model.zone.form.ZoneNPoly;
import gameserver.model.zone.type.ArenaZone;
import gameserver.model.zone.type.BossZone;
import gameserver.model.zone.type.OlympiadStadiumZone;
...
public class ZoneManager extends DocumentParser
{
	...
	private final List<ZoneType> _timeZones = new ArrayList<>();
	private final List<BossZone> _bossZones = new ArrayList<>();
			...
			if ("list".equalsIgnoreCase(n.getNodeName()))
			{
				...
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					if ("zone".equalsIgnoreCase(d.getNodeName()))
					{
						...
						try
						{
							newZone = Class.forName("gameserver.model.zone.type." + zoneType);
	...
	{
		_reflectionZones.stream().filter(z -> z != null).forEach(z -> z.generateReflection());
	}
	
	public void generateTimeZones()
	{
		_timeZones.stream().filter(z -> z != null).forEach(z -> z.calcActivationTime());
		_bossZones.stream().filter(z -> z != null).forEach(z -> EpicBossManager.getInstance().addZone(z));
	}
