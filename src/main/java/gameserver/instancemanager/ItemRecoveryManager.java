...
import gameserver.model.actor.templates.items.ItemRecovery;
		...
		if (item != null)
		{
			...
			if (item.getElementals() != null && !item.getElementals().isEmpty())
			{
				...
				for (final String el : elements)
				{
					...
					if (element != null)
					{
						itemRecovery.setElementAttr(Byte.parseByte(element[0]), Integer.parseInt(element[1]), false);
					}
				}
				itemRecovery.updateItemElementals();
			}
