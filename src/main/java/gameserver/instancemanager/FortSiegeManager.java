...
import gameserver.model.CombatFlag;
import gameserver.model.GameObject;
import gameserver.model.actor.Player;
import gameserver.model.entity.Fort;
import gameserver.model.entity.FortSiege;
import gameserver.model.items.instance.ItemInstance;
import gameserver.model.spawn.SpawnFortSiege;
...
public final class FortSiegeManager extends LoggerObject
{
	...
	{
		final GameSettings siegeSettings = new GameSettings();
		...
		_attackerMaxClans = siegeSettings.getProperty("AttackerMaxClans", 500);
		_flagMaxCount = siegeSettings.getProperty("MaxFlags", 1);
		_siegeClanMinLevel = siegeSettings.getProperty("SiegeClanMinLevel", 4);
		_siegeLength = siegeSettings.getProperty("SiegeLength", 60);
		_countDownLength = siegeSettings.getProperty("CountDownLength", 10);
		_suspiciousMerchantRespawnDelay = siegeSettings.getProperty("SuspiciousMerchantRespawnDelay", 180);
		_fortHwidLimit = siegeSettings.getProperty("FortSiegeLimitPlayers", 0);
		...
		for (final Fort fort : FortManager.getInstance().getForts())
		{
			...
			for (int i = 1; i < 5; i++)
			{
				final String _spawnParams = siegeSettings.getProperty(fort.getName().replace(" ", "") + "Commander" + i, "", false);
			...
			for (int i = 1; i < 4; i++)
			{
				final String _spawnParams = siegeSettings.getProperty(fort.getName().replace(" ", "") + "Flag" + i, "", false);
				if (_spawnParams.isEmpty())
				{
					break;
				}
			...
			for (int i = 1; i < 5; i++)
			{
				final String _spawnParams = siegeSettings.getProperty(fort.getName().replace(" ", "") + "PowerUnit" + i, "", false);
				if (_spawnParams.isEmpty())
				{
					break;
				}
			...
			for (int i = 1; i < 5; i++)
			{
				final String _spawnParams = siegeSettings.getProperty(fort.getName().replace(" ", "") + "ControlUnit" + i, "", false);
				if (_spawnParams.isEmpty())
				{
					break;
				}
			...
			for (int i = 1; i < 2; i++)
			{
				final String _spawnParams = siegeSettings.getProperty(fort.getName().replace(" ", "") + "MainMachine" + i, "", false);
				if (_spawnParams.isEmpty())
				{
					break;
				}
	...
	{
		final var fort = FortManager.getInstance().getFort(fortId);
		if (fort != null)
		{
			final var fcf = _flagList.get(fort.getId());
			for (final var cf : fcf)
			{
				if (cf.getObjectId() == player.getObjectId())
				{
					cf.dropIt();
