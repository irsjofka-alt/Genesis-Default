...
public class DayNightSpawnManager extends LoggerObject
{
	...
	private void spawnCreatures(List<Spawner> unSpawnCreatures, List<Spawner> spawnCreatures, String UnspawnLogInfo, String SpawnLogInfo)
	{
		try
		{
			...
			{
				int i = 0;
				for (final var spawn : unSpawnCreatures)
				{
					if (spawn == null)
					{
						continue;
					...
					spawn.stopRespawn();
					final var last = spawn.getLastSpawn();
					if (last != null)
					{
						last.onDecay();
						i++;
			...
			int i = 0;
			for (final var spawnDat : spawnCreatures)
			{
				if (spawnDat == null)
				{
					continue;
	...
	public void specialNightBoss(int mode)
	{
		try
		{
			...
			for (final var spawn : _bosses.keySet())
			{
	...
	{
		final var npc = boss.getSpawn().getLastSpawn();
		switch (mode)
		{
			case 0 :
				if (npc != null)
	...
	public RaidBossInstance handleBoss(Spawner spawnDat)
	{
		...
		if (!GameTimeController.getInstance().isNight())
		{
			final var npc = raidboss.getSpawn().getLastSpawn();
			if (npc != null)
			{
				npc.onDecay();
				return null;
