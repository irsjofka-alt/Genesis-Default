	...
	private class BotQuestionAsked implements Runnable
	{
		...
		public void run()
		{
			for (final var player : GameObjectsStorage.getPlayers())
			{
				...
				if (player.isInTownZone() || player.isInKrateisCube() || player.getUCState() > 0 || player.isInFightEvent() || player.inObserverMode() || player.isInsideZone(ZoneId.PVP) || player.isInSiege() || player.isInDuel() || player.isInStoreMode() || player.isInOfflineMode() || player.getPvpFlag() != 0 || player.isInOlympiadMode())
				{
					continue;
				}
				...
				for (final var mob : World.getAroundNpc(player, 1000, 600))
				{
					if (mob.isMonster())
					{
