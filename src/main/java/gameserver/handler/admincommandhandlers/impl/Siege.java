...
public class Siege implements IAdminCommandHandler
{
	...
	private void showSiegeClanHallSelectPage(Player activeChar)
	{
		...
		for (final SiegableHall hall : CHSiegeManager.getInstance().getConquerableHalls().values())
		{
			if (hall != null)
			{
				StringUtil.append(cList, "<td fixwidth=150 align=center><button value=\"" + hall.getTemplate().getName(activeChar.getLang()) + "\" action=\"bypass -h admin_chsiege_siegablehall ", String.valueOf(hall.getId()), "\" width=140 height=21 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
				i++;
			}
			if (i > 1)
			{
	...
	private void showClanHallSelectPage(Player activeChar)
	{
		int i = 0;
		final NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		adminReply.setFile(activeChar, activeChar.getLang(), "data/html/admin/hallinfo.htm", false);
		final StringBuilder cList = new StringBuilder(500);
		for (final ClanHall clanhall : ClanHallManager.getInstance().getClanHalls().values())
		{
			if (clanhall != null)
			{
				StringUtil.append(cList, "<td fixwidth=150 align=center><a action=\"bypass -h admin_clanhall ", String.valueOf(clanhall.getId()), "\">", clanhall.getTemplate().getName(activeChar.getLang()), "</a></td>");
				i++;
			}
			if (i > 1)
			{
	...
	{
		final NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		adminReply.setFile(activeChar, activeChar.getLang(), "data/html/admin/castle.htm", false);
	...
	private void showClanHallPage(Player activeChar, ClanHall clanhall)
	{
		...
		adminReply.replace("%clanhallName%", clanhall.getTemplate().getName(activeChar.getLang()));
	...
	private void showSiegableHallPage(Player activeChar, SiegableHall hall)
	{
		...
		msg.replace("%clanhallName%", hall.getTemplate().getName(activeChar.getLang()));
