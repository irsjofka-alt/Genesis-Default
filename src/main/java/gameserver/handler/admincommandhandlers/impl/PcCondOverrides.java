...
public class PcCondOverrides implements IAdminCommandHandler
{
	...
	private void refreshHidePlayers(Player player, boolean isEnable)
	{
		if (isEnable)
		{
			World.getAroundPlayers(player).stream().filter(p -> p != null && p.isInvisible()).forEach(p -> p.broadcastInfo());
		}
		else
		{
			World.getAroundPlayers(player).stream().filter(p -> p != null && p.isInvisible()).forEach(p -> player.sendPacket(new DeleteObject(p)));
		}
	}
