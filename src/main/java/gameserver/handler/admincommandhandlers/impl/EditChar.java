...
public class EditChar implements IAdminCommandHandler
{
	...
	private void listCharacters(Player activeChar, int type, int page)
	{
		...
		adminReply.setFile(activeChar, activeChar.getLang(), "data/html/admin/charlist.htm", false);
		var list = "";
	...
	private void gatherCharacterInfo(Player activeChar, Player player, String filename)
	{
		...
		adminReply.replace("%region%", player.getWorldRegion() != null ? player.getWorldRegion().toString() : "");
	...
	private void gatherSummonInfo(Summon target, Player activeChar)
	{
		...
		html.replace("%ai%", target.getAI() != null ? String.valueOf(target.getAI().getIntention().name()) : "NULL");
