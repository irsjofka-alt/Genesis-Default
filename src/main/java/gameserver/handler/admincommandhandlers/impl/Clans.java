	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		switch (cmd)
		{
			case "admin_clan_info" :
			{
				...
				html.replace("%clan_has_clanhall%", clan.getHideoutId() > 0 ? ClanHallManager.getInstance().getClanHallById(clan.getHideoutId()).getTemplate().getName(activeChar.getLang()) : "No");
				html.replace("%clan_has_fortress%", clan.getFortId() > 0 ? FortManager.getInstance().getFort(clan.getFortId()).getName() : "No");
