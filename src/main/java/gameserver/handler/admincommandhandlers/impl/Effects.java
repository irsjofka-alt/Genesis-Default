...
import gameserver.network.serverpackets.ExRedSky;
...
import gameserver.network.serverpackets.SunRise;
import gameserver.network.serverpackets.SunSet;
...
public class Effects implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.equals("admin_para_all"))
		{
			try
			{
				for (final Player player : World.getAroundPlayers(activeChar))
				{
					if (!player.isGM())
					{
		...
		else if (command.startsWith("admin_gmspeed"))
		{
			...
			try
			{
				...
				activeChar.stopSkillEffects(7029, true);
				if ((val >= 1) && (val <= 4))
				{
		...
		else if (command.startsWith("admin_setteam_close"))
		{
			try
			{
				...
				for (final Creature player : World.getAroundCharacters(activeChar, radius, 200))
				{
					...
					if (target != null)
					{
						...
						else
						{
							try
							{
								...
								for (final GameObject object : World.getAroundObjects(activeChar))
								{
