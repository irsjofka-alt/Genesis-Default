				...
				case "aggro" :
				{
					try
					{
						...
						if (pg != null)
						{
							...
							var aggroList = npc.getAggroList().getAggroInfo();
							final int perpage = 8;
							int counter = 0;
							...
							for (int i = (page - 1) * perpage; i < totalSize; i++)
							{
								...
								if (data != null)
								{
									final var attacker = objects.get(data.getAttackerId());
									if (attacker != null)
									{
										block = template;
										block = block.replace("{name}", (attacker.isSummon() || attacker.isPet()) ? ((Summon) attacker).getSummonName(activeChar.getLang()) : attacker.getName(activeChar.getLang()));
