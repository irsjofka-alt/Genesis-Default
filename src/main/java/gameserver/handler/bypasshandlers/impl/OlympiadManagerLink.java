...
import gameserver.data.holder.NpcBufferHolder.NpcBufferData;
...
import gameserver.network.serverpackets.ExHeroList;
	...
	public final boolean useBypass(String command, Player activeChar, Creature target)
	{
		...
		try
		{
			...
			else if (command.toLowerCase().startsWith("olympiadnoble"))
			{
				...
				if ((!activeChar.isNoble() && !Config.ALLOW_REG_WITHOUT_NOBLE) || activeChar.getClassId().level() < Config.OLY_REG_PARAM[1])
				{
			...
			else if (command.toLowerCase().startsWith("olybuff"))
			{
				...
				if (activeChar.olyBuff > 0)
				{
					if (skill != null)
					{
						...
						skill.getEffects(activeChar, activeChar, false, true);
						...
						if (summon != null)
						{
							...
							skill.getEffects(summon, summon, false, true);
						}
					}
				}
