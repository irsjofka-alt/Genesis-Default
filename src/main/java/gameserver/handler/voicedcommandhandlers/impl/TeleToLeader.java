...
public class TeleToLeader implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		...
		if (command.equalsIgnoreCase("teletocl"))
		{
			...
			else if (leader.isInFightEvent() || ((AerialCleftEvent.getInstance().isStarted() || AerialCleftEvent.getInstance().isRewarding()) && AerialCleftEvent.getInstance().isPlayerParticipant(leader.getObjectId())))
			{
			...
			else if (activeChar.isCombatFlagEquipped() || activeChar.isFishing() || activeChar.isDead() || activeChar.isInCombat() || activeChar.isInStoreMode() || activeChar.isSellingBuffs())
			{
				activeChar.sendMessage((new ServerMessage("Community.ALL_DISABLE", activeChar.getLang())).toString());
				return false;
			}
