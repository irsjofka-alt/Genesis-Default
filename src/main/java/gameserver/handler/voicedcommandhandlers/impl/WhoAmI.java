...
public class WhoAmI implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player player, String targets)
	{
		...
		final double magicRes = 100 * (playerToShow.calcStat(Stats.MAGIC_SUCCESS_RES, 1, null, null)) - 100;
		...
		final double pvpPhysSkillDmg = 100 * (playerToShow.calcStat(Stats.PVP_PHYS_SKILL_DMG, 1, null, null) - 1);
		final double pvpPhysSkillDef = 100 * (playerToShow.calcStat(Stats.PVP_PHYS_SKILL_DEF, 1, null, null) - 1);
		...
		final double pveBowDmg = 100 * (playerToShow.calcStat(Stats.PVE_BOW_DMG, 1, null, null) - 1);
		final double pveBowSkillDmg = 100 * (playerToShow.calcStat(Stats.PVE_BOW_SKILL_DMG, 1, null, null) - 1);
		final double pveBowDef = 100 * (playerToShow.calcStat(Stats.PVE_BOW_DEF, 1, null, null) - 1);
		final double pveBowSkillDef = 100 * (playerToShow.calcStat(Stats.PVE_BOW_SKILL_DEF, 1, null, null) - 1);
		...
		double skillMastery = 0.;
		final int Str = player.getSTR();
		final int Int = player.getINT();
		final int lvl331 = player.getSkillLevel(331);
		final int lvl330 = player.getSkillLevel(330);
		if (lvl331 > 0 && lvl330 > 0)
		{
			skillMastery = Str > Int ? player.calcStat(Stats.SKILL_MASTERY, Str, null, null) : player.calcStat(Stats.SKILL_MASTERY, Int, null, null);
		}
		else
		{
			skillMastery = lvl331 > 0 ? player.calcStat(Stats.SKILL_MASTERY, Int, null, null) : lvl330 > 0 ? player.calcStat(Stats.SKILL_MASTERY, Str, null, null) : 0.;
		}
		...
		sb.replaceAll("%magicRes%", df.format(magicRes));
		sb.replaceAll("%pvpPhysDmg%", df.format(pvpPhysDmg));
		sb.replaceAll("%pvpPhysSkillDmg%", df.format(pvpPhysSkillDmg));
		sb.replaceAll("%pvpPhysSkillDef%", df.format(pvpPhysSkillDef));
		...
		sb.replaceAll("%pveBowDmg%", df.format(pveBowDmg));
		sb.replaceAll("%pveBowSkillDmg%", df.format(pveBowSkillDmg));
		sb.replaceAll("%pveBowDef%", df.format(pveBowDef));
		sb.replaceAll("%pveBowSkillDef%", df.format(pveBowSkillDef));
		...
		sb.replaceAll("%skillMastery%", df.format(skillMastery));
