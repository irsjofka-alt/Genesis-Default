...
import gameserver.model.entity.auction.AccessoryItemType;
import gameserver.model.entity.auction.ArmorItemType;
import gameserver.model.entity.auction.Auction;
import gameserver.model.entity.auction.AuctionItemTypes;
import gameserver.model.entity.auction.AuctionsManager;
import gameserver.model.entity.auction.EtcAuctionItemType;
import gameserver.model.entity.auction.PetItemType;
import gameserver.model.entity.auction.SuppliesItemType;
import gameserver.model.entity.auction.WeaponItemType;
import gameserver.model.items.ItemAuction;
...
import gameserver.utils.comparators.GradeComparator;
import gameserver.utils.comparators.ItemNameComparator;
import gameserver.utils.comparators.PriceComparator;
import gameserver.utils.comparators.QuantityComparator;
...
public class CommunityAuction extends AbstractCommunity implements ICommunityBoardHandler
{
					...
					if (action == 1)
					{
						...
						else
						{
							...
							else
							{
								...
								final var item = auction.getItem();
								final var msg = new ServerMessage("CommunityAuction.WANT_TO_BUY", player.getLang());
								msg.add(quantity);
		...
		else if (command.startsWith("_bbsNewAuction"))
		{
			...
			if (buttonClicked != null)
			{
				if (buttonClicked.equals("0"))
				{
					...
					{
						final var msg = new ServerMessage("CommunityAuction.WANT_TO_SELL", player.getLang());
						msg.add(item.getItem().getName(player.getLang()));
				...
				else if (buttonClicked.equals("1"))
				{
					...
					{
						player.sendMessage((new ServerMessage("CommunityAuction.ALREADY_SOLD", player.getLang())).toString());
						final var auctions = AuctionsManager.getInstance().getMyAuctions(player.getObjectId(), Integer.parseInt(priceItemId));
						for (final var a : auctions)
						{
							if (a.getItem() == null)
							{
					...
					else
					{
						...
						{
							final var msg = new ServerMessage("CommunityAuction.WANT_TO_CANCEL", player.getLang());
							msg.add(item.getItem().getName(player.getLang()));
	...
	private String fillAuctionListPage(Player player, String html, int page, int priceItemId, int[] itemTypes, String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort)
	{
		...
		for (int i = 1; i <= 6; i++)
		{
			if (itemTypes[0] == i)
			{
				final var types = getGroupsInType(itemTypes[0]);
		...
		final var builder = new StringBuilder();
		final var allAuctions = AuctionsManager.getInstance().getAllAuctionsPerItemId(priceItemId);
		var auctions = getRightAuctions(allAuctions, itemTypes, itemGrade, search);
	...
	{
		final var auction = AuctionsManager.getInstance().getAuction(auctionId);
		if (auction == null || auction.getItem() == null)
		{
			return "";
		}
		final var choosenItem = auction.getItem();
		...
		if (choosenItem.isEquipable())
		{
			...
			{
				for (final var elm : choosenItem.getElementals())
				{
					if (elm.getValue() > 0)
					{
	...
	{
		final var itemsToAuction = getItemsToAuction(player, priceItemId);
		...
		{
			final var item = i >= 0 && itemsToAuction.size() > i ? itemsToAuction.get(i) : null;
			if (index % 6 == 0)
			{
				builder.append("<tr>");
				if (added)
		...
		final var myAuctions = AuctionsManager.getInstance().getMyAuctions(player, priceItemId);
		var auctions = myAuctions.toArray(new Auction[myAuctions.size()]);
		...
		{
			for (final var ti : player.getSellList().getItems())
			{
				for (final var auction : auctions)
				{
		...
		final var choosenItem = (currentItem > 0 ? player.getInventory().getItemByObjectId(currentItem) : null);
	...
	private List<Auction> getRightAuctions(Collection<Auction> allAuctions, int[] splitedTypes, String itemGrade, String search)
	{
		...
		for (final var auction : allAuctions)
		{
			if (splitedTypes != null && splitedTypes[0] >= 0)
			{
				boolean found = false;
				final var realItemType = auction.getItemType();
				var lookedTypes = getGroupsInType(splitedTypes[0]);
				
				if (splitedTypes[1] >= 0)
				{
					final var lookedType = lookedTypes[splitedTypes[1]];
					lookedTypes = new AuctionItemTypes[1];
					lookedTypes[0] = lookedType;
				}
				for (final var itemType : lookedTypes)
				{
	...
	private List<ItemInstance> getItemsToAuction(Player player, int priceItemId)
	{
		...
		for (final var item : player.getInventory().getAvailableSellItems(false, false))
		{
			if (item == null || item.getItemLocation() == ItemLocation.AUCTION)
			{
				continue;
			}
			...
			if (item.isStackable())
			{
				for (final var playerAuction : AuctionsManager.getInstance().getMyAuctions(player, priceItemId))
				{
	...
	{
		for (final var func : item.getItem().getAttachedFuncs())
		{
			if (func.stat == stat)
			{
	...
	protected int calc(ItemInstance item, int baseStat, Stats stat)
	{
		...
		if (item.isWeapon())
		{
			final var type = (WeaponType) item.getItemType();
			switch (item.getItem().getItemGradeSPlus())
			{
				case Item.CRYSTAL_S :
					switch (type)
