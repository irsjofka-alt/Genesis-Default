...
import gameserver.utils.comparators.ItemChanceComparator;
import gameserver.utils.comparators.ItemComparator;
import gameserver.utils.comparators.MonsterComparator;
...
public class CommunityNpcCalc extends AbstractCommunity implements ICommunityBoardHandler
{
	...
	private static void manageButton(Player player, int buttonId, int monsterId)
	{
		...
		{
			case 1 :
				final var locs = SpawnParser.getInstance().getRandomSpawnsByNpc(monsterId);
				if (locs == null || locs.isEmpty())
				{
					return;
				}
				
				if (Config.BLOCKED_RAID_LIST.contains(monsterId))
				{
					return;
				}
