...
import gameserver.model.holders.ClassBalanceHolder;
...
public class CommunityBalancer extends AbstractCommunity implements ICommunityBoardHandler
{
			...
			for (int cId = 0; cId < array.length; cId++)
			{
				...
				if (classId.getRace() != null)
				{
					if (isOly)
					{
						...
						{
							classes = classes + WordUtils.capitalizeFully(classId.name()) + ";";
						}
					}
			...
			for (int cId = 0; cId < array2.length; cId++)
			{
				...
				if (classId.getRace() != null)
				{
					if (isOly)
					{
						...
						{
							tClasses = tClasses + WordUtils.capitalizeFully(classId.name()) + ";";
						}
					}
	...
	private static String getBalanceInfo(Player activeChar, Collection<ClassBalanceHolder> collection, int pageId, boolean search, boolean isOly)
	{
		...
		for (final ClassBalanceHolder balance : collection)
		{
			...
			if ((!ClassId.getClassId(classId).name().equals("") && !ClassId.getClassId(targetClassId).name().equals("")) || !ClassId.getClassId(classId).name().equals("") || targetClassId == -1)
			{
				...
				for (final Map.Entry<AttackType, Double> dt : localCollection)
				{
					if ((count > limitInPage * (pageId - 1)) && (count <= limitInPage * pageId))
					{
						...
						content = content.replace("<?className?>", classId <= -2 ? "(All)" : classId == -1 ? "Monster" : WordUtils.capitalizeFully(ClassId.getClassId(classId).name().toLowerCase().replace("_", " ")));
						...
						content = content.replace("<?targetClassName?>", targetClassId <= -2 ? "(All)" : targetClassId == -1 ? "Monster" : WordUtils.capitalizeFully(ClassId.getClassId(targetClassId).name().toLowerCase().replace("_", " ")));
