...
import gameserver.handler.communityhandlers.impl.model.ForgeElement;
		...
		else
		{
			...
			else
			{
					...
					if (player.getInventory().destroyItemByObjectId(_item.getObjectId(), _item.getCount(), player, true) != null)
					{
						...
						if (_item.getElementals() != null)
						{
							for (final Elementals elm : _item.getElementals())
							{
								...
								{
									_found.setElementAttr(elm.getElement(), elm.getValue(), false);
								}
							}
							_found.updateItemElementals();
						}
						player.getInventory().equipItem(_found, true, true);
				...
				else if (command.startsWith("_bbsforge:attribute:"))
				{
					...
					player.destroyItemByItemId("ForgeBBS", Config.BBS_FORGE_ENCHANT_ITEM, price, player, true);
					player.getInventory().unEquipItem(_item, true);
					_item.setElementAttr(Elementals.getElementById(att), Value, true);
					player.getInventory().equipItem(_item, true, true);
