...
import gameserver.handler.communityhandlers.impl.model.NpcUtils;
...
import gameserver.utils.comparators.ValueComparator;
...
public class CommunityRaidBoss extends AbstractCommunity implements ICommunityBoardHandler
{
	...
	{
		final var bossTemplate = NpcsParser.getInstance().getTemplate(bossId);
		...
		{
			case 1 :
				if (bossTemplate.isEpicRaid())
				{
					EpicBossManager.getInstance().showBossLocation(player, bossTemplate);
				}
				else
				{
					RaidBossSpawnManager.getInstance().showBossLocation(player, bossTemplate);
				}
				break;
			case 2 :
			...
			case 3 :
				...
				break;
			case 4 :
				if (Config.BLOCKED_RAID_LIST.contains(bossId))
				{
					return;
				}
			...
			case 6 :
				...
				if (player.isInKrateisCube() || player.getUCState() > 0 || player.isCursedWeaponEquipped() || player.isInFightEvent() || player.isInCombat() || player.isInDuel() || !player.getReflection().isDefault() || player.isInStoreMode() || player.isInOfflineMode() || player.isJailed() || player.inObserverMode() || player.isInOlympiadMode() || player.isBlocked() || OlympiadManager.getInstance().isRegistered(player))
				{
					player.sendMessage((new ServerMessage("Community.ALL_DISABLE", player.getLang())).toString());
					return;
				}
				...
				if (bossTemplate.isEpicRaid())
				{
					final var b = EpicBossManager.getInstance().getBoss(bossId);
					if (b != null && !b.isDead() && b.isVisible())
					{
						player.enterObserverMode(b.getLocation());
					}
					else
					{
						player.sendMessage(bossTemplate.getName(player.getLang()) + " " + ServerStorage.getInstance().getString(player.getLang(), "CommunityRaidBoss.NOT_SPAWNED"));
					}
				}
				else
				{
					final var s = RaidBossSpawnManager.getInstance().getSpawns().get(bossId);
					if (s != null)
					{
						final var npc = s.getLastSpawn();
						if (npc != null)
						{
							player.enterObserverMode(npc.getLocation());
						}
						else
						{
							player.sendMessage(bossTemplate.getName(player.getLang()) + " " + ServerStorage.getInstance().getString(player.getLang(), "CommunityRaidBoss.NOT_SPAWNED"));
	...
	private static String getNormalReplacements(Player player, String html, int page, SortType sort, CharSequence search, int bossId)
	{
		...
		newHtml = newHtml.replace("<?sort?>", String.valueOf(sort._index));
		...
		for (int i = 1; i <= 6; i++)
		{
			if (Math.abs(sort._index) == i)
			{
				newHtml = newHtml.replace("<?sort" + i + "?>", String.valueOf(-sort._index));
			}
			else
			{
	...
	public static boolean isBossAlive(StatsSet set)
	{
	...
	{
		for (final var type : SortType.values())
		{
			if (type._index == i)
			{
				return type;
