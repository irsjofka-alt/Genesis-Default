...
import gameserver.data.dao.CharacterCBTeleportDAO;
...
import gameserver.model.actor.templates.community.CBTeleportTemplate;
...
public class CommunityTeleport extends AbstractCommunity implements ICommunityBoardHandler
{
	...
	public void onBypassCommand(String command, Player player)
	{
		...
		else if (command.startsWith("_bbsteleport;save;"))
		{
			...
			if (player.getCBTeleports().size() >= limit)
			{
				final ServerMessage msg = new ServerMessage("CommunityTeleport.MSG_6", player.getLang());
				msg.add(limit);
				player.sendMessage(msg.toString());
				showInfo(player, "index");
				if (player.getVarInt("cbTeleport", 0) < Config.SERVICES_EXPAND_TELE_TABS_LIMIT)
				{
					Util.setHtml(HtmCache.getInstance().getHtm(player, player.getLang(), "data/html/community/donate/teleport-tabs.htm"), player);
				}
				return;
			}
			...
			if (name != null)
			{
				addNewPosition(player, name, limit);
			}
			else
			{
		...
		else if (command.startsWith("_bbsteleport;tpl;"))
		{
			...
			if (id != null)
			{
				...
				if (tpl != null)
				{
					doTeleport(player, tpl.getX(), tpl.getY(), tpl.getZ(), Config.COMMUNITY_TAB_USE_ITEM[0], Config.COMMUNITY_TAB_USE_ITEM[1], true);
				}
			}
		}
		else if (command.startsWith("_bbsteleport;teleport;"))
		{
			...
			{
				doTeleport(player, Integer.parseInt(x), Integer.parseInt(y), Integer.parseInt(z), 57, Long.parseLong(price), false);
			}
		}
		else if (command.startsWith("_bbsteleport;id;"))
		{
			...
			if (id != null)
			{
				...
				if (template != null)
				{
					if (player.isFishing() || player.isCombatFlagEquipped() || player.isDead() || player.isAlikeDead() || player.isCastingNow() || player.isAttackingNow() || player.isInOlympiadMode() || player.isJailed() || player.isFlying() || (player.getKarma() > 0 && !template.canPk()) || player.isInDuel() || player.getLevel() < template.getMinLvl() || player.getLevel() > template.getMaxLvl())
					{
						player.sendMessage((new ServerMessage("CommunityTeleport.MSG_1", player.getLang())).toString());
						return;
					}
	...
	private void doTeleport(Player player, int x, int y, int z, int itemId, long count, boolean isTab)
	{
		if (player.isFishing() || player.isFestivalParticipant() || player.isInKrateisCube() || player.getUCState() > 0 || player.isCombatFlagEquipped() || player.isDead() || player.isAlikeDead() || player.isCastingNow() || player.isAttackingNow() || player.isInOlympiadMode() || player.isJailed() || player.isFlying() || player.isInDuel())
		{
			player.sendMessage((new ServerMessage("CommunityTeleport.MSG_1", player.getLang())).toString());
			return;
		}
		
		if (player.isInFightEvent() || ((AerialCleftEvent.getInstance().isStarted() || AerialCleftEvent.getInstance().isRewarding()) && AerialCleftEvent.getInstance().isPlayerParticipant(player.getObjectId())) || player.getReflectionId() != 0)
		{
			player.sendMessage((new ServerMessage("CommunityTeleport.MSG_1", player.getLang())).toString());
			return;
		}
		...
		if (count > 0 && ((player.getLevel() > Config.COMMUNITY_FREE_TP_LVL && !isTab) || isTab))
		{
			if (player.getInventory().getItemByItemId(itemId) == null || player.getInventory().getItemByItemId(itemId).getCount() < count)
			{
				final ServerMessage msg = new ServerMessage("CommunityTeleport.MSG_10", player.getLang());
				msg.add(Util.getItemName(player, itemId));
				msg.add(count);
				player.sendMessage(msg.toString());
				return;
			}
			player.destroyItemByItemId("Teleport", itemId, count, player, true);
		}
		player.sendPacket(new ShowBoard());
	...
	private void addNewPosition(Player player, String name, int limit)
	{
		...
		if (Config.COMMUNITY_TELEPORT_ITEM[0] > 0)
		{
			if (player.getInventory().getItemByItemId(Config.COMMUNITY_TELEPORT_ITEM[0]) == null || player.getInventory().getItemByItemId(Config.COMMUNITY_TELEPORT_ITEM[0]).getCount() < Config.COMMUNITY_TELEPORT_ITEM[1])
			{
				final ServerMessage msg = new ServerMessage("CommunityTeleport.MSG_8", player.getLang());
				msg.add(Util.getItemName(player, Config.COMMUNITY_TELEPORT_ITEM[0]));
				msg.add(Config.COMMUNITY_TELEPORT_ITEM[1]);
				player.sendMessage(msg.toString());
				return;
			}
			player.destroyItemByItemId("AddTeleport", Config.COMMUNITY_TELEPORT_ITEM[0], Config.COMMUNITY_TELEPORT_ITEM[1], player, true);
		}
		CharacterCBTeleportDAO.getInstance().add(player, name, limit);
	}
