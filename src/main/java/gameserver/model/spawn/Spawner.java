 ...
 */
package gameserver.model.spawn;
...
import gameserver.taskmanager.SpawnTaskManager;

public class Spawner implements Cloneable, IPositionable, IIdentifiable
{
	protected static final Logger _log = LogManager.getLogger(Spawner.class);
	...
	public Spawner(NpcTemplate template)
	{
		_template = template;
	}
	...
	{
		final var s = new Spawner(_template);
		s.setAmount(_maximumCount);
		s.setTerritoryName(_territoryName);
		s.setSpawnTemplate(_spawnTemplate);
		s.setLocation(_location);
		s.setMinionList(_minionList);
		s.setRespawnPattern(_respawnPattern);
		s.setRespawnTime(_respawnTime);
		s.setRespawnMinDelay(_respawnMinDelay);
		s.setRespawnMaxDelay(_respawnMaxDelay);
		return s;
	}
	...
	public void decreaseCount(Npc oldNpc)
	{
		...
		if (_doRespawn && ((_scheduledCount + _currentCount) < _maximumCount))
		{
			...
			else
			{
				...
				{
					final String[] time = new SimpleDateFormat("HH:mm").format(new Date(System.currentTimeMillis())).split(":");
					final var curHour = Integer.parseInt(time[0]);
					for (final var list : _respawnTime.split(";"))
					{
						final String[] info = list.split(":");
						if (info.length == 2)
						{
							final var hours = info[0].split("-");
							final var respawns = info[1].split("-");
							if (hours.length == 2 && respawns.length == 2)
							{
								if (curHour >= Integer.parseInt(hours[0]) && curHour <= Integer.parseInt(hours[1]))
								{
									final int resp = Integer.parseInt(respawns[0]);
									final int respRnd = Integer.parseInt(respawns[1]);
									respawnTime = respRnd > 0 ? Rnd.get(respRnd, resp) * 1000 : resp * 1000;
									break;
								}
							}
						}
	...
	public Npc doSpawn(boolean isSummonSpawn, int triggerId)
	{
		...
		try
		{
			...
			{
				_currentCount++;
				if (_template.isType("Trap"))
				{
					if ((_template.getId() >= 18720) && (_template.getId() <= 18774))
					{
						final var trap = new TrapInstance(IdFactory.getInstance().getNextId(), _template, _reflection, -1);
						trap.setCurrentHp(trap.getMaxHp());
						trap.setCurrentMp(trap.getMaxMp());
						trap.setIsInvul(true);
						trap.setHeading(getHeading());
						trap.spawnMe(getX(), getY(), getZ());
						return trap;
					}
				}
				return mob;
			}
			mob = _template.getNewInstance();
			mob.setReflection(getReflection());
			if (isSummonSpawn)
			{
				mob.setShowSummonAnimation(isSummonSpawn);
			}
	...
	{
		stopRespawn();
		_spawnedNpcs.stream().filter(n -> n != null).forEach(n -> n.deleteMe());
		_spawnedNpcs.clear();
		_scheduledCount = 0;
		_currentCount = 0;
	}
