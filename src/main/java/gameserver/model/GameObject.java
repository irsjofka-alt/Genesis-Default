...
import gameserver.model.entity.events.model.FightEventOwner;
import gameserver.model.interfaces.IIdentifiable;
import gameserver.model.interfaces.ILocational;
import gameserver.model.interfaces.IPositionable;
...
{
	public static final Logger _log = LogManager.getLogger(GameObject.class);
	...
	public void setXYZInvisible(ILocational loc)
	{
		_x = Util.limit(loc.getX(), World.MAP_MIN_X, World.MAP_MAX_X);
		_y = Util.limit(loc.getY(), World.MAP_MIN_Y, World.MAP_MAX_Y);
		_z = loc.getZ();
		setIsVisible(false);
	}
	...
	public synchronized void setReflection(Reflection ref)
	{
		...
		{
			r.removeObject(this);
			if (player != null && _reflection.isShowTimer())
			{
				...
				{
					sendPacket(new ExSendUIEvent(player, true, true, startTime, endTime, _reflection.getTimerText()));
				}
				else
				{
					sendPacket(new ExSendUIEvent(player, true, false, endTime - startTime, 0, _reflection.getTimerText()));
				}
			}
		}
		...
		{
			ref.addObject(this);
			if (player != null && ref.isShowTimer())
			{
				...
				{
					sendPacket(new ExSendUIEvent(player, false, true, startTime, endTime, ref.getTimerText()));
				}
				else
				{
					sendPacket(new ExSendUIEvent(player, false, false, endTime - startTime, 0, ref.getTimerText()));
				}
			}
		}
		else
	...
	public void setInvisible(boolean invis)
	{
		...
		if (invis)
		{
			...
			try
			{
				...
				{
					if (npc != null && npc.isAttackable() && !npc.isDead())
					{
	...
	public void broadcastInfo()
	{
		...
		{
			players = World.getAroundPlayers(this);
			players.stream().filter(p -> p != null && isVisibleFor(p)).forEach(p -> sendInfo(p));
		}
		finally
		{
			players.clear();
	...
	public boolean isInFrontDoor(List<DoorInstance> allDoors, Location loc, Location end, Reflection r)
	{
	...
	{
		return isInFrontDoor(DoorParser.getInstance().getRegionDoors(getLocation(), r), getX(), getY(), getZ(), tx, ty, tz, r);
	}
	
	public boolean isInFrontDoor(Location loc, Location end, Reflection r)
	{
		return isInFrontDoor(DoorParser.getInstance().getRegionDoors(loc, r), loc.getX(), loc.getY(), loc.getZ(), end.getX(), end.getY(), end.getZ(), r);
	}
	
	public boolean isInFrontDoor(List<DoorInstance> allDoors, int x, int y, int z, int tx, int ty, int tz, Reflection r)
	{
	...
	public int getDialogRadius(Player player, boolean checkRadius)
	{
		return 150;
	}
	
	public boolean isInNeighboringRegion(GameObject worldObject)
	{
		if (worldObject == null)
		{
			return false;
		...
		final var worldRegion = worldObject.getWorldRegion();
		if (worldRegion == null || _worldRegion == null)
		{
			return false;
		}
		return World.isNeighbour(worldRegion.getX(), worldRegion.getY(), worldRegion.getZ(), _worldRegion.getX(), _worldRegion.getY(), _worldRegion.getZ());
	}
