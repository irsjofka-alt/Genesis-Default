 ...
 */
package gameserver.model.olympiad;
...
import gameserver.model.actor.templates.player.OlympiadTemplate;
...
public class Olympiad
{
	...
	private void load()
	{
		...
		if (!loaded)
		{
			...
			_currentCycle = OlympiadProperties.getProperty("CurrentCycle", 1, false);
			_period = OlympiadProperties.getProperty("Period", 0, false);
			_olympiadEnd = OlympiadProperties.getProperty("OlympiadEnd", 0, false);
			_validationEnd = OlympiadProperties.getProperty("ValidationEnd", 0, false);
			_nextWeeklyChange = OlympiadProperties.getProperty("NextWeeklyChange", 0, false);
		}
	...
	protected void init()
	{
		...
		if ((_compStart < now) && ((_compEnd - 60000L) < now))
		{
	...
	private class CompStartTask implements Runnable
	{
		...
		public void run()
		{
			...
			_scheduledCompEnd = ThreadPoolManager.getInstance().schedule(new CompEndTask(1), (getMillisToCompEnd() - 60000));
		}
	}
