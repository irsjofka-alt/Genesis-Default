...
public class ToggleList
{
	...
	private final List<Effect> _effectList = new CopyOnWriteArrayList<>();
	...
	public boolean calcToggleDam()
	{
		...
		if (!effectList.isEmpty())
		{
			...
			for (final Effect effect : effectList)
			{
				if (effect != null)
				{
					_actor.stopSkillEffects(effect.getSkill().getId(), false);
				}
			}
			_actor.getEffectList().updateEffectIcons();
			effectList.clear();
		}
		return true;
	}
