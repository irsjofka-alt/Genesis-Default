...
import gameserver.model.actor.templates.ExtractableProductItemTemplate;
import gameserver.model.actor.templates.ExtractableSkillTemplate;
...
{
	protected static final Logger _log = LogManager.getLogger(Skill.class.getName());
	...
	private final boolean _allowWithMoveDisabled;
	...
	private final boolean _isBlockReoverlay;
	private final boolean _allowDispel;
	private final boolean _isBattlefieldPenalty;
	private final boolean _applyOnSummon;
	...
	protected Skill(StatsSet set)
	{
		...
		_allowWithMoveDisabled = set.getBool("allowWithMoveDisabled", false);
		...
		_blockSave = set.getBool("isBlockSave", false);
		...
		_isBlockReoverlay = set.getBool("isBlockReoverlay", false);
		_allowDispel = set.getBool("allowDispel", false);
		_isBattlefieldPenalty = set.getBool("isBattlefieldPenalty", false);
		_applyOnSummon = set.getBool("applyOnSummon", true);
		_hashCode = _id * 1023 + _level;
	}
	
	public abstract void useSkill(Creature caster, GameObject[] targets, double cubicPower);
	...
	public final boolean hasBuffEffects()
	{
		return !_isDebuff || hasEffectType(EffectType.BUFF) || _hpConsume > 0 || getSkillType() == SkillType.BUFF;
	}
	...
	{
		final Creature target = activeChar.getTarget() != null ? (Creature) activeChar.getTarget() : null;
	...
	public final List<Effect> getEffects(Creature effector, Creature effected, Env env, boolean allowSkillMastery, boolean update)
	{
		...
		boolean found = false;
		for (final var et : _effectTemplates)
		{
			if ((et.isSimilar(Config.EFFECTS_REOVERLAY) || _isBlockReoverlay) && effected.getEffectList().getFirstEffect(this) != null)
			{
				continue;
			}
			
			if (Formulas.calcSkillSuccess(effector, effected, this, et, et.getEffectPower(), env.getShield(), env.isSoulShot(), env.isSpiritShot(), env.isBlessedSpiritShot()))
			{
				final var e = et.getEffect(env);
				if (e != null)
				{
					...
					if (allowReoverlay && !e.getSkill().hasEffectType(EffectType.CANCEL) && !e.isNoNameType())
					{
						if (e.getSkill().getSkillType() != SkillType.BUFF && !e.isSimilar("buff"))
						{
							effected.getEffectList().stopSkillEffect(e);
						}
					}
					e.scheduleEffect(true, false);
					effects.add(e);
					found = true;
				}
			}
	...
	public final List<Effect> getEffects(Creature effector, Creature effected, boolean allowSkillMastery, boolean update)
	{
		return getEffects(effector, effected, null, allowSkillMastery, update);
	}
	
	public final List<Effect> getEffects(CubicInstance effector, Creature effected, Env env, boolean update)
	{
		if (!hasEffects() || isPassive())
		{
			return Collections.<Effect> emptyList();
		...
		for (final var et : _effectTemplates)
		{
			if (Formulas.calcSkillSuccess(effector.getOwner(), effected, this, et, et.getEffectPower(), env.getShield(), env.isSoulShot(), env.isSpiritShot(), env.isBlessedSpiritShot()))
			{
				final var e = et.getEffect(env);
				if (e != null)
				{
	...
	public final List<Effect> getEffectsSelf(Creature effector, boolean update)
	{
		...
		for (final var et : _effectTemplatesSelf)
		{
			final Env env = new Env();
			env.setCharacter(effector);
	...
	public final List<Effect> getEffectsPassive(Creature effector, boolean update)
	{
		...
		for (final var et : _effectTemplatesPassive)
		{
			final Env env = new Env();
			env.setCharacter(effector);
			env.setTarget(effector);
	...
	public boolean isAllowWithMoveDisabled()
	{
		return _allowWithMoveDisabled;
	}
	...
	public boolean isAllowDispel()
	{
		return _allowDispel;
	}
	
	public boolean checkNormalRange(Creature effector, Creature origin, Creature target, boolean srcInArena, Creature geoTarget)
	{
		if (Util.checkIfInRange(getAffectRange(), origin, target, true))
		{
			if (!checkForAreaOffensiveSkills(effector, target, this, srcInArena, geoTarget))
			{
				return false;
			}
	...
	public boolean checkFanRange(Creature effector, Creature origin, Creature target, boolean srcInArena, Creature geoTarget)
	{
		...
		final double fanHalfAngle = fanAngle / 2;
		
		if (Math.abs(PositionUtils.calculateAngleFrom(effector, target) - (headingAngle + fanStartAngle)) > fanHalfAngle)
		{
			return false;
		}
	...
	public boolean isBattlefieldPenalty()
	{
		return _isBattlefieldPenalty;
	}
	
	public boolean isApplyOnSummon()
	{
		return _applyOnSummon;
	}
