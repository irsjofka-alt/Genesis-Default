...
import gameserver.listener.ManagedLoader;
...
import gameserver.model.actor.templates.quest.QuestExperience;
import gameserver.model.actor.templates.quest.QuestRewardItem;
import gameserver.model.actor.templates.quest.QuestTemplate;
...
import gameserver.network.serverpackets.ExNpcQuestHtmlMessage;
...
{
	public static final Logger _log = LogManager.getLogger(Quest.class);
	
	private volatile Map<String, List<QuestTimer>> _questTimers = new ConcurrentHashMap<>(1);
	...
	private boolean _isUseSwitch = Config.ALLOW_CUSTOM_BUTTONS;
	...
	{
		final QuestState qs = player.getQuestState(getClass().getSimpleName());
	...
	{
		final var template = QuestsParser.getInstance().getTemplate(_questId);
		if (template != null)
		{
	...
	{
		final var path = getClass().getName().replace('.', '/').replace("/" + getClass().getSimpleName(), "").split("/");
	...
	public String showHtmlFile(Player player, String fileName)
	{
		...
		final String directory = getPath();
	...
	{
		String filepath = "data/html/scripts/" + getPath() + "/" + getName() + "/" + fileName;
	...
	{
		if (_questTimers.isEmpty())
		{
			return;
		}
		
		for (final var timers : _questTimers.values())
		{
			for (final QuestTimer timer : timers)
			{
				timer.cancel();
	...
	public boolean unload(boolean removeFromList)
	{
		...
		for (final List<QuestTimer> timers : _questTimers.values())
		{
			for (final QuestTimer timer : timers)
			{
				timer.cancel();
		...
		for (final var npcId : _questInvolvedNpcs)
		{
			final var template = NpcsParser.getInstance().getTemplate(npcId.intValue());
			if (template != null)
			{
	...
	public static void giveItems(Player player, int itemId, long count, byte attributeId, int attributeLevel)
	{
		...
		{
			item.setElementAttr(attributeId, attributeLevel, true);
			if (item.isEquipped())
			{
	...
	public static boolean takeItems(Player player, int itemId, long amount)
	{
		...
		if (item.isEquipped())
		{
			final var unequiped = player.getInventory().unEquipItemInBodySlotAndRecord(item.getItem().getBodyPart(), true);
			final InventoryUpdate iu = new InventoryUpdate();
	...
	public static void calcExpAndSp(Player player, int questId)
	{
		...
		if (template != null)
		{
			...
			if (reward != null)
			{
				...
				exp *= (reward.isExpRateable() ? Config.RATE_QUEST_REWARD_XP : reward.getExpRate()) * player.getPremiumBonus().getQuestExpRewardRate();
				sp *= (reward.isSpRateable() ? Config.RATE_QUEST_REWARD_SP : reward.getSpRate()) * player.getPremiumBonus().getQuestSpRewardRate();
