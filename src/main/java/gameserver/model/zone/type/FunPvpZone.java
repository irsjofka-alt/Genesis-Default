...
public class FunPvpZone extends ZoneType
{
			...
			for (final ItemInstance o : activeChar.getInventory().getItems())
			{
				if (o != null)
				{
					if (o.isEquipable() && o.isEquipped() && !checkItem(o))
					{
						...
						activeChar.getInventory().unEquipItemInBodySlot(slot, false);
						found = true;
	...
	public void onReviveInside(Creature character)
	{
		...
		if (character.isPlayer())
		{
			...
			if (_reviveNoblesse)
			{
				SkillsParser.getInstance().getInfo(1323, 1).getEffects(character.getActingPlayer(), character.getActingPlayer(), false, true);
			}
			if (_reviveHeal)
			{
	...
	{
		if (killer != null && killer.isInFightEvent() || killer.getReflectionId() > 0 || !isEnabled())
		{
			return;
		}
	...
	private void buffPlayer(Player player)
	{
		if (_giveNoblesse)
		{
			SkillsParser.getInstance().getInfo(1323, 1).getEffects(player, player, false, true);
		}
