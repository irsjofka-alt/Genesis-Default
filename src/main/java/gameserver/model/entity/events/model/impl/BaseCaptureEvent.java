...
public class BaseCaptureEvent extends AbstractFightEvent
{
	...
	private final boolean _calcWinnerByCast;
	private Skill _captureSkill = null;
	...
	public BaseCaptureEvent(MultiValueSet<String> set)
	{
		...
		_calcWinnerByCast = set.getBool("calcWinnerByCast", false);
		if (_calcWinnerByCast)
		{
			_captureSkill = SkillsParser.getInstance().getInfo(set.getInteger("captureSkill", 0), 1);
		}
	}
	...
	public void startEvent(boolean checkReflection)
	{
		try
		{
			...
			for (final FightEventTeam team : getTeams())
			{
				...
				if (_calcWinnerByCast && _captureSkill != null)
				{
					team.getPlayers().stream().filter(f -> f != null && f.getPlayer() != null).forEach(f ->
					{
						f.getPlayer().addSkill(_captureSkill, false);
						f.getPlayer().sendSkillList(false);
					});
				}
				i++;
			}
			...
			if (_calcWinnerByCast && _captureSkill != null)
			{
				for (final var team : getTeams())
				{
					if (team != null)
					{
						team.getPlayers().stream().filter(f -> f != null && f.getPlayer() != null).forEach(f ->
						{
							f.getPlayer().removeSkill(_captureSkill, false, false);
							f.getPlayer().sendSkillList(false);
						});
					}
				}
	...
	public void loggedOut(Player player)
	{
		if (_calcWinnerByCast && _captureSkill != null)
		{
			player.removeSkill(_captureSkill, false, false);
		}
	...
	public boolean kickFromEvent(Player player, boolean isDestroy)
	{
		...
		return super.kickFromEvent(player, isDestroy);
	}
	...
	private void spawnBase(CaptureBaseTeam baseTeam)
	{
		try
		{
			...
			if (_calcWinnerByCast)
			{
				base.setIsInvul(true);
				base.setIsImmobilized(true);
				base.setSkill(_captureSkill);
			}
			baseTeam._base = base;
		}
		catch (final Exception e)
