...
import gameserver.utils.comparators.RankingComparator;
...
public class FightLastStatsManager extends LoggerObject
{
	...
	{
		var myStat = getMyStat(eventId, player);
		if (myStat == null)
		{
	...
	public FightEventLastPlayerStats getMyStat(int id, Player player)
	{
		for (final int eventId : _allStats.keySet())
		{
			if (eventId == id)
			{
				final var stats = _allStats.get(eventId);
				if (stats != null)
				{
	...
	public void updateEventStats(int id)
	{
		for (final int eventId : _allStats.keySet())
		{
			if (eventId == id)
			{
				for (final var stat : _allStats.get(eventId))
				{
	...
	{
		final List<FightEventLastPlayerStats> listToSort = new CopyOnWriteArrayList<>();
		listToSort.addAll(_allStats.get(eventId));
		if (sortByScore)
		{
			Collections.sort(listToSort, RankingComparator.getInstance());
		}
		return listToSort;
	}
	...
	public void restoreStats()
	{
		for (final var event : FightEventParser.getInstance().getEvents().valueCollection())
		{
			if (event != null)
			{
				...
				{
					final List<FightEventLastPlayerStats> list = new CopyOnWriteArrayList<>();
