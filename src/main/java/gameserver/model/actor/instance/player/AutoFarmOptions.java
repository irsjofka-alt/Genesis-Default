...
import gameserver.model.actor.instance.player.impl.AutoFarmTask;
import gameserver.model.actor.instance.player.impl.CleanUpTask;
import gameserver.model.actor.tasks.player.AutoArcherFarmTask;
import gameserver.model.actor.tasks.player.AutoHealFarmTask;
import gameserver.model.actor.tasks.player.AutoMagicFarmTask;
import gameserver.model.actor.tasks.player.AutoPhysicalFarmTask;
import gameserver.model.actor.tasks.player.AutoSummonFarmTask;
...
import gameserver.network.serverpackets.ExServerPrimitive;
import gameserver.utils.Log;
import gameserver.utils.comparators.DistanceComparator;
import gameserver.utils.comparators.HealComparator;
...
public class AutoFarmOptions extends LoggerObject
{
	...
	private long _emptyTotalTime = 0;
	...
	private Attackable _lockTarget;
	...
	public void setRadiusValue(int value)
	{
		...
		if (isAutofarming() && _keepLocation != null)
		{
			showRangeZone(true);
		}
	}
	...
	{
		if (isAutofarming() || _player.isDead() || !FarmSettings.ALLOW_AUTO_FARM || (FarmSettings.AUTO_FARM_FOR_PREMIUM && !_player.hasPremiumBonus()))
		{
			return;
		}
		...
		_player.getListeners().onAutoFarmStart();
		_player.sendMessage("Autofarming activated");
		Log.addLogFarm("AUTOFARM:", "Activated by", _player);
	}
	
	public void stopFarmTask(boolean isSwitch, Creature killer)
	{
		...
		{
			showRangeZone(false);
			_player.getListeners().onAutoFarmStop(killer);
	...
	public void setKeepLocation(boolean active, boolean store)
	{
		_set.set("keepLocation", active);
		if (!store)
		{
			_player.setVar("farmKeepLocation", active ? 1 : 0);
			setKeepLocation(active ? _player.getLocation() : null);
		}
	}
	...
	public final Attackable getAroundNpc()
	{
		List<Attackable> notSeeList = new ArrayList<>();
		int count = 0;
		final var targets = World.getAroundFarmNpc(_player, getFarmRadius(), getHeightRadius());
		for (final var npc : targets)
		{
			if (npc != null && !npc.isDead())
			{
				if (GeoEngine.getInstance().canSeeTarget(_player, npc))
				{
					if (_lockTarget != null && _lockTarget == npc)
					{
						continue;
					}
				...
				{
					if (count < 3 && !_keepLocationMowing && !_searchPathFind && !_notSeeList.contains(npc.getObjectId()))
					{
						if (_lockTarget != null && _lockTarget == npc)
						{
							continue;
		...
		if (!notSeeList.isEmpty())
		{
			for (final var npc : notSeeList)
			{
				if (npc != null && !npc.isDead())
				{
					...
					var path = PathFinding.getInstance().findPath(_player, _player.getX(), _player.getY(), _player.getZ(), npc.getX(), npc.getY(), npc.getZ(), _player.getReflection(), true, false);
	...
	public Skill nextChanceSkill(Attackable target, long extraDelay)
	{
		...
		for (final var skillId : getChanceSpells())
		{
			...
			if ((skill.isSpoilSkill() && target.isSpoiled()) || (skill.isSweepSkill() && !target.isDead()))
			{
				continue;
			}
	...
	public Skill nextSelfSkill(Creature ownerTarget)
	{
		...
		for (final var skillId : getSelfSpells())
		{
			...
			if (!_player.checkDoCastConditions(skill, false) || !skill.checkCondition(_player, _player, false, false))
			{
				continue;
			}
	...
	public void setKeepLocation(Location loc)
	{
		...
		if (isAutofarming())
		{
			showRangeZone(_keepLocation != null);
		}
	}
	...
	public boolean isNeedToReturn()
	{
		...
		if (_keepLocationMowing && getKeepLocation() != null && !_player.isMoving())
		{
			...
			{
				var path = PathFinding.getInstance().findPath(_player, _player.getX(), _player.getY(), _player.getZ(), getKeepLocation().getX(), getKeepLocation().getY(), getKeepLocation().getZ(), _player.getReflection(), true, false);
				if (path != null && path.size() > 2)
				{
					_keepLocationMowing = true;
					_player.getAI().setIntention(CtrlIntention.MOVING, getKeepLocation(), 0);
	...
	public void checkTargetForRessurect(Creature killer)
	{
		if (!FarmSettings.ALLOW_RESURRECTION)
		{
			stopFarmTask(false, killer);
			return;
		}
	...
	public void checkEmptyTime()
	{
		if (_emptyTime == 0)
		{
			...
			_emptyTotalTime += FarmSettings.WAIT_TIME * 1000L;
		}
		...
		if (_player.isDead())
		{
			stopFarmTask(false, null);
			_emptyTime = 0L;
			if (_player.isInOfflineFarmMode())
			{
				_player.getPersonalTasks().addTask(new CleanUpTask(5000L));
				return;
			}
			else
			{
		...
		{
			_player.getListeners().onInaction(_emptyTotalTime);
		}
		_emptyTime = 0L;
	}
	...
	public boolean isFlyLocked(GameObject target)
	{
		if (_farmTask != null)
		{
			if (target != null)
			{
				final var path = PathFinding.getInstance().findPath(_player, _player.getX(), _player.getY(), _player.getZ(), target.getX(), target.getY(), target.getZ(), _player.getReflection(), true, false);
				if (path == null || path.size() < 2)
				{
					_abortTarget = true;
					return false;
				}
				_player.getAI().setIntention(CtrlIntention.MOVING, target.getLocation(), 0);
				_searchPathFind = true;
				return false;
			}
			return false;
	...
	public void setAbortTarget(boolean val, Attackable lockTarget)
	{
		_abortTarget = val;
		_lockTarget = lockTarget;
	}
	
	private void showRangeZone(boolean isEnable)
	{
		if (isEnable && _keepLocation != null)
		{
			final var exsp = new ExServerPrimitive("autofarm_range", _keepLocation.getX(), _keepLocation.getY(), _keepLocation.getZ() - 1000);
			final int radius = getFarmRadius();
			
			final int zMin = -100;
			final int zMax = 400;
			
			final Color color = Color.GREEN;
			final int x = _keepLocation.getX();
			final int y = _keepLocation.getY();
			final int z = _keepLocation.getZ() + zMin;
			final int count = (int) (2 * Math.PI * radius / 150);
			final double angle = 2 * Math.PI / count;
			int prevX = x + (int) (Math.cos(angle * 0) * radius);
			int prevY = y + (int) (Math.sin(angle * 0) * radius);
			
			for (int i = 1; i < count + 1; i++)
			{
				final int x2 = x + (int) (Math.cos(angle * i) * radius);
				final int y2 = y + (int) (Math.sin(angle * i) * radius);
				exsp.addLine(color, prevX, prevY, z, x2, y2, z);
				exsp.addLine(color, prevX, prevY, z, x2, y2, z + zMax);
				
				final int x3 = x + (int) (Math.cos(angle * (i + 1)) * radius);
				final int y3 = y + (int) (Math.sin(angle * (i + 1)) * radius);
				exsp.addLine(color, x2, y2, z + zMax, x3, y3, z + zMax);
				prevX = x2;
				prevY = y2;
			}
			_player.sendPacket(exsp);
		}
		else
		{
			final var exsp = new ExServerPrimitive("autofarm_range", _player.getX(), _player.getY(), _player.getZ() - 1000);
			exsp.addPoint(Color.GREEN, _player.getX(), _player.getY(), _player.getZ() - 1000);
			_player.sendPacket(exsp);
		}
	}
}