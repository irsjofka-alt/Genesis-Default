...
public class BypassStorage extends LoggerObject
{
	...
	public BypassType getBypassType(final String bypass)
    {
        switch(bypass.charAt(0))
        {
			...
			case '2' :
				return BypassType.ENCODED_TUTORIAL;
            default:
	...
	public String encodeTutorialBypasses(final String html)
	{
		if (html == null)
		{
			return null;
		}
		final var storage = _tutorialBypasses;
		storage.clear();
		...
		while (m.find())
		{
			...
			m.appendReplacement(sb, "\"bypass -h 2" + Integer.toHexString(encodeBypass(code, storage)) + params + "\"");
		}
		m.appendTail(sb);
		return sb.toString();
	}
	...
	public String decodeTutorialBypass(final String bypass)
	{
		final var storage = _tutorialBypasses;
