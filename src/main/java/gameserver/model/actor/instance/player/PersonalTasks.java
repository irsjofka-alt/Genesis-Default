...
import gameserver.model.actor.instance.player.impl.AbstractPlayerTask;
...
public class PersonalTasks extends LoggerObject
{
	...
	public AbstractPlayerTask getActiveTask(int... ids)
	{
		final var tasks = _playerTasks;
		if (tasks.isEmpty())
		{
	...
	public boolean removeTasks(boolean isRecalc, int... ids)
	{
		boolean found = false;
		for (final var id : ids)
		{
	...
	public void recalcTime()
	{
		...
		final var sorted = _playerTasks.entrySet().stream().sorted(Map.Entry.comparingByValue()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, _) -> e1, LinkedHashMap::new));
		if (!sorted.isEmpty())
		{
			...
			_timeTask = ThreadPoolManager.getInstance().schedule(() -> checkTasks(), nextTime);
		...
		try
		{
			...
			for (final var entry : _playerTasks.entrySet())
			{
				...
				if (task != null)
				{
					...
					{
						_playerTasks.replace(task, task.getInterval());
					}
				}
			}
		}
