...
public final class EventChestInstance extends EventMonsterInstance
{
	...
	{
		World.getAroundPlayers(this).stream().filter(p -> p != null && (_isTriggered || canSee(p))).forEach(p -> p.sendPacket(packets));
	}
	
	@Override
	public void broadcastPacket(int range, GameServerPacket... packets)
	{
		World.getAroundPlayers(this, range, 300).stream().filter(p -> p != null && (_isTriggered || canSee(p))).forEach(p -> p.sendPacket(packets));
	}
