...
public final class TeleporterInstance extends NpcInstance
{
	...
	private void doTeleport(Player player, int val)
	{
		...
		if (list != null)
		{
			...
			if (player.getLevel() < 41 || Config.ALT_GAME_FREE_TELEPORT)
			{
				price = 0;
			}
			...
			if (price <= 0 || (price > 0 && player.destroyItemByItemId("Teleport " + (list.isForNoble() ? " nobless" : ""), list.getId(), price, this, true)))
			{
				if (Config.DEBUG)
				{
