...
import gameserver.model.entity.Auction;
		...
		else if (condition == COND_REGULAR)
		{
			...
			if (actualCommand.equalsIgnoreCase("auction"))
			{
				...
				try
				{
					...
					try
					{
						...
						final var a = new Auction(player.getClan().getHideoutId(), player.getClan(), days * 86400000L, bid);
						...
						html.replace("%AGIT_AUCTION_DESC%", ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getTemplate().getDescr(player.getLang()));
			...
			else if (actualCommand.equalsIgnoreCase("bidding"))
			{
				...
				try
				{
					...
					final var a = AuctionManager.getInstance().getAuction(auctionId);
					
					final var html = new NpcHtmlMessage(1);
					html.setFile(player, player.getLang(), filename);
					if (a != null)
					{
						final var hall = ClanHallManager.getInstance().getAuctionableHallById(a.getId());
						if (hall == null)
						{
							return;
						}
						html.replace("%AGIT_NAME%", hall.getTemplate().getName(player.getLang()));
						html.replace("%OWNER_PLEDGE_NAME%", a.getSellerClanName());
						html.replace("%OWNER_PLEDGE_MASTER%", a.getSellerName());
						html.replace("%AGIT_SIZE%", String.valueOf(hall.getGrade() * 10));
						html.replace("%AGIT_LEASE%", String.valueOf(hall.getRentPrice()));
						html.replace("%AGIT_LOCATION%", hall.getTemplate().getLocation(player.getLang()));
						html.replace("%AGIT_AUCTION_END%", String.valueOf(format.format(a.getEndDate())));
						...
						html.replace("%AGIT_AUCTION_DESC%", hall.getTemplate().getDescr(player.getLang()));
			...
			{
				final var auctions = AuctionManager.getInstance().getAuctions();
				final var format = new SimpleDateFormat("yy/MM/dd");
				int limit = 15;
				int start;
				int i = 1;
				...
				for (final Auction a : auctions)
				{
					...
					final var template = ClanHallManager.getInstance().getClanHallById(a.getId()).getTemplate();
					if (template != null)
					{
						items.append("<tr>");
						items.append("<td fixwidth=75><font color=4169E1>");
						items.append(template.getLocation(player.getLang()));
						...
						items.append(template.getName(player.getLang()));
			...
			else if (actualCommand.equalsIgnoreCase("bidlist"))
			{
				...
				String biders = "";
				final var auc = AuctionManager.getInstance().getAuction(auctionId);
				final var bidders = auc.getBidders();
				for (final var b : bidders.values())
				{
					...
					biders += "<tr>" + "<td fixwidth=160><font color=4169E1>" + ClanHallManager.getInstance().getClanHallById(auc.getId()).getTemplate().getName(player.getLang()) + "</font></td><td fixwidth=100><font color=EEE8AA>" + clan.getName() + "</font></td><td fixwidth=80>" + b.getTimeBid().get(Calendar.YEAR) + "/" + (b.getTimeBid().get(Calendar.MONTH) + 1) + "/" + b.getTimeBid().get(Calendar.DATE) + "</td>" + "</tr>";
				}
			...
			else if (actualCommand.equalsIgnoreCase("selectedItems"))
			{
				if ((player.getClan() != null) && (player.getClan().getHideoutId() == 0) && (player.getClan().getAuctionBiddedAt() > 0))
				{
					...
					final var html = new NpcHtmlMessage(1);
					html.setFile(player, player.getLang(), filename);
					final var a = AuctionManager.getInstance().getAuction(player.getClan().getAuctionBiddedAt());
					if (a != null)
					{
						final var hall = ClanHallManager.getInstance().getAuctionableHallById(a.getId());
						if (hall == null)
				...
				else if ((player.getClan() != null) && (AuctionManager.getInstance().getAuction(player.getClan().getHideoutId()) != null))
				{
					...
					final var html = new NpcHtmlMessage(1);
					html.setFile(player, player.getLang(), filename);
					final var a = AuctionManager.getInstance().getAuction(player.getClan().getHideoutId());
					if (a != null)
					{
						final var hall = ClanHallManager.getInstance().getAuctionableHallById(a.getId());
						if (hall == null)
				...
				else if ((player.getClan() != null) && (player.getClan().getHideoutId() != 0))
				{
					...
					final var html = new NpcHtmlMessage(1);
					html.setFile(player, player.getLang(), filename);
					final var hall = ClanHallManager.getInstance().getAuctionableHallById(ItemId);
					if (hall != null)
					{
						html.replace("%AGIT_NAME%", hall.getTemplate().getName(player.getLang()));
			...
			{
				final var auc = AuctionManager.getInstance().getAuction(player.getClan().getAuctionBiddedAt());
				if (auc != null)
				{
					auc.cancelBid(player.getClanId());
			...
			else if (actualCommand.equalsIgnoreCase("cancelAuction"))
			{
				...
				final var html = new NpcHtmlMessage(1);
				html.setFile(player, player.getLang(), filename);
				html.replace("%AGIT_DEPOSIT%", String.valueOf(ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getRentPrice()));
				html.replace("%AGIT_LINK_BACK%", "bypass -h npc_" + getObjectId() + "_selectedItems");
				html.replace("%objectId%", String.valueOf(getObjectId()));
				player.sendPacket(html);
				return;
			...
			{
				final var auc = AuctionManager.getInstance().getAuction(player.getClan().getHideoutId());
				if (auc != null)
				{
					auc.cancelAuction();
			...
			else if (actualCommand.equalsIgnoreCase("sale2"))
			{
				...
				final var html = new NpcHtmlMessage(1);
				html.setFile(player, player.getLang(), filename);
				html.replace("%AGIT_LAST_PRICE%", String.valueOf(ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getRentPrice()));
