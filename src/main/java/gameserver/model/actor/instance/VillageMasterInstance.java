...
import gameserver.model.base.ClassType;
...
public class VillageMasterInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("change_clan_leader"))
		{
			...
			final var member = clan.getClanMember(cmdParams);
			if (member == null)
			{
			...
			{
				final var msg = new NpcHtmlMessage(getObjectId());
			...
			switch (cmdChoice)
			{
				...
				case 1 :
					...
					if ((subsAvailable != null) && !subsAvailable.isEmpty())
					{
						...
						for (final var subClass : subsAvailable)
						{
							StringUtil.append(content1, "<a action=\"bypass -h npc_%objectId%_Subclass 4 ", String.valueOf(subClass.getId()), "\" msg=\"1268;", Util.className(player, ClassListParser.getInstance().getClass(subClass.getId()).getClassId().getId()), "\">", Util.className(player, ClassListParser.getInstance().getClass(subClass.getId()).getClassId().getId()), "</a><br>");
						}
				...
				case 2 :
					...
					else
					{
						...
						{
							StringUtil.append(content2, "<a action=\"bypass -h npc_%objectId%_Subclass 5 0\">", Util.className(player, ClassListParser.getInstance().getClass(player.getBaseClass()).getClassId().getId()), "</a><br>");
						}
						...
						for (final var subList = iterSubClasses(player); subList.hasNext();)
						{
							...
							{
								StringUtil.append(content2, "<a action=\"bypass -h npc_%objectId%_Subclass 5 ", String.valueOf(subClass.getClassIndex()), "\">", Util.className(player, ClassListParser.getInstance().getClass(subClass.getClassId()).getClassId().getId()), "</a><br>");
							}
						}
				...
				case 3 :
					...
					if (player.getTotalSubClasses() > 3)
					{
						...
						{
							final var subClass = subList.next();
							StringUtil.append(content3, "Sub-class ", String.valueOf(classIndex++), "<br>", "<a action=\"bypass -h npc_%objectId%_Subclass 6 ", String.valueOf(subClass.getClassIndex()), "\">", Util.className(player, ClassListParser.getInstance().getClass(subClass.getClassId()).getClassId().getId()), "</a><br>");
						}
					...
					else
					{
						...
						{
							html.replace("%sub1%", Util.className(player, ClassListParser.getInstance().getClass(player.getSubClasses().get(1).getClassId()).getClassId().getId()));
						}
						else
						{
						...
						{
							html.replace("%sub2%", Util.className(player, ClassListParser.getInstance().getClass(player.getSubClasses().get(2).getClassId()).getClassId().getId()));
						}
						else
						{
						...
						{
							html.replace("%sub3%", Util.className(player, ClassListParser.getInstance().getClass(player.getSubClasses().get(3).getClassId()).getClassId().getId()));
						}
						else
						{
				...
				case 6 :
					...
					final var content6 = StringUtil.startAppend(200);
					for (final var subClass : subsAvailable)
					{
						StringUtil.append(content6, "<a action=\"bypass -h npc_%objectId%_Subclass 7 ", String.valueOf(paramOne), " ", String.valueOf(subClass.getId()), "\" msg=\"1445;", "\">", Util.className(player, ClassListParser.getInstance().getClass(subClass.getId()).getClassId().getId()), "</a><br>");
					}
				...
				case 7 :
					...
					if (player.modifySubClass(paramOne, paramTwo))
					{
						...
						html.replace("%name%", Util.className(player, ClassListParser.getInstance().getClass(paramTwo).getClassId().getId()));
	...
	public final Set<ClassId> getAvailableSubClasses(Player player)
	{
	...
	private static final void createSubPledge(Player player, String clanName, String leaderName, int pledgeType, int minClanLvl)
	{
		...
		for (final var tempClan : ClanHolder.getInstance().getClans())
		{
		...
		if (pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			final var leaderSubPledge = clan.getClanMember(leaderName);
			final var leaderPlayer = leaderSubPledge.getPlayerInstance();
	...
	public static final void showPledgeSkillList(Player player)
	{
		...
		final var skills = SkillTreesParser.getInstance().getAvailablePledgeSkills(player.getClan());
		final var asl = new ExAcquirableSkillListByClass(AcquireSkillType.PLEDGE);
		int counts = 0;
