...
import gameserver.instancemanager.games.MonsterRaceManager.RaceState;
...
public class RaceManagerInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		final var monsters = MonsterRaceManager.getInstance().getMonsters();
		if (command.startsWith("BuyTicket"))
		{
			if (MonsterRaceManager.getInstance().getCurrentRaceState() != RaceState.ACCEPTING_BETS || monsters.isEmpty())
			{
			...
			if (val < 10)
			{
				...
				int i = 0;
				for (final var monster : monsters.values())
				{
					i++;
					search = "Mob" + i;
					html.replace(search, monster.getTemplate().getName(player.getLang()));
				}
				search = "No1";
				if (val == 0)
				{
			...
			{
				if (player.getRace(0) == 0 || monsters.isEmpty())
				{
					return;
				}
				...
				search = "Mob1";
				replace = monsters.get(player.getRace(0) - 1).getTemplate().getName(player.getLang());
			...
			{
				if (player.getRace(0) == 0 || player.getRace(1) == 0 || monsters.isEmpty())
				{
					return;
				}
		...
		else if (command.equals("ShowOdds"))
		{
			...
			{
				i++;
				html.replace("Mob" + i, monster.getTemplate().getName(player.getLang()));
				
				final double odd = MonsterRaceManager.getInstance().getOdds().get(i);
				html.replace("Odd" + i, (odd > 0D) ? String.format(Locale.ENGLISH, "%.1f", odd) : "&$804;");
			}
			html.replace("1race", MonsterRaceManager.getInstance().getRaceNumber());
			html.replace("%objectId%", getObjectId());
			player.sendPacket(html);
	...
	public void removeInfoObject(GameObject object)
	{
		...
		if (object.isPlayer())
		{
			for (final Npc npc : MonsterRaceManager.getInstance().getMonsters().values())
			{
