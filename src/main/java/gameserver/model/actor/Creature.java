...
import gameserver.model.ChanceSkillList;
import gameserver.model.CharEffectList;
import gameserver.model.Clan;
import gameserver.model.FusionSkill;
...
import gameserver.model.ToggleList;
import gameserver.model.World;
import gameserver.model.WorldRegion;
import gameserver.model.actor.instance.EventMapGuardInstance;
...
import gameserver.model.actor.stat.CharStat;
import gameserver.model.actor.status.CharStatus;
import gameserver.model.actor.tasks.character.HitTask;
import gameserver.model.actor.tasks.character.MagicGeoCheckTask;
import gameserver.model.actor.tasks.character.MagicUseTask;
import gameserver.model.actor.tasks.character.NextActionTask;
import gameserver.model.actor.tasks.character.NotifyAITask;
import gameserver.model.actor.tasks.character.QueuedMagicUseTask;
...
import gameserver.model.holders.InvulSkillHolder;
...
import gameserver.model.stats.Calculator;
...
import gameserver.model.zone.type.NoGeoZone;
import gameserver.model.zone.type.PeaceZone;
...
import gameserver.network.serverpackets.Revive;
...
import gameserver.network.serverpackets.TeleportToLocation;
...
{
	public static final Logger _log = LogManager.getLogger(Creature.class);
	...
	private boolean _showSummonAnimation = false;
	...
	private final IntObjectMap<TimeStamp> _skillReuses = new CHashIntObjectMap<>();
	...
	private Future<?> _broadcastModifiedStatTask = null;
	private final Set<Stats> _broadcastModifiedStatChanges = EnumSet.noneOf(Stats.class);
	...
	public void broadcastPacket(int range, GameServerPacket... packets)
	{
		...
		World.getAroundPlayers(this, range, 300).stream().filter(p -> p != null).forEach(p -> p.sendPacket(packets));
	}
	...
	public void broadcastPacketToOthers(GameServerPacket... packets)
	{
		...
		World.getAroundPlayers(this).stream().filter(p -> p != null).forEach(p -> p.sendPacket(packets));
	}
	...
	public void teleToLocation(int x, int y, int z, int heading, int randomOffset, boolean revalidateZone, boolean sendTelePacket, Reflection r)
	{
		...
		if (isPlayer())
		{
			...
			{
				final var qs = getActingPlayer().getQuestState("_255_Tutorial");
				if (qs != null)
				{
					qs.getQuest().notifyEvent("CE42", null, getActingPlayer());
				}
			}
		}
	...
	public void doAttack(Creature target)
	{
		...
		try
		{
			...
			if (!isAlikeDead())
			{
				if ((isNpc() && target.isAlikeDead()) || !isInNeighboringRegion(target))
				{
					getAI().setIntention(CtrlIntention.ACTIVE);
					sendActionFailed();
					return;
			...
			switch (getAttackType())
			{
				case BOW :
				{
					...
					if (isAroundAttack)
					{
						hitted |= doAttackHitByAround(attack, target, timeAtk);
					}
					break;
				}
				case CROSSBOW :
				...
				{
					_attackEndTime = System.nanoTime() + TimeUnit.NANOSECONDS.convert(timeAtk, TimeUnit.MILLISECONDS);
					hitted = doAttackHitByPole(attack, target, timeAtk);
					if (isAroundAttack)
					{
						hitted |= doAttackHitByAround(attack, target, timeAtk);
					}
				...
				case FIST :
				{
					...
					{
						_attackEndTime = System.nanoTime() + TimeUnit.NANOSECONDS.convert(timeAtk, TimeUnit.MILLISECONDS);
						hitted = doAttackHitSimple(attack, target, timeAtk, true);
						break;
					}
				}
				case DUAL :
	...
	private boolean doAttackHitByBow(Attack attack, Creature target, int sAtk, int reuse)
	{
		...
		ThreadPoolManager.getInstance().schedule(new HitTask(this, target, damage1, crit1, miss1, attack.hasSoulshot(), shld1), Formulas.calculateTimeToHit(sAtk, getAttackType(), false));
	...
	private boolean doAttackHitByDual(Attack attack, Creature target, int sAtk)
	{
		...
		ThreadPoolManager.getInstance().schedule(new HitTask(this, target, damage1, crit1, miss1, attack.hasSoulshot(), shld1), Formulas.calculateTimeToHit(sAtk, getAttackType(), false));
		ThreadPoolManager.getInstance().schedule(new HitTask(this, target, damage2, crit2, miss2, attack.hasSoulshot(), shld2), Formulas.calculateTimeToHit(sAtk, getAttackType(), true));
	...
	private boolean doAttackHitByPole(Attack attack, Creature target, int sAtk)
	{
		...
		if (_fanRange != null)
		{
			...
			for (final Creature obj : World.getAroundCharacters(this, (_fanRange[2] + (getPhysicalAttackRange() / 2)), 200))
			{
				...
				if (!obj.isAlikeDead())
				{
					if ((obj == getTarget()) || obj.isAutoAttackable(this, !Config.ALLOW_POLE_FLAG_AROUND))
					{
						targets.add(obj);
						attackcount++;
					}
	...
	private boolean doAttackHitByAround(Attack attack, Creature target, int sAtk)
	{
		...
		final List<Creature> targets = new ArrayList<>();
		for (final Creature obj : World.getAroundCharacters(target, attackRageMax, 200))
		{
	...
	private void beginCast(Skill skill, boolean simultaneously)
	{
		...
		Creature target = getTarget() != null ? (Creature) getTarget() : null;
	...
	private void beginCast(Skill skill, boolean simultaneously, Creature target, GameObject[] targets)
	{
		...
		switch (skill.getFlyType())
		{
			...
			case CHARGE :
				...
				else
				{
					...
					if (isPlayer())
					{
						if (getActingPlayer().getFarmSystem().isFlyLocked(getTarget()))
						{
							sendPacket(SystemMessageId.CANT_SEE_TARGET);
						}
					}
		...
		final int reuseDelay = Formulas.calcSkillReuseDelay(this, skill, skillMastery, isOldCast);
		...
		final int initmpcons = skill.getMpInitialConsume();
		if (initmpcons > 0)
		{
		...
		if (isPlayer())
		{
			...
			{
				if (!getActingPlayer().decreaseCharges(skill.getChargeConsume()))
				{
					if (simultaneously)
					{
						setIsCastingSimultaneouslyNow(false);
		...
		if (effectWhileCasting)
		{
			...
			if (skill.getMaxSoulConsumeCount() > 0)
			{
				if (isPlayer())
				{
					getActingPlayer().decreaseSouls(skill.getMaxSoulConsumeCount());
				}
			}
		...
		if (!skill.isToggle())
		{
			broadcastPacket(new MagicSkillUse(this, target, (skill.getDisplayId() == 2099 || skill.getDisplayId() == 2100) ? 1050 : skill.getDisplayId(), skill.getDisplayLevel(), hitTime, reuseDelay));
		...
		for (final int negateSkillId : skill.getNegateCasterId())
		{
			if (negateSkillId != 0)
			{
				stopSkillEffects(negateSkillId, true);
			}
		}
	...
	{
		if ((skill == null) || isSkillDisabled(skill) || isSkillBlocked(skill) || (((skill.getFlyRadius() > 0) || skill.getFlyType() != FlyType.NONE) && isMovementDisabled() && !skill.isAllowWithMoveDisabled()))
		{
			sendActionFailed();
			return false;
		}
		...
		if (getCurrentMp() < (mpConsume + skill.getMpInitialConsume()))
		{
			if (msg)
			{
				sendPacket(SystemMessageId.NOT_ENOUGH_MP);
	...
	protected void startAttackStanceTask0()
	{
		...
		AttackStanceTaskManager.getInstance().addAttackStanceTask(this);
	}
	...
	protected void onDeath(Creature killer)
	{
		...
		if (!isPlayable())
		{
			...
			if (ai != null)
			{
				...
				ai.notifyEvent(CtrlEvent.EVT_DEAD, killer);
			}
			if (_fusionSkill != null)
			{
				abortCast();
	...
	public void doRevive()
	{
		...
		if (!isTeleporting())
		{
			...
			{
				restorefull = true;
				stopEffects(EffectType.PHOENIX_BLESSING, true);
			}
			if (restorefull)
			{
	...
	public CharacterAI getAI()
	{
		if (_ai == null)
		{
			synchronized (this)
			{
				if (_ai == null)
				{
					return _ai = new CharacterAI(this);
				}
			}
		}
		return _ai;
	...
	public void setAI(CharacterAI newAI)
	{
		...
		if (oldAI != null && oldAI.isActive())
		{
			oldAI.stopAITask();
			newAI.startAITask();
			newAI.setIntention(CtrlIntention.ACTIVE);
		}
	}
	...
	public void addEffect(Effect newEffect, boolean update)
	{
		_effects.queueEffect(newEffect, false, true, update);
	}
	
	public void removeEffect(Effect effect, boolean printMessage, boolean update)
	{
		_effects.queueEffect(effect, true, printMessage, update);
	}
	...
	{
		_effects.stopAllEffects(isAll, true);
	}
	...
	public void stopSkillEffects(int skillId, boolean update)
	{
		_effects.stopSkillEffects(skillId, update);
	}
	
	public final void stopEffects(EffectType type, boolean update)
	{
		_effects.stopEffects(type, update);
	}
	...
	public final void stopFakeDeath(boolean removeEffects)
	{
		if (removeEffects)
		{
			stopEffects(EffectType.FAKE_DEATH, true);
		}
	...
	public final void stopTransformation(boolean removeEffects)
	{
		if (removeEffects)
		{
			stopEffects(EffectType.TRANSFORMATION, false);
		}
	...
	protected void broadcastModifiedStats(Set<Stats> stats)
	{
		...
		{
			_broadcastModifiedStatChanges.addAll(stats);
		}
		
		if (_broadcastModifiedStatTask == null)
		{
			_broadcastModifiedStatTask = ThreadPoolManager.getInstance().schedule(() ->
			{
				Set<Stats> currentChanges;
				synchronized (_broadcastModifiedStatChanges)
				{
				...
				else
				{
					...
					for (final var stat : currentChanges)
					{
						switch (stat)
						{
							case MOVE_SPEED :
			...
			}, Config.USER_STATS_UPDATE_INTERVAL);
		}
	}
	...
	public void abortCast()
	{
		if (isCastingNow() || isCastingSimultaneouslyNow())
		{
			...
			final var mog = getFirstEffect(EffectType.SIGNET_GROUND);
			if (mog != null)
			{
				mog.exit(true);
			}
	...
	public void revalidateZone(boolean force)
	{
		...
		if (force || _zoneValidateCounter > 4)
		{
			...
			final List<ZoneType> currentZone = ZoneManager.getInstance().getZones(this);
	...
	public void onHitTimer(Creature target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld)
	{
		...
		if ((isNpc() && target.isAlikeDead()) || target.isDead() || (!isInNeighboringRegion(target) && !isDoor()))
		{
			rechargeShots(true, false);
			getAI().notifyEvent(CtrlEvent.EVT_CANCEL);
			sendActionFailed();
		...
		if (!target.isDead() && target.isRaid() && target.giveRaidCurse() && !Config.RAID_DISABLE_CURSE)
		{
			if (getLevel() > (target.getLevel() + 8) && target.getId() != 29054)
			{
				...
				if (skill != null)
				{
					...
					abortCast();
					getAI().setIntention(CtrlIntention.IDLE);
					skill.getEffects(target, this, false, true);
				}
				damage = 0;
			}
		}
		...
		if (!miss && (damage > 0))
		{
			final boolean isBow = isBowAttack();
			...
			if (_chanceSkills != null && !target.isInvul())
			{
			...
			{
				_triggerSkills.values().stream().filter(t -> ((!crit && (t.getSkillType() == OptionsSkillType.ATTACK)) || ((t.getSkillType() == OptionsSkillType.CRITICAL) && crit)) && Rnd.chance(t.getChance())).forEach(t -> makeTriggerCast(t.getSkill(), target));
			}
			
			if (target.getChanceSkills() != null && !isInvul())
			{
				target.getChanceSkills().onHit(this, damage, true, crit);
			}
		}
		
		final var activeWeapon = getActiveWeaponItem();
	...
	public Skill addSkill(Skill newSkill)
	{
		...
		if (newSkill != null)
		{
			...
			if (!isBlocked)
			{
				...
				newSkill.getEffectsPassive(this, false);
			}
		}
		return oldSkill;
	}
	...
	public Skill removeSkill(int skillId, boolean cancelEffect)
	{
		...
		if (oldSkill != null)
		{
			...
			if (_castingSkill != null && isCastingNow() && skillId == _castingSkill.getId())
			{
				abortCast();
			}
			
			if (_lastSimultaneousSkillCast != null && isCastingSimultaneouslyNow() && skillId == _lastSimultaneousSkillCast.getId())
			{
				abortCast();
			}
			...
			if (cancelEffect || oldSkill.isToggle())
			{
				...
				{
					removeStatsOwner(oldSkill);
					stopSkillEffects(oldSkill.getId(), true);
				}
			}
	...
	public void onMagicLaunchedTimer(MagicUseTask mut)
	{
		...
		final Creature t = _target != null && _target.isCreature() ? (Creature) _target : null;
		GameObject[] targets = isPlayer() ? skill.isAura() ? skill.getTargetList(this) : skill.isArea() ? skill.getTargetList(this, false, t) : mut.getTargets() : mut.getTargets();
		...
		if (skill.oneTarget() && targets.length > 0)
		{
			final var target = targets[0];
	...
	public void onMagicFinalizer(MagicUseTask mut)
	{
		...
		if (isPlayable() && skill.nextActionIsAttack())
		{
			if (isPlayer())
			{
			...
			{
				ThreadPoolManager.getInstance().schedule(new NextActionTask(this, target), 100);
			}
		}
	...
	public void addBlockSkill(Skill sk, boolean removeStats)
	{
		...
		if (removeStats)
		{
			...
			stopSkillEffects(sk.getId(), true);
	...
	public void removeBlockSkill(Skill sk, boolean restoreStats)
	{
		...
		if (isSkillBlocked(sk))
		{
			if (restoreStats)
			{
				...
				sk.getEffectsPassive(this, false);
			}
		...
		try
		{
			...
			for (final GameObject trg : targets)
			{
				...
				if (trg instanceof Creature t)
				{
					...
					if (!Config.RAID_DISABLE_CURSE && (!target.isDead() && ((target.isRaid() && target.giveRaidCurse() && (getLevel() > (target.getLevel() + 8))) || (!skill.isOffensive() && target.isRaid() && target.giveRaidCurse() && target.getAttackByList().contains(target) && (getLevel() > (target.getLevel() + 8))))))
					{
						if (skill.isMagic())
						{
							...
							if (tempSkill != null)
							{
								...
								abortCast();
								getAI().setIntention(CtrlIntention.IDLE);
								tempSkill.getEffects(target, this, false, true);
							}
						}
						else
						{
			...
			if (handler != null)
			{
				handler.useSkill(this, skill, targets, 0);
			}
			else
			{
				skill.useSkill(this, targets, 0);
			}
			...
			if (player != null)
			{
				for (final Npc npcMob : World.getAroundNpc(player, 1000, 200))
				{
					final var ai = npcMob.getAI();
					if (ai != null)
					{
						ai.notifyEvent(CtrlEvent.EVT_SEE_SPELL, skill, this);
						...
						if (npcMob.isAttackable() && !skill.isEffectTypeBattle())
						{
							...
							if (skillEffectPoint > 0 && !attackable.isInvul())
							{
	...
	{
		if (skill != null && !skill.isStatic() && getChanceSkills() != null && !attacker.isInvul())
		{
	...
	{
		getStatus().clearStatusListeners();
	}
	...
	public boolean updatePosition()
	{
		...
		int yPrev = getY();
		int zPrev = getZ();
		if (m._wasdMove && getStartLocWASD() != null)
		{
			final int startX = getStartLocWASD().getX();
			final int startY = getStartLocWASD().getY();
			final double dx = startX - xPrev;
			final double dy = startY - yPrev;
		...
		if (delta > 1)
		{
			final double distPassed = (getStat().getRealMoveSpeed(!isFlying() && moveSpeed > Config.GEO_MOVE_SPEED && m._tick <= 1) * (gameTicks - m._moveTimestamp)) / GameTimeController.TICKS_PER_SECOND;
			distFraction = distPassed / delta;
		}
		
		if (m._wasdMove && !isFloating)
		{
			final double checkDist = distFraction + 0.1;
			final int checkX = (int) (m._xAccurate + (dx * checkDist));
			final int checkY = (int) (m._yAccurate + (dy * checkDist));
			
			if (checkX > World.MAP_MIN_X && checkX < World.MAP_MAX_X && checkY > World.MAP_MIN_Y && checkY < World.MAP_MAX_Y)
			{
				final int colRadius = (int) getColRadius();
				...
				if (dist > 1)
				{
					...
					if (!GeoEngine.getInstance().canMoveToCoord(this, xPrev, yPrev, zPrev, checkX, checkY, zPrev, getReflection(), true))
					{
						blocked = true;
					}
					...
					if (!blocked)
					{
						...
						if (!GeoEngine.getInstance().canMoveToCoord(this, xPrev, yPrev, zPrev, leftX, leftY, zPrev, getReflection(), true))
						{
							blocked = true;
						}
					}
					...
					if (!blocked)
					{
						...
						if (!GeoEngine.getInstance().canMoveToCoord(this, xPrev, yPrev, zPrev, rightX, rightY, zPrev, getReflection(), true))
						{
							blocked = true;
						}
					}
					...
					if (!blocked)
					{
						final int futureZ = GeoEngine.getInstance().getHeight(checkX, checkY, zPrev);
						final int zDiff = Math.abs(futureZ - zPrev);
						if (zDiff > 64)
						{
							blocked = true;
						}
		...
		if (distFraction > 1)
		{
			...
			else
			{
				...
				if (m._wasdMove && Math.abs(zDiff) > 64)
				{
					_move = null;
					broadcastPacket(new StopMove(this));
					return true;
	...
	public void moveToLocation(int x, int y, int z, int offset, boolean wasdMove)
	{
		...
		final var m = new MoveData();
		m._wasdMove = wasdMove;
		m._pathFindIndex = -1;
		...
		if (!isNoGeoZone && !isFlying())
		{
			...
			if ((Config.PATHFIND_BOOST) && ((originalDistance - distance) > 30) && !isFloating() && !m._wasdMove && !isFalling() && !isInWater(this))
			{
