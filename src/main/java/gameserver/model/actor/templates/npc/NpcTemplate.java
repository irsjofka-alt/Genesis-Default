...
import gameserver.instancemanager.NpcStatManager.CustomStatType;
...
{
	private static final Logger _log = LogManager.getLogger(NpcTemplate.class);
	
	@SuppressWarnings("unchecked")
	public static final Constructor<Npc> DEFAULT_TYPE_CONSTRUCTOR = (Constructor<Npc>) Npc.class.getConstructors()[0];
	@SuppressWarnings("unchecked")
	...
	private Class<Npc> _classType = Npc.class;
	private Class<CharacterAI> _aiClass = CharacterAI.class;
	private Constructor<Npc> _constructorType = DEFAULT_TYPE_CONSTRUCTOR;
	private Constructor<CharacterAI> _aiConstructor = DEFAULT_AI_CONSTRUCTOR;
	...
	private final int _minionRange;
	private final int _minionAngleRange;
	...
	public NpcTemplate(StatsSet set)
	{
		...
		_minionRange = set.getInteger("minionRange", 150);
		_minionAngleRange = set.getInteger("minionAngleRange", 70);
		setType(_type);
	...
	public CharacterAI getNewAI(final Attackable npc)
	{
		try
		{
			return _aiConstructor.newInstance(npc);
		}
		catch (final Exception e)
		{
	...
	private void setType(String type)
	{
		...
		else
		{
			...
			_constructorType = (Constructor<Npc>) _classType.getConstructors()[0];
		}
	...
	private void setAI(String ai)
	{
		...
		{
			_aiClass = classAI;
			_aiConstructor = (Constructor<CharacterAI>) _aiClass.getConstructors()[0];
		}
		
		if (_aiClass.isAnnotationPresent(Deprecated.class))
		{
			_log.error("Ai type: " + ai + ", is deprecated. NpcId: " + _npcId);
		}
		_classAI = ai;
		_isEpicRaid = isType("GrandBoss") || isType("FlyGrandBoss");
		_isRaid = isType("RaidBoss") || isType("FlyRaidBoss") || isType("LostCaptain");
		_isSiegeGuard = isType("Defender") || isType("FortCommander");
		_isFlying = isType("FlyRaidBoss") || isType("FlyGrandBoss") || isType("FlyMonster") || isType("FlyNpc");
	}
	...
	{
		return classId.level() == 3 ? _teachInfo.contains(classId.getParent()) : _teachInfo.contains(classId);
	}
	...
	{
		if (getParameter("blockAutoFarmTarget", false))
		{
			return true;
		}
		return false;
	...
	public int getExpReward(Creature attacker, boolean isRaid)
	{
		final var player = attacker.getActingPlayer();
		if (player != null)
		{
			final var party = player.getParty();
			return (int) (getRewardExp() * (isRaid ? Config.RATE_RAID_XP_BY_LVL[party != null ? party.getLevel() : player.getLevel()] * (!player.getPremiumBonus().isPersonal() ? (party != null && Config.PREMIUM_PARTY_RATE ? party.getRateXp() : player.getPremiumBonus().getRateXp()) : 1) : Config.RATE_XP_BY_LVL[party != null ? party.getLevel() : player.getLevel()] * (!player.getPremiumBonus().isPersonal() ? (party != null && Config.PREMIUM_PARTY_RATE ? party.getRateXp() : player.getPremiumBonus().getRateXp()) : 1)));
		}
		return (int) (getRewardExp() * (isRaid ? Config.RATE_RAID_XP_BY_LVL[attacker.getLevel()] : Config.RATE_XP_BY_LVL[attacker.getLevel()]));
	}
	
	public int getSpReward(Creature attacker, boolean isRaid)
	{
		final var player = attacker.getActingPlayer();
		if (player != null)
		{
			final var party = player.getParty();
			return (int) (getRewardSp() * (isRaid ? Config.RATE_RAID_SP_BY_LVL[party != null ? party.getLevel() : player.getLevel()] * (!player.getPremiumBonus().isPersonal() ? (party != null && Config.PREMIUM_PARTY_RATE ? party.getRateSp() : player.getPremiumBonus().getRateSp()) : 1) : Config.RATE_SP_BY_LVL[party != null ? party.getLevel() : player.getLevel()] * (!player.getPremiumBonus().isPersonal() ? (party != null && Config.PREMIUM_PARTY_RATE ? party.getRateSp() : player.getPremiumBonus().getRateSp()) : 1)));
		}
		return (int) (getRewardSp() * (isRaid ? Config.RATE_RAID_SP_BY_LVL[attacker.getLevel()] : Config.RATE_SP_BY_LVL[attacker.getLevel()]));
	}
	...
	public int getMinionAngleRange()
	{
		return _minionAngleRange;
	}
}