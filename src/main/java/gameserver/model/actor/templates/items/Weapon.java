...
public final class Weapon extends Item
{
	...
	public Effect[] getSkillEffects(Creature caster, Creature target, boolean crit)
	{
		...
		if (caster.isSkillDisabled(onCritSkill) || caster.isSkillBlocked(onCritSkill))
		{
			return _emptyEffectSet;
		}
		
		final var player = caster.getActingPlayer();
		final var playerTarget = target.getActingPlayer();
		if ((player != null && playerTarget != null && onCritSkill.isDebuff() && player.isFriend(playerTarget)))
		{
			return _emptyEffectSet;
		}
		...
		if (!Formulas.calcSkillSuccess(caster, target, onCritSkill, null, onCritSkill.getPower(), shld, false, false, false))
		{
			return _emptyEffectSet;
		}
		...
		for (final var e : onCritSkill.getEffects(caster, target, new Env(shld, false, false, false), true, false))
		{
			effects.add(e);
		}
		if (effects.isEmpty())
	...
	public Effect[] getSkillEffects(Creature caster, Creature target, Skill trigger)
	{
		...
		if (onMagicSkill.isOffensive() && !Formulas.calcSkillSuccess(caster, target, onMagicSkill, null, onMagicSkill.getPower(), shld, false, false, false))
		{
			return _emptyEffectSet;
		}
		...
		if (handler != null)
		{
			handler.useSkill(caster, onMagicSkill, targets, 0);
		}
		else
		{
			onMagicSkill.useSkill(caster, targets, 0);
		}
		
		if (caster.isPlayer())
		{
			for (final var npcMob : World.getAroundNpc(caster))
			{
