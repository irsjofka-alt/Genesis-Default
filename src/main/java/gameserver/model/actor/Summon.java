...
import gameserver.network.serverpackets.ExPartyPetWindowUpdate;
import gameserver.network.serverpackets.GameServerPacket;
import gameserver.network.serverpackets.MoveToLocation;
import gameserver.network.serverpackets.NpcInfo.SummonInfo;
import gameserver.network.serverpackets.PetDelete;
import gameserver.network.serverpackets.PetInfo;
import gameserver.network.serverpackets.PetItemList;
import gameserver.network.serverpackets.PetStatusUpdate;
...
public abstract class Summon extends Playable
{
	...
	public void unSummonForce(Player owner)
	{
		if (isVisible())
		{
			if (isDead())
			...
			{
				owner.sendPacket(new PetDelete(getSummonType(), getObjectId()));
				final var party = owner.getParty();
				if (party != null)
				{
					party.broadcastToPartyMembers(owner, new ExPartyPetWindowDelete(this));
				}
	...
	{
		return _owner == null ? false : _owner.isInFightEvent();
	}
