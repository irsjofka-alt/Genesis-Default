...
import gameserver.network.serverpackets.ExVitalityPointInfo;
...
public class PcStat extends PlayableStat
{
	...
	public boolean addExpAndSp(long addToExp, int addToSp, boolean useBonuses)
	{
		...
		if (Config.ENABLE_SPECIAL_TUTORIAL && addToExp > 0 || (activeChar.getLevel() >= 6 && activeChar.getLevel() <= 10))
		{
			final var qs = activeChar.getQuestState("_255_Tutorial");
			if (qs != null)
			{
				qs.getQuest().notifyEvent("CE41", null, getActiveChar());
			}
		}
		return true;
	}
