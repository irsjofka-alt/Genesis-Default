 ...
 */
package gameserver.model.actor.tasks.player;
...
public class AutoSummonFarmTask extends LoggerObject implements Runnable
{
	...
	private void tryAttack(boolean selected, boolean isMagic)
	{
		if (_player.isDead())
		{
			...
			if (!_player.getFarmSystem().isInPause())
			{
		...
		try
		{
			if (!_player.getFarmSystem().isInWaitStatus(_committedTarget))
			{
				...
				{
					_player.getFarmSystem().setAbortTarget(false, _committedTarget);
	...
	private boolean tryUseSpell(boolean isMagic)
	{
		...
		final var selfSkills = _player.getFarmSystem().nextSelfSkill(_committedSummon);
		if (selfSkills != null)
		{
	...
	private boolean selectRandomTarget()
	{
		...
		if (_committedTarget != null)
		{
			if (_committedTarget.isDead() && _committedTarget.isSpoiled() && canUseSweep())
			{
				return false;
			}
		...
		else
		{
			...
			final var target = _player.getFarmSystem().getAroundNpc();
			if (target == null)
			{
				_player.getFarmSystem().checkEmptyTime();
				return false;
