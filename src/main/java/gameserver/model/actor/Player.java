...
import static gameserver.network.serverpackets.ExSetCompassZoneCode.ZONE_ALTERED_FLAG;
import static gameserver.network.serverpackets.ExSetCompassZoneCode.ZONE_PEACE_FLAG;
import static gameserver.network.serverpackets.ExSetCompassZoneCode.ZONE_PVP_FLAG;
import static gameserver.network.serverpackets.ExSetCompassZoneCode.ZONE_SIEGE_FLAG;
import static gameserver.network.serverpackets.ExSetCompassZoneCode.ZONE_SSQ_FLAG;
...
import gameserver.ai.character.PlayerAI;
import gameserver.ai.model.CtrlIntention;
import gameserver.data.dao.AchievementsDAO;
import gameserver.data.dao.CharacterBookMarkDAO;
import gameserver.data.dao.CharacterDAO;
import gameserver.data.dao.CharacterHennaDAO;
import gameserver.data.dao.CharacterItemReuseDAO;
import gameserver.data.dao.CharacterPremiumDAO;
import gameserver.data.dao.CharacterSkillSaveDAO;
import gameserver.data.dao.CharacterSkillsDAO;
import gameserver.data.dao.CharacterVariablesDAO;
import gameserver.data.dao.CharacterVisualDAO;
import gameserver.data.dao.DailyTasksDAO;
import gameserver.data.holder.CharNameHolder;
import gameserver.data.htm.HtmCache;
import gameserver.data.htm.WarehouseCache;
...
import gameserver.instancemanager.BotCheckManager.BotCheckQuestion;
...
import gameserver.instancemanager.DuelManager;
...
import gameserver.instancemanager.MatchingRoomManager;
...
import gameserver.model.TimeStamp;
import gameserver.model.TradeItem;
import gameserver.model.TradeList;
...
import gameserver.model.actor.instance.ControlTowerInstance;
...
import gameserver.model.actor.instance.EventChestInstance;
...
import gameserver.model.actor.instance.TamedBeastInstance;
import gameserver.model.actor.instance.TrapInstance;
import gameserver.model.actor.instance.player.AchievementCounters;
import gameserver.model.actor.instance.player.AutoFarmOptions;
import gameserver.model.actor.instance.player.BypassStorage;
import gameserver.model.actor.instance.player.CharacterVariable;
import gameserver.model.actor.instance.player.DonateRates;
import gameserver.model.actor.instance.player.NevitSystem;
import gameserver.model.actor.instance.player.PersonalTasks;
import gameserver.model.actor.instance.player.PremiumBonus;
import gameserver.model.actor.instance.player.Recommendation;
import gameserver.model.actor.instance.player.impl.AutoSaveTask;
import gameserver.model.actor.instance.player.impl.CheckBotTask;
import gameserver.model.actor.instance.player.impl.CleanUpTask;
import gameserver.model.actor.instance.player.impl.DismountTask;
import gameserver.model.actor.instance.player.impl.FallingTask;
import gameserver.model.actor.instance.player.impl.FameTask;
import gameserver.model.actor.instance.player.impl.HardWareTask;
import gameserver.model.actor.instance.player.impl.InventoryEnableTask;
import gameserver.model.actor.instance.player.impl.LookingForFishTask;
import gameserver.model.actor.instance.player.impl.OnlineRewardTask;
import gameserver.model.actor.instance.player.impl.PcPointsTask;
import gameserver.model.actor.instance.player.impl.PetFeedTask;
import gameserver.model.actor.instance.player.impl.PremiumItemTask;
import gameserver.model.actor.instance.player.impl.PremiumTask;
import gameserver.model.actor.instance.player.impl.PunishmentTask;
import gameserver.model.actor.instance.player.impl.PvPFlagTask;
import gameserver.model.actor.instance.player.impl.RemoveWearItemsTask;
import gameserver.model.actor.instance.player.impl.RentPetTask;
import gameserver.model.actor.instance.player.impl.ResetChargesTask;
import gameserver.model.actor.instance.player.impl.ResetSoulsTask;
import gameserver.model.actor.instance.player.impl.ReviveTask;
import gameserver.model.actor.instance.player.impl.SitDownTask;
import gameserver.model.actor.instance.player.impl.StandUpTask;
import gameserver.model.actor.instance.player.impl.TeleportTask;
import gameserver.model.actor.instance.player.impl.TempHeroTask;
import gameserver.model.actor.instance.player.impl.VitalityTask;
import gameserver.model.actor.instance.player.impl.WarnUserTakeBreakTask;
import gameserver.model.actor.instance.player.impl.WaterTask;
import gameserver.model.actor.protection.AdminProtection;
import gameserver.model.actor.stat.PcStat;
import gameserver.model.actor.status.PcStatus;
import gameserver.model.actor.tasks.player.AcpTask;
import gameserver.model.actor.templates.BookmarkTemplate;
import gameserver.model.actor.templates.DressArmorTemplate;
import gameserver.model.actor.templates.DressCloakTemplate;
import gameserver.model.actor.templates.DressHatTemplate;
import gameserver.model.actor.templates.DressShieldTemplate;
import gameserver.model.actor.templates.DressWeaponTemplate;
import gameserver.model.actor.templates.ManufactureItemTemplate;
import gameserver.model.actor.templates.PetLevelTemplate;
import gameserver.model.actor.templates.PremiumItemTemplate;
import gameserver.model.actor.templates.ShortCutTemplate;
import gameserver.model.actor.templates.daily.DailyTaskTemplate;
...
import gameserver.model.actor.templates.player.AchiveTemplate;
import gameserver.model.actor.templates.player.FakeLocTemplate;
import gameserver.model.actor.templates.player.FakePassiveLocTemplate;
import gameserver.model.actor.templates.player.PcTeleportTemplate;
...
import gameserver.model.actor.templates.player.ranking.PartyTemplate;
import gameserver.model.actor.templates.player.vip.VipTemplate;
import gameserver.model.actor.transform.Transform;
import gameserver.model.actor.transform.TransformTemplate;
...
import gameserver.model.entity.events.model.impl.MonsterAttackEvent;
import gameserver.model.entity.events.model.template.FightEventGameRoom;
import gameserver.model.entity.mods.SellBuffsManager;
import gameserver.model.entity.underground_coliseum.UCTeam;
import gameserver.model.fishing.Fish;
import gameserver.model.fishing.Fishing;
import gameserver.model.holders.ItemHolder;
import gameserver.model.holders.SellBuffHolder;
import gameserver.model.holders.SkillHolder;
import gameserver.model.holders.SkillUseHolder;
...
import gameserver.model.items.itemcontainer.PcFreight;
import gameserver.model.items.itemcontainer.PcInventory;
import gameserver.model.items.itemcontainer.PcPrivateInventory;
import gameserver.model.items.itemcontainer.PcRefund;
import gameserver.model.items.itemcontainer.PcWarehouse;
import gameserver.model.items.itemcontainer.PetInventory;
...
import gameserver.model.olympiad.AbstractOlympiadGame;
import gameserver.model.olympiad.OlympiadGameManager;
import gameserver.model.olympiad.OlympiadManager;
import gameserver.model.petition.PetitionMainGroup;
...
import gameserver.model.service.academy.AcademyList;
import gameserver.model.service.autoenchant.EnchantParams;
import gameserver.model.service.autofarm.FarmSettings;
import gameserver.model.service.buffer.PlayerScheme;
import gameserver.model.service.premium.PremiumGift;
import gameserver.model.service.premium.PremiumTemplate;
...
import gameserver.model.skills.l2skills.SkillSiegeFlag;
...
import gameserver.model.zone.type.JailZone;
import gameserver.model.zone.type.SiegeZone;
import gameserver.model.zone.type.WaterZone;
import gameserver.network.GameClient;
import gameserver.network.SystemMessageId;
import gameserver.network.communication.AuthServerCommunication;
import gameserver.network.serverpackets.AutoAttackStart;
import gameserver.network.serverpackets.CameraMode;
import gameserver.network.serverpackets.ChairSit;
...
import gameserver.network.serverpackets.ExBrAgathionEnergyInfo;
import gameserver.network.serverpackets.ExBrExtraUserInfo;
import gameserver.network.serverpackets.ExBrPremiumState;
import gameserver.network.serverpackets.ExDominionWarStart;
import gameserver.network.serverpackets.ExDuelUpdateUserInfo;
import gameserver.network.serverpackets.ExFishingEnd;
import gameserver.network.serverpackets.ExFishingStart;
import gameserver.network.serverpackets.ExGetBookMarkInfo;
import gameserver.network.serverpackets.ExGetOnAirShip;
import gameserver.network.serverpackets.ExNotifyPremiumItem;
import gameserver.network.serverpackets.ExOlympiadMode;
import gameserver.network.serverpackets.ExPrivateStorePackageMsg;
import gameserver.network.serverpackets.ExQuestItemList;
import gameserver.network.serverpackets.ExSetCompassZoneCode;
import gameserver.network.serverpackets.ExStartScenePlayer;
import gameserver.network.serverpackets.ExStorageMaxCount;
import gameserver.network.serverpackets.ExUseSharedGroupItem;
import gameserver.network.serverpackets.ExVoteSystemInfo;
import gameserver.network.serverpackets.FlyToLocation.FlyType;
import gameserver.network.serverpackets.GameGuardQuery;
...
import gameserver.network.serverpackets.L2FriendStatus;
import gameserver.network.serverpackets.LogOutOk;
import gameserver.network.serverpackets.MagicSkillUse;
import gameserver.network.serverpackets.MoveToLocation;
import gameserver.network.serverpackets.MyTargetSelected;
import gameserver.network.serverpackets.NickNameChanged;
import gameserver.network.serverpackets.NormalCamera;
import gameserver.network.serverpackets.NpcHtmlMessage;
import gameserver.network.serverpackets.NpcInfo;
import gameserver.network.serverpackets.NpcInfoPoly;
import gameserver.network.serverpackets.ObserverEnd;
import gameserver.network.serverpackets.ObserverStart;
import gameserver.network.serverpackets.PartySmallWindowUpdate;
import gameserver.network.serverpackets.PetInventoryUpdate;
import gameserver.network.serverpackets.PlaySound;
import gameserver.network.serverpackets.PrivateStoreBuyList;
import gameserver.network.serverpackets.PrivateStoreBuyManageList;
import gameserver.network.serverpackets.PrivateStoreBuyMsg;
import gameserver.network.serverpackets.PrivateStoreSellList;
import gameserver.network.serverpackets.PrivateStoreSellManageList;
import gameserver.network.serverpackets.PrivateStoreSellMsg;
import gameserver.network.serverpackets.RecipeShopMsg;
import gameserver.network.serverpackets.RecipeShopSellList;
import gameserver.network.serverpackets.RelationChanged;
import gameserver.network.serverpackets.Ride;
import gameserver.network.serverpackets.ServerClose;
import gameserver.network.serverpackets.SetupGauge;
...
import gameserver.network.serverpackets.SkillCoolTime;
import gameserver.network.serverpackets.SkillList;
import gameserver.network.serverpackets.Snoop;
...
import gameserver.network.serverpackets.TargetSelected;
import gameserver.network.serverpackets.TargetUnselected;
import gameserver.network.serverpackets.TradeDone;
import gameserver.network.serverpackets.TradePressOtherOk;
import gameserver.network.serverpackets.TradePressOwnOk;
import gameserver.network.serverpackets.TradeStart;
import gameserver.network.serverpackets.UserInfo;
...
import gameserver.network.serverpackets.pledge.PledgeShowMemberListDelete;
import gameserver.network.serverpackets.pledge.PledgeShowMemberListDeleteAll;
...
public class Player extends Playable implements PlayerGroup
{
	...
	private boolean _offlineFarm = false;
	...
	private final List<ItemInstance> _itemEquipChanges = new CopyOnWriteArrayList<>();
	...
	private Location _startLocWASD;
	...
	private long _attackInfoDelay = 0;
	...
	public boolean isAttackInfoDelay()
	{
		return _attackInfoDelay > System.currentTimeMillis();
	}
	
	public void setAttackInfoDelay(long val)
	{
		_attackInfoDelay = System.currentTimeMillis() + val;
	}
	
	public boolean isSpawnProtected()
	{
		return _protectEndTime > System.currentTimeMillis();
	}
	...
	private long _activeEnchantAttrItemCount = 1;
	...
	private boolean _canSelfResurrection = true;
	...
	public void startPvPFlag()
	{
		...
		{
			getFarmSystem().stopFarmTask(false, null);
		}
	...
	public Collection<Quest> getAllActiveQuests()
	{
		...
		_quests.values().stream().filter(q -> q != null && q.getQuest() != null && q.isStarted() && q.getQuest().getId() > 0).forEach(q -> quests.add(q.getQuest()));
		return quests;
	}
		...
		if ((_karma == 0) && (karma > 0))
		{
			...
			try
			{
				...
				for (final GameObject object : targets)
				{
					...
					final var guard = (GuardInstance) object;
					
					if (guard.getAI().getIntention() == CtrlIntention.IDLE)
					{
						guard.getAI().setIntention(CtrlIntention.ACTIVE);
						((DefaultAI) guard.getAI()).startAITask();
					}
				}
			}
			finally
	...
	public void refreshExpertisePenalty()
	{
		...
		for (final ItemInstance item : _inventory.getItems())
		{
	...
	{
		final var item = _inventory.getItemByObjectId(objectId);
		if (item == null)
		{
			return;
		}
		...
		if (isEquiped)
		{
			...
			final var slot = getInventory().getSlotFromItem(item);
			if (slot == Item.SLOT_DECO)
			{
				_itemEquipChanges.addAll(getInventory().unEquipItemInSlotAndRecord(item.getLocationSlot(), false));
			}
			else
			{
				_itemEquipChanges.addAll(getInventory().unEquipItemInBodySlotAndRecord(slot, false));
			}
		}
		else
		{
			_itemEquipChanges.addAll(getInventory().equipItemAndRecord(item, false));
			if (item.isEquipped())
			{
				if (item.getEnchantLevel() > 0)
				{
		...
		if (_itemEquipTask == null)
		{
			_itemEquipTask = ThreadPoolManager.getInstance().schedule(() ->
			{
				final List<ItemInstance> updateItems = new ArrayList<>();
				synchronized (_itemEquipChanges)
				{
					if (_itemEquipChanges.isEmpty())
					{
						return;
					}
					updateItems.addAll(_itemEquipChanges);
					_itemEquipChanges.clear();
				}
				
				refreshExpertisePenalty();
				sendUserInfo(true);
				final InventoryUpdate iu = new InventoryUpdate();
				iu.addItems(updateItems);
	...
	public void setClassId(int Id)
	{
		...
		try
		{
			if ((getLvlJoinedAcademy() != 0) && (_clan != null) && CategoryParser.getInstance().isInCategory(CategoryType.THIRD_CLASS_GROUP, Id))
			{
				...
				_inventory.addItem("Gift", 8181, 1, this, null);
	...
	public void setActiveEnchantAttrItemId(int objectId)
	{
		...
		if (_activeEnchantAttrItemId == ID_NONE)
		{
			_activeEnchantAttrItemCount = 1L;
		}
	}
	...
	public void setActiveEnchantAttrItemCount(long count)
	{
		_activeEnchantAttrItemCount = count;
	}
	
	public long getActiveEnchantAttrItemCount()
	{
		return _activeEnchantAttrItemCount;
	}
	...
	public int giveAvailableSkills(boolean includedByFs, boolean includeAutoGet)
	{
		...
		final var skills = SkillTreesParser.getInstance().getAllAvailableSkills(this, getClassId(), includedByFs, includeAutoGet);
		final List<Skill> skillsForStore = new ArrayList<>();
		for (final var sk : skills)
		{
			final var skill = getKnownSkill(sk.getId());
			if (skill == sk || (skill != null && skill.getLevel() > sk.getLevel()))
			{
				continue;
			}
			...
			if (sk.isToggle())
			{
				...
				if (toggleEffect != null)
				{
					...
					sk.getEffects(this, this, true, false);
					updateEffectIcons();
				}
			}
			addSkill(sk, false);
	...
	{
		final var autoGetSkills = SkillTreesParser.getInstance().getAvailableAutoGetSkills(this);
		final SkillsParser st = SkillsParser.getInstance();
		Skill skill;
		for (final var s : autoGetSkills)
		{
	...
	public void standUp()
	{
		...
		if (_waitTypeSitting && !isInStoreMode() && !isAlikeDead())
		{
			...
			{
				stopEffects(EffectType.RELAXING, true);
			}
	...
	{
		_protectEndTime = protect ? System.currentTimeMillis() + (Config.PLAYER_SPAWN_PROTECTION * 1000) : 0;
	}
	...
	{
		_recentFakeDeathEndTime = protect ? System.currentTimeMillis() + (Config.PLAYER_FAKEDEATH_UP_PROTECTION * 1000) : 0;
	}
	...
	{
		return _client != null ? _client.isDetached() ? _cacheIp != null ? _cacheIp : "" : _client.getIPAddress() : "N/A";
	}
	
	public String getDefaultAddress()
	{
		return _client != null ? _client.isDetached() ? _cacheIp != null ? _cacheIp : "" : _client.getDefaultAddress() : "";
	}

	public String getHWID()
	{
		return _client != null ? _client.isDetached() ? _cacheHwid != null ? _cacheHwid : "N/A" : _client.getHWID() : "N/A";
	}
	...
	public boolean canOfflineFarmMode()
	{
		return !isDead() && getFarmSystem().isAutofarming() && canActivateFarm();
	}
	
	private boolean canActivateFarm()
	{
		if (!FarmSettings.ALLOW_CHECK_HWID_LIMIT || FarmSettings.FARM_ACTIVE_LIMITS <= 0)
		{
			return true;
		}
		...
		{
			if (player.getHWID() != null && player.getHWID().equalsIgnoreCase(getHWID()) && (player.isInOfflineFarmMode() || player.getFarmSystem().isAutofarming()))
			{
				i++;
				if (i > FarmSettings.FARM_ACTIVE_LIMITS)
				{
					return false;
				}
			}
		}
		return i <= FarmSettings.FARM_ACTIVE_LIMITS;
	}
	...
	public boolean checkDoCastConditions(Skill skill, boolean msg)
	{
		...
		if ((skill.getSkillType() == SkillType.SUMMON) && (isMounted() || inObserverMode()))
		{
			if (msg)
			{
	...
	public void doAutoLoot(Attackable target, int itemId, long itemCount)
	{
		...
		Log.addLogItem("PickUp", getName(null) + " -> receive ->", Util.getItemName(itemId), itemCount, 0, "from " + target.getName(null));
	}
	...
	public void doPickupItem(GameObject object)
	{
		...
		else
		{
			...
			else if ((target.getId() == PcInventory.ADENA_ID) && (_inventory.getAdenaInstance() != null))
			{
				addAdena("Pickup", target.getCount(), null, true);
				ItemsParser.getInstance().destroyItem("Pickup", target, this, null);
				Log.addLogItem("PickUp", getName(null) + " -> receive ->", "Adena", target.getCount(), 0, null);
			}
			else
			{
				addItem("Pickup", target, null, true);
				Log.addLogItem("PickUp", getName(null) + " -> receive ->", target.getName(null), target.getCount(), target.getEnchantLevel(), null);
				if (target.isEtcItem())
				{
	...
	public void checkCombineTalisman(ItemInstance newItem, boolean sendMessage)
	{
		...
		if (newItem != null || sendMessage)
		{
			...
			for (final var item : _inventory.getItems())
			{
			...
			for (final var idCount : _sameIds)
			{
				...
				{
					var lifeTime = 0;
					final var existingTalismans = _inventory.getItemsByItemId(idCount[0]);
					for (final var existingTalisman : existingTalismans)
					{
						lifeTime += existingTalisman.getMana();
						_inventory.destroyItem("Combine Talismans", existingTalisman, this, null);
					}
	...
	private boolean isValidPrivateStoreZone(boolean isTrade, boolean isSellBuff)
	{
		...
		if (World.getAroundTraders(this, 80, 200) > 0 && inNone)
		{
	...
	public void transform(Transform transformation)
	{
		...
		if (template != null)
		{
			...
			for (final var effect : getAllEffects())
			{
				if (effect != null)
				{
	...
	public void untransform()
	{
		if (_transformation != null)
		{
			...
			if (skill != null)
			{
				stopSkillEffects(skill.getId(), true);
			}
			sendSkillList(true);
			broadcastUserInfo(true);
		}
	...
	{
		final var weapon = getActiveWeaponInstance();
		if (weapon == null)
		{
	...
	public boolean isWearingHeavyArmor()
	{
		...
		if (armor != null)
		{
			if (((_inventory.getPaperdollItem(Inventory.PAPERDOLL_CHEST).getItem().getBodyPart() == Item.SLOT_FULL_ARMOR) && ((ArmorType) armor.getItemType() == ArmorType.HEAVY)))
			{
				return true;
			}
		}
	...
	public boolean isWearingLightArmor()
	{
		...
		if (armor != null)
		{
			if (((_inventory.getPaperdollItem(Inventory.PAPERDOLL_CHEST).getItem().getBodyPart() == Item.SLOT_FULL_ARMOR) && ((ArmorType) armor.getItemType() == ArmorType.LIGHT)))
			{
				return true;
			}
		}
	...
	public boolean isWearingMagicArmor()
	{
		...
		if (armor != null)
		{
			if (((_inventory.getPaperdollItem(Inventory.PAPERDOLL_CHEST).getItem().getBodyPart() == Item.SLOT_FULL_ARMOR) && ((ArmorType) armor.getItemType() == ArmorType.MAGIC)))
			{
				return true;
			}
		}
	...
	protected void onDeath(Creature killer)
	{
			...
			else if (isCombatFlagEquipped() && !isInFightEvent())
			{
				...
				else
				{
					...
					else
					{
						...
						_inventory.unEquipItemInBodySlot(slot, true);
		...
		{
			getFarmSystem().checkTargetForRessurect(killer);
		}
	...
	private void onDieDropItem(Creature killer)
	{
		...
		if ((!isInsideZone(ZoneId.PVP) || (pk == null)) && (!isGM() || Config.KARMA_DROP_GM))
		{
			...
			if ((dropPercent > 0) && (Rnd.get(100) < dropPercent))
			{
				...
				for (final ItemInstance itemDrop : _inventory.getItems())
				{
					...
					if (itemDrop.isEquipped())
					{
						...
						_inventory.unEquipItemInSlot(itemDrop.getLocationSlot(), true);
					}
					else
					{
					...
					if (Rnd.get(100) < itemDropPercent)
					{
						...
						Log.addLogItem("DieDrop", getName(null) + " -> drop ->", itemDrop.getName(null), itemDrop.getCount(), itemDrop.getEnchantLevel(), null);
	...
	{
		final var requester = _activeRequester;
	...
	public boolean isProgressTransaction()
	{
		return _activeRequester != null && _activeTradeList != null && _requestExpireTime > System.currentTimeMillis();
	}
	...
	{
		final var inventory = _inventory;
		if (inventory != null)
		{
	...
	protected boolean checkAndEquipArrows()
	{
		...
		if (inventory != null)
		{
			if (inventory.getPaperdollItem(Inventory.PAPERDOLL_LHAND) == null)
			{
				...
				if (_arrowItem != null)
				{
					inventory.setPaperdollItem(Inventory.PAPERDOLL_LHAND, _arrowItem, true);
					if (!Config.FORCE_INVENTORY_UPDATE)
					{
						final InventoryUpdate iu = new InventoryUpdate();
	...
	protected boolean checkAndEquipBolts()
	{
		...
		if (inventory != null)
		{
			if (inventory.getPaperdollItem(Inventory.PAPERDOLL_LHAND) == null)
			{
				...
				if (_boltItem != null)
				{
					inventory.setPaperdollItem(Inventory.PAPERDOLL_LHAND, _boltItem, true);
					if (!Config.FORCE_INVENTORY_UPDATE)
					{
						final InventoryUpdate iu = new InventoryUpdate();
	...
	public boolean disarmWeapons()
	{
		...
		final var unequiped = _inventory.unEquipItemInBodySlotAndRecord(wpn.getItem().getBodyPart(), true);
		final InventoryUpdate iu = new InventoryUpdate();
		...
		if (isFakePlayer())
		{
			ThreadPoolManager.getInstance().schedule(() -> _inventory.equipItem(wpn, true, true), 2000);
		}
		return true;
	}
	...
	public boolean disarmShield()
	{
		...
		if (sld != null)
		{
			...
			final var unequiped = _inventory.unEquipItemInBodySlotAndRecord(sld.getItem().getBodyPart(), true);
			final InventoryUpdate iu = new InventoryUpdate();
			for (final ItemInstance itm : unequiped)
			{
				iu.addModifiedItem(itm);
	...
	public boolean removeHenna(int slot)
	{
		...
		Log.addLogItem("RequestHennaUnequip", getName(null) + " -> destroy ->", "Adena", henna.getCancelFee(), 0, null);
		Log.addLogItem("RequestHennaUnequip", getName(null) + " -> return ->", Util.getItemName(henna.getDyeItemId()), henna.getCancelCount(), 0, null);
		final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S);
	...
	public boolean isAutoAttackable(Creature attacker, boolean isPoleAttack)
	{
		...
		if (isInFightEvent())
		{
			...
			for (final var e : getFightEvents())
			{
				if (e != null)
				{
					return e.canAttack(attacker, this) ? true : false;
				}
			}
		}
	...
	private boolean checkUseMagicConditions(Skill skill, boolean forceUse, boolean dontMove, boolean msg)
	{
		...
		if (skill.isToggle())
		{
			final var effect = getFirstEffect(skill.getId());
			if (effect != null)
			{
				effect.exit(true);
				sendActionFailed();
		...
		if (skill.isOffensive())
		{
			...
			if (isInOlympiadMode() && !isOlympiadStart())
			{
				if (target != null)
				{
					if ((target.isSummon() && getSummon() != target) || (target.isPlayer() && target.getActingPlayer() != this))
					{
						sendActionFailed();
						return false;
					}
	...
	public boolean checkPvpSkill(GameObject target, Skill skill, boolean srcIsSummon)
	{
		...
		if (!(target instanceof EventChestInstance) && (targetPlayer != null) && (target != this) && !(isInDuel() && (targetPlayer.getDuelId() == getDuelId())) && !isInsideZone(ZoneId.PVP) && !targetPlayer.isInsideZone(ZoneId.PVP))
		{
			...
			if (((skilldat != null) && !skilldat.isCtrlPressed() && skill.isOffensive() && !srcIsSummon) || ((skilldatpet != null) && !skilldatpet.isCtrlPressed() && skill.isOffensive() && srcIsSummon))
			{
				...
				if (getPvpFlag() == 0 && skill.getTargetType() == TargetType.AURA && !skill.isDebuff())
				{
					return false;
				}
			}
	...
	{
		boolean found = false;
		for (final var effect : getEffectList().getAllEffects())
		{
			if ((effect != null) && !effect.getSkill().isStayOnSubclassChange())
			{
				effect.exit(true, true, false);
				found = true;
			}
		}
	...
	public final void stopCubicsByOthers()
	{
		if (!_cubics.isEmpty())
		{
			...
			for (final var cubic : _cubics.values())
			{
				final var valid = Config.ALLOW_STOP_ALL_CUBICS || (!Config.ALLOW_STOP_ALL_CUBICS && cubic.isGivenByOther());
				if (valid)
				{
					cubic.deactivate(false);
					broadcast = true;
	...
	public void enterObserverMode(int x, int y, int z)
	{
		...
		stopEffects(EffectType.HIDE, true);
	...
	{
		enterObserverMode(loc.getX(), loc.getY(), loc.getZ());
	}
	...
	{
		if (isInHideStatus() || isFakePlayer())
		{
			return;
		}
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		try
		{
			...
			for (final ItemInstance item : _inventory.getAugmentedItems())
			{
			...
			_charges = 0;
			getPersonalTasks().removeTasks(true, 5, 16);
	...
	{
		super.doRevive();
		stopEffects(EffectType.CHARMOFCOURAGE, true);
		updateEffectIcons();
		sendPacket(new EtcStatusUpdate(this));
		if (isMounted())
		{
	...
	public void reviveAnswer(int answer)
	{
		...
		{
			stopEffects(EffectType.PHOENIX_BLESSING, false);
			stopAllEffectsExceptThoseThatLastThroughDeath();
		}
	...
	public final void sendDamageMessage(Creature target, int damage, Skill skill, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (target.isCrono())
		{
			damage = target.getCronoDamageLimit(this);
		}
	...
	public void checkItemRestriction(boolean sendInfo)
	{
		final Set<ItemInstance> items = new HashSet<>();
		final InventoryUpdate iu = new InventoryUpdate();
		for (int i = 0; i < Inventory.PAPERDOLL_TOTALSLOTS; i++)
		{
			...
			if (equippedItem != null)
			{
				...
				{
					items.add(_inventory.unEquipItemInSlot(i, false));
	...
	public boolean decreaseCharges(int count)
	{
		...
		if (_charges == 0)
		{
			if (getPersonalTasks().isActiveTask(5))
			{
	...
	public void teleportBookmarkGo(int id)
	{
		...
		else if (_inventory.getInventoryItemCount(13302, 0) > 0)
		{
			itemId = 13302;
		}
		else if (_inventory.getInventoryItemCount(20025, 0) > 0)
		{
			itemId = 20025;
		}
		else
	...
	{
		return _lang != null && !_lang.isEmpty() && _lang.equalsIgnoreCase("ru");
	}
	...
	{
		return (_inventory.getSize(includeQuestInv) < (getInventoryLimit() * (percent / 100.)));
	}
	...
	public boolean isPKProtected(final Player target)
	{
		...
		if (getStat().calcStat(Stats.PK_PROTECT, 0) > 0 && target.getPvpFlag() == 0 && target.getKarma() == 0)
		{
			return true;
		}
		return false;
	...
	public void setOfflineFarmMode(boolean val)
	{
		_offlineFarm = val;
	}
	
	public boolean isInOfflineFarmMode()
	{
		return _offlineFarm;
	}
	...
	public void startHpPotionTask()
	{
		...
		final ItemInstance hpPotion = _inventory.getItemByItemId(potionId);
		if (hpPotion == null)
		{
	...
	public void startMpPotionTask()
	{
		...
		final ItemInstance mpPotion = _inventory.getItemByItemId(potionId);
		if (mpPotion == null)
		{
	...
	public void startCpPotionTask()
	{
		...
		final ItemInstance cpPotion = _inventory.getItemByItemId(potionId);
		if (cpPotion == null)
		{
	...
	public void startSoulPotionTask()
	{
		...
		final ItemInstance soulPotion = _inventory.getItemByItemId(potionId);
		if (soulPotion == null)
		{
	...
	public void onUseAcp()
	{
		...
		{
			final ItemInstance hpPoint = _inventory.getItemByItemId(getVarInt("autoHpItemId", 0));
			if (hpPoint != null)
			{
		...
		{
			final ItemInstance mpPoint = _inventory.getItemByItemId(getVarInt("autoMpItemId", 0));
			if (mpPoint != null)
			{
		...
		{
			final ItemInstance cpPoint = _inventory.getItemByItemId(getVarInt("autoCpItemId", 0));
			if (cpPoint != null)
			{
		...
		{
			final ItemInstance soulPoint = _inventory.getItemByItemId(getVarInt("autoSoulItemId", 0));
			if (soulPoint != null)
			{
	...
	{
		return getParty() != null ? getParty().getCommandChannel() != null ? getParty().getCommandChannel() : getParty() : this;
	}
	...
	public void restoreTradeList()
	{
		...
		if (var != null)
		{
			...
			for (final String item : items)
			{
				...
				final ItemInstance itemToSell = _inventory.getItemByObjectId(oId);
				...
				getSellList().addItem(itemToSell.getObjectId(), count, price, false);
			}
		...
		if (var != null)
		{
			...
			for (final String item : items)
			{
				...
				final ItemInstance itemToSell = _inventory.getItemByItemId(Integer.parseInt(values[0]));
				if (itemToSell == null)
				{
					continue;
				}
	...
	public void removePremiumAccount(boolean isSwitch)
	{
		...
		if (template != null)
		{
			...
			for (final PremiumGift gift : template.getGifts())
			{
				...
				{
					if (_inventory.getItemByItemId(gift.getId()) != null)
					{
						_inventory.destroyItemByItemId(gift.getId(), gift.getCount(), "Remove Premium");
					}
	...
	public boolean hasHairForVisualEquipped(boolean sendSkillList)
	{
		...
		final ItemInstance item = _inventory.getPaperdollItem(Inventory.PAPERDOLL_HAIR2);
		if (item != null)
		{
		...
		final ItemInstance slot = _inventory.getPaperdollItem(paperdoll);
		if (slot == null)
		{
			switchHairSkills(false, sendSkillList);
			return false;
	...
	public boolean hasFaceForVisualEquipped(boolean sendSkillList)
	{
		...
		final ItemInstance item = _inventory.getPaperdollItem(Inventory.PAPERDOLL_HAIR);
		if (item != null)
		{
			final Item itemTemp = ItemsParser.getInstance().getTemplate(item.getId());
			if (itemTemp.getBodyPart() != template.getBodyPart())
	...
	{
		final ItemInstance chestItem = _inventory.getPaperdollItem(Inventory.PAPERDOLL_CHEST);
		final ItemInstance legsItem = _inventory.getPaperdollItem(Inventory.PAPERDOLL_LEGS);
		final ItemInstance glovesItem = _inventory.getPaperdollItem(Inventory.PAPERDOLL_GLOVES);
		final ItemInstance feetItem = _inventory.getPaperdollItem(Inventory.PAPERDOLL_FEET);
	...
	public boolean hasShieldForVisualEquipped(boolean sendSkillList)
	{
		...
		final ItemInstance shield = _inventory.getPaperdollItem(Inventory.PAPERDOLL_LHAND);
	...
	public boolean hasCloakForVisualEquipped(boolean sendSkillList)
	{
		...
		final ItemInstance cloak = _inventory.getPaperdollItem(Inventory.PAPERDOLL_CLOAK);
		if (cloak == null)
		{
	...
	private class BroadcastCharInfoAround implements Runnable
	{
		@Override
		public void run()
		{
	...
	public void sendPreviewUserInfo(int weapon, int shield, int gloves, int chest, int legs, int feet, int cloak, int hair, int mask)
	{
		...
		if ((_activeWeaponSkin != null) && hasWeaponForVisualEquipped(true) && weapon == 0)
		{
			...
			else
			{
				...
				_previewSlots[10] = _activeWeaponSkin.isAllowAugment() ? _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND) : 0;
			}
		}
		else
		{
			if (Config.ALLOW_VISUAL_ARMOR_COMMAND && weapon == 0)
			{
				...
				{
					final ItemInstance item = _inventory.getPaperdollItem(Inventory.PAPERDOLL_RHAND);
					if (item != null && item.getVisualItemId() > 0)
					{
						...
						_previewSlots[10] = wp != null && wp.isAllowAugment() ? _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND) : 0;
					}
					else
					{
						_previewSlots[0] = _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_RHAND);
						_previewSlots[9] = getEnchantEffect();
						_previewSlots[10] = _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND);
					}
				}
			}
			else
		...
		{
			_previewSlots[1] = shield == -1 ? 0 : shield > 0 ? shield : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_LHAND) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LHAND);
		}
		
		if (isCostumeBuff && chest == 0)
		{
			final DressArmorTemplate tpl = DressArmorParser.getInstance().getArmor(visualId);
			_previewSlots[2] = tpl != null ? tpl.getGloves() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_GLOVES);
			_previewSlots[3] = tpl != null ? tpl.getChest() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CHEST);
			_previewSlots[4] = tpl != null ? tpl.getLegs() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LEGS);
			_previewSlots[5] = tpl != null ? tpl.getFeet() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_FEET);
		}
		...
		{
			_previewSlots[2] = gloves == -1 ? 0 : gloves > 0 ? gloves : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_GLOVES) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_GLOVES);
			_previewSlots[3] = chest > 0 ? chest : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_CHEST) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CHEST);
			_previewSlots[4] = legs == -1 ? 0 : legs > 0 ? legs : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_LEGS) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LEGS);
			_previewSlots[5] = feet == -1 ? 0 : feet > 0 ? feet : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_FEET) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_FEET);
		}
		
		if (isCostumeBuff && (cloak == 0 || cloak == -1))
		{
			final DressArmorTemplate tpl = DressArmorParser.getInstance().getArmor(visualId);
			_previewSlots[6] = tpl != null ? tpl.getCloakId() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CLOAK);
		}
		...
		{
			_previewSlots[6] = cloak > 0 ? cloak : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_CLOAK) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CLOAK);
		}
		...
		{
			_previewSlots[7] = mask == -1 ? 0 : mask > 0 ? mask : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_HAIR) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_HAIR);
		}
		...
		{
			_previewSlots[8] = hair == -1 ? 0 : hair > 0 ? hair : Config.ALLOW_VISUAL_ARMOR_COMMAND ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_HAIR2) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_HAIR2);
		}
	...
	public void updateEffectIcons(boolean partyOnly)
	{
		...
		getEffectList().updateEffectIcons(partyOnly, true);
	}
	...
	public final void refreshInfos()
	{
		...
		try
		{
			targets = World.getAroundObjects(this);
			for (final GameObject object : targets)
			{
	...
	public void sendItemList(boolean show)
	{
		...
		sendPacket(new ItemList(_inventory, allSize - questItemsSize, items, show));
		sendPacket(new ExQuestItemList(_inventory, questItemsSize, items));
	...
	public int[] getUserVisualSlots()
	{
		...
		if ((_activeWeaponSkin != null) && hasWeaponForVisualEquipped(true) && !allowBlock)
		{
			...
			_userInfoSlots[10] = _activeWeaponSkin.isAllowAugment() ? _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND) : 0;
		}
		else
		{
			if (Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock)
			{
				...
				if (item != null && item.getVisualItemId() > 0)
				{
					...
					_userInfoSlots[10] = weapon != null && weapon.isAllowAugment() ? _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND) : 0;
				}
				else
				{
					_userInfoSlots[0] = _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_RHAND);
					_userInfoSlots[9] = getEnchantEffect();
					_userInfoSlots[10] = _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND);
				}
			}
			else
			{
		...
		{
			_userInfoSlots[1] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_LHAND) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LHAND);
		}
		
		if (isCostumeBuff && !allowBlock)
		{
			final DressArmorTemplate tpl = DressArmorParser.getInstance().getArmor(visualId);
			_userInfoSlots[2] = tpl != null ? tpl.getGloves() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_GLOVES);
			_userInfoSlots[3] = tpl != null ? tpl.getChest() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CHEST);
			_userInfoSlots[4] = tpl != null ? tpl.getLegs() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LEGS);
			_userInfoSlots[5] = tpl != null ? tpl.getFeet() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_FEET);
		}
		...
		{
			_userInfoSlots[2] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_GLOVES) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_GLOVES);
			_userInfoSlots[3] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_CHEST) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CHEST);
			_userInfoSlots[4] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_LEGS) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LEGS);
			_userInfoSlots[5] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_FEET) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_FEET);
		}
		
		if (isCostumeBuff && !allowBlock)
		{
			final DressArmorTemplate tpl = DressArmorParser.getInstance().getArmor(visualId);
			_userInfoSlots[6] = tpl != null ? tpl.getCloakId() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CLOAK);
		}
		...
		{
			_userInfoSlots[6] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_CLOAK) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CLOAK);
		}
		...
		{
			_userInfoSlots[7] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_HAIR) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_HAIR);
		}
		...
		{
			_userInfoSlots[8] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_HAIR2) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_HAIR2);
		}
	...
	public int[] getCharVisualSlots(Player viewer)
	{
		...
		if ((_activeWeaponSkin != null) && hasWeaponForVisualEquipped(true) && !allowBlock)
		{
			...
			_charInfoSlots[10] = _activeWeaponSkin.isAllowAugment() ? _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND) : 0;
		}
		else
		{
			if (Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock)
			{
				...
				if (item != null && item.getVisualItemId() > 0)
				{
					...
					_charInfoSlots[10] = weapon != null && weapon.isAllowAugment() ? _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND) : 0;
				}
				else
				{
					_charInfoSlots[0] = _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_RHAND);
					_charInfoSlots[9] = getEnchantEffect();
					_charInfoSlots[10] = _inventory.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND);
				}
			}
			else
			{
		...
		{
			_charInfoSlots[1] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_LHAND) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LHAND);
		}
		
		if (isCostumeBuff && !allowBlock)
		{
			final DressArmorTemplate tpl = DressArmorParser.getInstance().getArmor(visualId);
			_charInfoSlots[2] = tpl != null ? tpl.getGloves() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_GLOVES);
			_charInfoSlots[3] = tpl != null ? tpl.getChest() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CHEST);
			_charInfoSlots[4] = tpl != null ? tpl.getLegs() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LEGS);
			_charInfoSlots[5] = tpl != null ? tpl.getFeet() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_FEET);
		}
		else if ((_activeArmorSkin != null) && hasArmorForVisualEquipped(true) && !allowBlock)
		{
		...
		{
			_charInfoSlots[2] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_GLOVES) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_GLOVES);
			_charInfoSlots[3] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_CHEST) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CHEST);
			_charInfoSlots[4] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_LEGS) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_LEGS);
			_charInfoSlots[5] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_FEET) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_FEET);
		}
		
		if (isCostumeBuff && !allowBlock)
		{
			final DressArmorTemplate tpl = DressArmorParser.getInstance().getArmor(visualId);
			_charInfoSlots[6] = tpl != null ? tpl.getCloakId() : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CLOAK);
		}
		if ((_activeCloakSkin != null) && hasCloakForVisualEquipped(true) && !allowBlock)
		{
		...
		{
			_charInfoSlots[6] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_CLOAK) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_CLOAK);
		}
		...
		{
			_charInfoSlots[7] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_HAIR) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_HAIR);
		}
		...
		{
			_charInfoSlots[8] = Config.ALLOW_VISUAL_ARMOR_COMMAND && !allowBlock ? _inventory.getPaperdollVisualItemId(Inventory.PAPERDOLL_HAIR2) : _inventory.getPaperdollItemDisplayId(Inventory.PAPERDOLL_HAIR2);
		}
	...
	public void prepareToLogout()
	{
		...
		if (isCombatFlagEquipped() && !isInFightEvent())
		{
			if (!TerritoryWarManager.getInstance().dropCombatFlag(this, true, false))
			{
				...
				{
					final var flag = getInventory().getItemByItemId(9819);
					if (flag != null)
					{
						final var slot = getInventory().getSlotFromItem(flag);
						getInventory().unEquipItemInBodySlot(slot, true);
	...
	public void finishToLogout()
	{
		...
		for (final ItemInstance item : _inventory.getItems())
		{
			...
			{
				_inventory.destroyItemByObjectId(item.getObjectId(), item.getCount(), this, null);
			}
		}
	...
	public void setInOfflineFarmMode(boolean closeClient)
	{
		if (isFakePlayer())
		{
			return;
		...
		if (_client != null && canOfflineFarmMode())
		{
			setOfflineFarmMode(true);
			if (closeClient)
			{
				_client.close(LogOutOk.STATIC_PACKET);
			}
			...
			if (clan != null)
			{
				final var clanMember = clan.getClanMember(getObjectId());
				if (clanMember != null)
				{
					clanMember.setPlayerInstance(null);
				}
			...
			final var room = getMatchingRoom();
			if (room != null)
			{
				if (room.getLeader() == this)
				{
			...
			setMatchingRoom(null);
			MatchingRoomManager.getInstance().removeFromWaitingList(this);
			setVar("offlineFarm", String.valueOf(System.currentTimeMillis() / 1000L), -1);
			var info = "";
			for (final int itemId : _activeSoulShots)
			{
				info += "" + itemId + ";";
			}
			setVar("offlineFarmShots", info, -1);
			Log.addLogGame("OFFLINEFARM:", "Start OfflineFarm!", getName(null));
		}
		else
		{
			scheduleDelete();
		...
		try
		{
			switch (bypassType)
			{
				...
				case ENCODED :
					...
					break;
				case ENCODED_TUTORIAL :
					bypass = getBypassStorage().decodeTutorialBypass(bypass.substring(1));
					if (bypass == null)
					{
						return null;
					}
	...
	public int getBattlefieldPenalty()
	{
		return getFirstEffect(5660) != null ? 0x00 : 0x01;
	}
	...
	@Override
	public boolean isInHideStatus()
	{
		return _entering || isLogoutStarted();
	}
	
	public boolean canSelfResurrection()
	{
		return _canSelfResurrection;
	}
	
	public void setCanSelfResurrection(boolean val)
	{
		_canSelfResurrection = val;
	}
	
	public void setStartLocWASD(Location loc)
	{
		_startLocWASD = loc;
	}
	
	@Override
	public Location getStartLocWASD()
	{
		return _startLocWASD;
	}
