 ...
 */
package gameserver.model.actor.transform;
...
import gameserver.model.holders.AdditionalItemHolder;
import gameserver.model.holders.AdditionalSkillHolder;
...
public final class Transform implements IIdentifiable
{
	...
	{
		final var template = getTemplate(player);
		if (template != null)
		{
			boolean found = false;
			if (!template.getAdditionalSkills().isEmpty())
			{
				for (final var holder : template.getAdditionalSkills())
				{
					if (player.getLevel() >= holder.getMinLevel())
					{
						if (Config.ALLOW_MUTIPROFF_SYSTEM && SkillTreesParser.getInstance().isAllowMultiSkill(holder.getId()))
						{
							if (player.getKnownSkill(holder.getId()) == null)
							{
								player.addSkill(holder.getSkill(), false);
								player.addTransformSkill(holder.getId());
								player.updateShortCuts(holder.getId(), holder.getLvl());
								found = true;
							}
						}
						else
