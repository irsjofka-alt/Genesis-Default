...
public class PreparedListContainer extends ListContainer
{
	...
	public PreparedListContainer(ListContainer template, boolean inventoryOnly, Player player, Npc npc)
	{
		...
		_isSkillProducts = template.isSkillProducts();
		...
		else if (_isSkillProducts)
		{
			_entries = new LinkedList<>();
			for (final var ent : template.getEntries())
			{
				for (final var product : ent.getProducts())
				{
					if (product != null && product.getSkillId() > 0)
					{
						final var sk = player.getKnownSkill(product.getSkillId());
						if (sk != null)
						{
							if (sk.getLevel() == (product.getSkillLevel() - 1))
							{
								_entries.add(new PreparedEntry(ent, null, _applyTaxes, false, taxRate));
							}
						}
						else if (product.getSkillLevel() == 1)
						{
							_entries.add(new PreparedEntry(ent, null, _applyTaxes, false, taxRate));
						}
					}
