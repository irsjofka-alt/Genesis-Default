 ...
 */
package gameserver.model.items.instance;
...
import gameserver.model.DropProtection;
...
import gameserver.model.items.type.ItemType;
...
import gameserver.model.skills.options.EnchantOptions;
import gameserver.model.skills.options.Options;
import gameserver.network.SystemMessageId;
import gameserver.network.serverpackets.DropItem;
import gameserver.network.serverpackets.GetItem;
import gameserver.network.serverpackets.InventoryUpdate;
import gameserver.network.serverpackets.NpcHtmlMessage;
import gameserver.network.serverpackets.SpawnItem;
...
public final class ItemInstance extends GameObject
{
	...
	public void changeCount(String process, long count, Player creator, Object reference)
	{
		...
		final long max = getId() == ADENA_ID ? MAX_ADENA : Long.MAX_VALUE;
	...
	{
		if (!_isEventItem && (getItemLocation() == ItemLocation.INVENTORY) || (getItemLocation() == ItemLocation.PAPERDOLL))
		{
	...
	public boolean isAvailableSell(Player player, boolean allowAdena)
	{
		return ((!isEquipped()) && (getItem().getType2() != Item.TYPE2_QUEST) && ((getItem().getType2() != Item.TYPE2_MONEY) || (getItem().getType1() != Item.TYPE1_SHIELD_ARMOR)) && (!player.hasSummon() || (getObjectId() != player.getSummon().getControlObjectId())) && (player.getActiveEnchantItemId() != getObjectId()) && (player.getActiveEnchantSupportItemId() != getObjectId()) && (player.getActiveEnchantAttrItemId() != getObjectId()) && (allowAdena || (getId() != PcInventory.ADENA_ID)) && ((player.getCurrentSkill() == null) || (player.getCurrentSkill().getSkill().getItemConsumeId() != getId())) && (!player.isCastingSimultaneouslyNow() || player.getLastSimultaneousSkillCast() == null || (player.getLastSimultaneousSkillCast().getItemConsumeId() != getId())) && (isSellable() && (!((getItem().getItemType() == EtcItemType.PET_COLLAR) && player.havePetInvItems()))));
	}

	public boolean isAvailable(Player player, boolean allowAdena, boolean allowNonTradeable)
	{
		return ((!isEquipped()) && (getItem().getType2() != Item.TYPE2_QUEST) && ((getItem().getType2() != Item.TYPE2_MONEY) || (getItem().getType1() != Item.TYPE1_SHIELD_ARMOR)) && (!player.hasSummon() || (getObjectId() != player.getSummon().getControlObjectId())) && (player.getActiveEnchantItemId() != getObjectId()) && (player.getActiveEnchantSupportItemId() != getObjectId()) && (player.getActiveEnchantAttrItemId() != getObjectId()) && (allowAdena || (getId() != PcInventory.ADENA_ID)) && ((player.getCurrentSkill() == null) || (player.getCurrentSkill().getSkill().getItemConsumeId() != getId())) && (!player.isCastingSimultaneouslyNow() || player.getLastSimultaneousSkillCast() == null || (player.getLastSimultaneousSkillCast().getItemConsumeId() != getId())) && (allowNonTradeable || (isTradeable() && (!((getItem().getItemType() == EtcItemType.PET_COLLAR) && player.havePetInvItems())))));
	}
	...
	public void setElementAttr(byte element, int value, boolean updateDatabase)
	{
	...
	public void decreaseMana(boolean resetConsumingMana, int count)
	{
		...
		if (player != null)
		{
			...
			if (_mana == 0)
			{
				...
				if (isEquipped())
				{
					final var unequiped = player.getInventory().unEquipItemInSlotAndRecord(getLocationSlot(), true);
	...
	public boolean giveSkillsToOwner(boolean updateSkills)
	{
		if (!hasPassiveSkills())
		{
			return false;
		...
		if (command.startsWith("Quest"))
		{
			...
			if (q != null)
			{
				...
				if (qs == null)
				{
					...
					{
						if (activeChar.getAllActiveQuests().size() > 40)
						{
