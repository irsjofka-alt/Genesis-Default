...
public class PcInventory extends Inventory
{
	...
	public ItemInstance[] getUniqueItems(boolean allowAdena, boolean allowAncientAdena, boolean onlyAvailable)
	{
		...
		for (final ItemInstance item : _items)
		{
			...
			if (!isDuplicate && (!onlyAvailable || ((item != null) && item.isSellable() && item.isAvailableSell(getOwner(), false))))
			{
				list.add(item);
			}
		}
	...
	public ItemInstance[] getAvailableSellItems(boolean allowAdena, boolean feightable)
	{
		final List<ItemInstance> list = new LinkedList<>();
		for (final ItemInstance item : _items)
		{
			if ((item == null) || !item.isAvailableSell(getOwner(), allowAdena) || !canManipulateWithItemId(item.getId()))
			{
				continue;
			}
			else if (feightable)
	...
	public TradeItem[] getAvailableSellItems(TradeList tradeList)
	{
		final List<TradeItem> list = new LinkedList<>();
		for (final var item : _items)
		{
			if ((item != null) && item.isAvailableSell(getOwner(), false))
			{
				final var adjItem = tradeList.adjustAvailableItem(item);
				if (adjItem != null)
				{
					boolean found = false;
					if (!adjItem.getItem().isStackable())
					{
						for (final var temp : getOwner().getSellList().getItems())
						{
	...
	public void applyItemSkills()
	{
		...
		for (final var item : _items)
		{
			if (item != null)
			{
				if (item.giveSkillsToOwner(false))
				{
					isUpdateSkills = true;
				}
			}
	...
	public boolean reduceShortsCount(ItemInstance item, int count)
	{
		...
		return updateItemCountNoDbUpdate("Shorts", item, -count, _owner, null);
	}
	...
	public boolean reduceItemsCount(String process, ItemInstance item, int count)
	{
		...
		return updateItemCountNoDbUpdate(process, item, -count, _owner, null);
	}
	...
	public final ItemInstance getSelfResurrection()
	{
		if (!_owner.canSelfResurrection())
		{
			return null;
		}
