...
{
	private static final Logger _log = LogManager.getLogger(ChanceCondition.class);
	...
	public static ChanceCondition parse(StatsSet set)
	{
		try
		{
			final var trigger = set.getEnum("chanceType", TriggerType.class, null);
	...
	public static ChanceCondition parse(String chanceType, int chance, int mindmg, String elements, String activationSkills, boolean pvpOnly)
	{
		try
		{
			...
			final var trigger = Enum.valueOf(TriggerType.class, chanceType);
			if (trigger != null)
			{
				return new ChanceCondition(trigger, chance, mindmg, parseElements(elements), parseActivationSkills(activationSkills), pvpOnly);
			}
	...
	{
		if ((_pvpOnly && !playable) || !_triggerType.check(event))
		{
			return false;
		}
		...
		if (_mindmg > -1 && damage < _mindmg)
		{
			return false;
		}
		return _chance < 0 || Rnd.chance(_chance);
	}
