...
public class RewardData implements Cloneable
{
	...
	protected RewardItem rollAdena(Player player, double mod, double rate, double premiumBonus)
	{
		...
		if (mod > 0 && rate > 0)
		{
			...
			{
				final var t = new RewardItem(_item.getId());
	...
	public static RewardData parseReward(NamedNodeMap rewardElement, RewardType type)
	{
		...
		double chance_dop = Math.min((chance * Config.RATE_CHANCE_DROP_ITEMS), 1000000D);
		double chance_h = Math.min((chance * Config.RATE_CHANCE_DROP_HERBS), 1000000D);
		double chance_sp = Math.min((chance * Config.RATE_CHANCE_SPOIL), 1000000D);
		double chance_weapon = Math.min((chance * Config.RATE_CHANCE_DROP_WEAPON_ARMOR_ACCESSORY), 1000000D);
		double chance_weapon_sp = Math.min((chance * Config.RATE_CHANCE_SPOIL_WEAPON_ARMOR_ACCESSORY), 1000000D);
		double chance_epolet = Math.min((chance * Config.RATE_CHANCE_DROP_EPOLET), 1000000D);
		double chance_att = Math.min((chance * Config.RATE_CHANCE_ATTRIBUTE), 1000000D);
		double chance_common = Math.min((chance * Config.RATE_CHANCE_COMMON), 1000000D);
		
		final var data = new RewardData(itemId);
		...
		{
			if (data.getItem().isCommon())
			{
				data.setChance(chance_common);
			}
