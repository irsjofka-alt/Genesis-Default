 ...
 */
package gameserver.model.service.autoenchant;
	...
	public void run()
	{
		...
		try
		{
			for (int i = 0; i < _player.getEnchantParams().upgradeItemLimit && checkAttributeLvl(_player); i++)
			{
				...
				if (!itemToEnchant.isEnchantable())
				{
					_player.sendPacket(ActionFail.STATIC_PACKET, SystemMessageId.INAPPROPRIATE_ENCHANT_CONDITION);
                    return;
                }
				...
				if (boolka)
				{
					...
					{
						_player.getInventory().unEquipItem(itemToEnchant, true);
                        isNeedEquip = true;
                    }
					itemToEnchant.setElementAttr(element, itemToEnchant.getAttributeElementValue(element) + value, true);
                }
		...
		finally
		{
			...
			if (isNeedUpdate)
			{
                ...
                if (isNeedEquip)
				{
					_player.getInventory().equipItem(itemToEnchant, true, true);
				}
            }
        }
    }
