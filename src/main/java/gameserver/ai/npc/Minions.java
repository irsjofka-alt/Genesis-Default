...
import gameserver.model.MobGroup;
import gameserver.model.MobGroupData;
...
import gameserver.model.actor.instance.ControllableMobInstance;
...
public class Minions extends DefaultAI
{
	...
	protected void thinkCast()
	{
		...
		{
			npc.setTarget(findNextRndTarget());
		...
		if (!_actor.isMuted())
		{
			...
			{
				final var mpConsume = _actor.getStat().getMpConsume(sk);
				if (Util.checkIfInRange(sk.getCastRange(), _actor, target, true) && !_actor.isSkillDisabled(sk) && (_actor.getCurrentMp() > mpConsume))
				{
					_actor.doCast(sk);
					return;
				}
	...
	protected void thinkAttack()
	{
		...
		if ((target == null) || target.isAlikeDead())
		{
			if (target != null)
			{
				me.getAggroList().stopHating(target);
			}
			setIntention(ACTIVE);
		}
		else
		{
			if (!me.getFaction().isNone())
			{
				List<Npc> targets = null;
				try
				{
					targets = World.getAroundNpc(me, 2000, 200);
					for (final var npc : targets)
					{
						if (!npc.getFaction().isNone() && !me.isInFaction(npc))
						{
							continue;
						}
						
						if (me.isInsideRadius(npc, npc.getFaction().getRange(), false, true) && (Math.abs(target.getZ() - npc.getZ()) < 200))
						{
			...
			me.setTarget(target);
			final var dist2 = me.getPlanDistanceSq(target.getX(), target.getY());
			final var range = (int) (me.getPhysicalAttackRange() + me.getColRadius() + target.getColRadius());
			int max_range = range;

			if (!me.isMuted() && (dist2 > ((range + 20) * (range + 20))))
			{
				for (final var sk : me.getAllSkills())
				{
					final var castRange = sk.getCastRange();
					final var mpConsume = me.getStat().getMpConsume(sk);
					if (((castRange * castRange) >= dist2) && !me.isSkillDisabled(sk) && (me.getCurrentMp() > mpConsume))
					{
						me.doCast(sk);
						return;
					}
