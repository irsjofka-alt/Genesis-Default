...
public class EffectPointAI extends CharacterAI implements Runnable
{
	...
	protected boolean thinkActive()
	{
		...
		List<Creature> characters = null;
		try
		{
			characters = World.getAroundCharacters(actor, actor.getMainSkill().getAffectRange(), 300);
			for (final var cha : characters)
			{
				if (cha == null || cha.isAlikeDead() || cha.isDoor() || cha.isEffectPoint() || ((cha == owner) && skill.isOffensive()) || (!skill.isOffensive() && !cha.isPlayable()))
				{
					continue;
				}
		...
		if (!targets.isEmpty())
		{
			...
			else
			{
				...
				for (final var target : targets)
				{
					...
					final int mdam = (int) Formulas.calcMagicDam(owner, target, actor.getMainSkill(), shld, sps, bss, mcrit, 0);
					...
					if (ai != null)
					{
						ai.notifyEvent(CtrlEvent.EVT_ATTACKED, owner, mdam);
					}
				}
