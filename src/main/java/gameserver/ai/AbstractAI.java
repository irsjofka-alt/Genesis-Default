...
import gameserver.ai.model.Ctrl;
...
import gameserver.network.serverpackets.MoveToPawn;
...
public abstract class AbstractAI extends LoggerObject implements Ctrl
{
	...
	public void setIsShiftClick(boolean value)
	{
		_shift = value;
	}
	...
	public final void setIntention(CtrlIntention intention, Object... args)
	{
		...
		switch (intention)
		{
			...
			case REST :
				...
				break;
			case ATTACK :
				onIntentionAttack((Creature) args[0], args.length > 1 ? (boolean) args[1] : false);
				break;
			case CAST :
				onIntentionCast((Skill) args[0], (GameObject) args[1]);
				break;
			case MOVING :
				onIntentionMoveTo((Location) args[0], args.length > 1 ? (int) args[1] : 0);
				break;
			case FOLLOW :
				onIntentionFollow((Creature) args[0]);
				break;
			case PICK_UP :
				onIntentionPickUp((GameObject) args[0]);
				break;
			case INTERACT :
				onIntentionInteract((GameObject) args[0]);
				break;
		}
	}
	...
	protected void moveToPawn(GameObject pawn, int offset)
	{
		if (!_actor.isMovementDisabled())
		{
			...
			_actor.moveToLocation(pawn.getX(), pawn.getY(), pawn.getZ(), offset, false);
	...
	protected void moveTo(Location loc, int offset)
	{
		if (!_actor.isMovementDisabled())
		{
			_clientMoving = true;
			_clientMovingToPawnOffset = 0;
			_actor.moveToLocation(loc.getX(), loc.getY(), loc.getZ(), offset, loc.isWasdMove());
			if (!_actor.isMoving())
			{
				if (offset > 0)
				{
					_actor.moveToLocation(loc.getX(), loc.getY(), loc.getZ(), 0, loc.isWasdMove());
				}
