...
public class DoorAI extends CharacterAI
{
	...
	{
		if (System.currentTimeMillis() > _lastActiveCheck)
		{
			_lastActiveCheck = System.currentTimeMillis() + 10000L;
			final var me = (DoorInstance) _actor;
			List<DefenderInstance> targets = null;
			try
			{
				targets = me.getKnownDefenders();
				for (final var guard : targets)
				{
					if (_actor.isInsideRadius(guard, guard.getFaction().getRange(), false, true) && (Math.abs(attacker.getZ() - guard.getZ()) < 200))
					{
						if (Rnd.chance(20))
						{
							guard.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, attacker, 10000);
						}
						else
						{
							guard.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, attacker, 2000);
						}
					}
				}
			}
