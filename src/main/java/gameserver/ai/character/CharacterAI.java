...
import static gameserver.ai.model.CtrlIntention.FOLLOW;
...
import gameserver.ai.AbstractAI;
...
public class CharacterAI extends AbstractAI
{
	...
	{
		if ((_actor.isSitting() && skill.isMagic()) || _actor.isAfraid())
		{
			clientActionFailed();
		...
		{
			ThreadPoolManager.getInstance().schedule(new CastTask(_actor, skill, target), normalAttackDelay);
			return;
		}
	...
	protected void onEvtArrived()
	{
		...
		if (_actor.isPlayer() && (_actor.getAI().getIntention() == CtrlIntention.CAST) && target != null && target.isDoor())
		{
			int x = 0, y = 0, z = 0;
			if (target.isDoor())
			{
	...
	protected boolean checkTargetLostOrDead(Creature target)
	{
		...
		{
			if (target != null && target.isPlayer() && target.isFakeDeathNow())
			{
	...
	protected boolean checkTargetLost(GameObject target)
	{
		...
		if ((_actor != null) && (_skill != null) && _skill.isOffensive() && (_skill.getAffectRange() > 0) && !GeoEngine.getInstance().canSeeTarget(_actor, target))
		{
			setIntention(ACTIVE);
			return true;
		}
	...
	public boolean canAura(Skill sk)
	{
		...
		{
			final var select = _actor.getTarget();
			List<Creature> targets = null;
			try
			{
				targets = World.getAroundCharacters(_actor, sk.getAffectRange(), 200);
				...
				try
				{
					...
					for (final var target : targets)
					{
						if (target != null)
						{
							...
							if (target.getAllEffects().length > 0)
							{
								cancast = true;
								break;
							}
		...
		if (sk.getTargetType() == TargetType.PARTY || sk.getTargetType() == TargetType.CLAN)
		{
			...
			try
			{
				...
				for (final var target : targets)
				{
					...
					final var tgt = ((Npc) target);
					final var actors = ((Npc) _actor);
					if ((!actors.getFaction().isNone() && actors.isInFaction((tgt)) || minions != null && minions.hasNpcId(tgt.getId())))
					{
						count++;
						final var effects = target.getAllEffects();
						for (int i = 0; (effects != null) && (i < effects.length); i++)
