...
import gameserver.data.holder.SecPasswordHolder;
import gameserver.database.DatabaseFactory;
import gameserver.model.CharSelectInfoPackage;
...
import gameserver.network.communication.gameserverpackets.PlayerLogout;
import gameserver.network.serverpackets.GameServerPacket;
import gameserver.utils.FloodProtectorAction;
import gameserver.utils.FloodProtectorConfig;
import gameserver.utils.Util;
import gameserver.utils.strixplatform.StrixPlatform;
import gameserver.utils.strixplatform.network.cipher.StrixGameCrypt;
...
{
	protected static final Logger _log = LogManager.getLogger(GameClient.class);
	...
	public Player loadCharFromDisk(int charslot)
	{
		...
		if (oldPlayer != null)
		{
			if (oldPlayer.isInOfflineMode() || oldPlayer.isInOfflineFarmMode() || oldPlayer.isLogoutStarted())
			{
				oldPlayer.setOfflineMode(false);
				oldPlayer.setOfflineFarmMode(false);
				oldPlayer.kick();
			}
			else
			{
	...
	protected void onDisconnection()
	{
		...
		if (player != null && (player.isInOfflineMode() || player.isInOfflineFarmMode()))
		{
			return;
		}
		...
		if (player != null)
		{
			player.getListeners().onDisconnect(true);
	...
	{
		if (Config.PROTECTION.equalsIgnoreCase("ANTICHEAT") && hwid != null && hwid.length() == 64)
		{
			_hwid = hwid.substring(0, 16) + hwid.substring(32, 64);
		}
		else
		{
			_hwid = hwid;
	...
	{
		return _realIpAddress != null && !_realIpAddress.isEmpty() ? _realIpAddress : getConnectionAddress().getHostAddress();
	}
