...
public class RequestMultiSellChoose extends GameClientPacket
{
	...
	public void runImpl()
	{
		...
		for (final Entry entry : list.getEntries())
		{
			if (entry.getEntryId() == _entryId)
			{
				...
				for (final Ingredient e : entry.getProducts())
				{
					if (e.getSkillId() > 0)
					{
						final int charLevel = e.getCharLevel();
						if (charLevel > 0 && player.getLevel() < charLevel)
						{
							final var msg = new ServerMessage("Multisell.WRONG_LEVEL", player.getLang());
							msg.add(charLevel);
					...
					if (e.getId() < 0 || e.getId() == Config.SP_ID || e.getId() == Config.EXP_ID || entry.isSkillProducts())
					{
						continue;
					}
					...
					else
					{
						...
						if (Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient())
						{
							if (itemToTake.isStackable())
							{
								...
								Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> destroy ->", itemToTake.getName(null), (e.getCount() * _amount), 0, "Multisell[id:" + _listId + ":" + _entryId + "]");
							}
							else
							{
								if (list.getMaintainEnchantment())
								{
									...
									for (int i = 0; i < (e.getCount() * _amount); i++)
									{
										...
										Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> destroy ->", inventoryContents[i].getName(null), 1, inventoryContents[i].getEnchantLevel(), "Multisell[id:" + _listId + ":" + _entryId + "]");
									}
								}
								else
								{
									for (int i = 1; i <= (e.getCount() * _amount); i++)
									{
										...
										Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> destroy ->", itemToTake.getName(null), 1, itemToTake.getEnchantLevel(), "Multisell[id:" + _listId + ":" + _entryId + "]");
									}
								}
							}
						}
				...
				for (final Ingredient e : entry.getProducts())
				{
					...
					else if (e.getSkillId() > 0)
					{
						final int skillId = e.getSkillId();
						final int skillLevel = e.getSkillLevel();
						final var sk = player.getKnownSkill(skillId);
						boolean isValid = false;
						if (sk != null)
						{
							if (sk.getLevel() == (skillLevel - 1))
							{
								isValid = true;
							}
						}
						...
						if (isValid)
						{
							final var s = SkillsParser.getInstance().getInfo(skillId, skillLevel);
							if (s != null)
							{
								player.addSkill(s, true);
								final var msg = new ServerMessage("Multisell.ADD_SKILL", player.getLang());
								msg.add(s.getName(player.getLang()));
								msg.add(skillLevel);
								player.sendMessage(msg.toString());
					...
					else
					{
						...
						if (e.isStackable())
						{
							product = inv.addItem("Multisell", e.getId(), e.getCount() * _amount, player, false);
							if (product.getCount() > 1)
							{
								final var sm = SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S);
								sm.addItemName(product);
								sm.addLong(e.getCount() * _amount);
								player.sendPacket(sm);
							}
							else
							{
								final var sm = SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1);
								sm.addItemName(product);
								player.sendPacket(sm);
							}
						}
						else
						{
							for (int i = 0; i < (e.getCount() * _amount); i++)
							{
								...
								if (product != null)
								{
									if (list.getMaintainEnchantment())
									{
										...
										if (elemental != null)
										{
											for (final var elm : elemental)
											{
												product.setElementAttr(elm.getElement(), elm.getValue(), false);
											}
											product.updateItemElementals();
										}
									...
									else
									{
										...
										{
											for (final var elm : e.getElementals())
											{
												product.setElementAttr(elm.getElement(), elm.getValue(), false);
											}
											product.updateItemElementals();
									...
									Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> buy ->", product.getName(null), product.getCount(), product.getEnchantLevel(), "Multisell[id:" + _listId + ":" + _entryId + "]");
								}
							}
							...
							else if (product.getEnchantLevel() > 0)
							{
								final var sm = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_S2);
								sm.addLong(product.getEnchantLevel());
								sm.addItemName(product);
								player.sendPacket(sm);
							}
							else
		...
		if (list.isSkillProducts())
		{
			MultiSellParser.getInstance().separateAndSend(_listId, player, target, false);
		}
		else if (list.getMaintainEnchantment() && list.isNpcOnly())
		{
