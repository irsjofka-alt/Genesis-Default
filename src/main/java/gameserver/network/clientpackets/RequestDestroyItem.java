...
public final class RequestDestroyItem extends GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.isCastingNow() && activeChar.getCurrentSkill().getSkill() != null && activeChar.getCurrentSkill().getSkill().getItemConsumeId() == itemToRemove.getId())
		{
			activeChar.sendPacket(SystemMessageId.CANNOT_DISCARD_THIS_ITEM);
			return;
		}
		...
		Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> destroy ->", itemToRemove.getName(null), itemToRemove.getCount(), itemToRemove.getEnchantLevel(), null);
	}
}