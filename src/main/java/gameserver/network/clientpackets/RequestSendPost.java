...
import gameserver.network.serverpackets.ExNoticePostSent;
...
public final class RequestSendPost extends GameClientPacket
{
	...
	public void runImpl()
	{
		...
		final var accessLevel = AdminParser.getInstance().getAccessLevel(level);
		...
		final var msg = new Message(activeChar.getObjectId(), receiverId, _isCod, _subject, _text, _reqAdena);
	...
	private final boolean removeItems(Player player, Message msg)
	{
		...
		if (_count > 0)
		{
			for (int i = 0; i < _count; i++)
			{
				final var item = player.getInventory().getItemByObjectId(_items[i]);
		...
		for (int i = 0; i < _count; i++)
		{
			final var oldItem = player.checkItemManipulation(_items[i], _itemQ[i], "attach");
			...
			final var newItem = player.getInventory().transferItem("SendMail", _items[i], _itemQ[i], attachments, player, receiver);
			if (newItem == null)
			{
			...
			Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> send mail to -> " + _receiver + " with item ->", newItem.getName(null), newItem.getCount(), newItem.getEnchantLevel(), null);
		}
