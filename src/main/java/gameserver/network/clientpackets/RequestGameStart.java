...
import gameserver.network.serverpackets.CharacterSelected;
...
public class RequestGameStart extends GameClientPacket
{
		...
		if (client.getActiveCharLock().tryLock())
		{
			try
			{
				if (client.getActiveChar() == null)
				{
					...
					if (PunishmentManager.getInstance().checkPunishment(client, PunishmentType.BAN) || !checkSecurity(client, info.getAccessLevel()))
					{
						client.close(ServerClose.STATIC_PACKET);
						return;
					}
	...
	private boolean checkSecurity(GameClient client, int accessLevel)
	{
		if (Config.ALLOW_PRE_START_SYSTEM && Config.PRE_START_PATTERN > 0 && accessLevel < 7)
		{
