...
public final class RequestHennaEquip extends GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (henna.isAllowedClass(activeChar.getClassId()) && (_count >= henna.getWearCount()) && (activeChar.getAdena() >= henna.getWearFee()) && activeChar.addHenna(henna))
		{
			...
			Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> draw symbol ->", Util.getItemName(henna.getDyeItemId()), henna.getWearCount(), 0, null);
			Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> destroy ->", "Adena", henna.getWearFee(), 0, null);
		}
		else
		{
