		...
		try
		{
			...
			if (activeChar.getObjectId() == item.getOwnerId())
			{
				...
				else
				{
					...
					{
						activeChar.getInventory().setPaperdollItem(Inventory.PAPERDOLL_LHAND, item, true);
			...
			else
			{
				if (activeChar.isGM())
				{
					...
					if (count > 0)
					{
						if (item.isEquipped())
						{
							final var unequiped = owner.getInventory().unEquipItemInSlotAndRecord(item.getLocationSlot(), true);
