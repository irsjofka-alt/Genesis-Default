...
public final class RequestReceivePost extends GameClientPacket
{
	...
	public void runImpl()
	{
		...
		final var msg = MailManager.getInstance().getMessage(_msgId);
		if (msg == null)
		{
			return;
		}
		...
		final var attachments = msg.getAttachments();
		if (attachments == null)
		{
			return;
		}
		...
		for (final var item : attachments.getItems())
		{
			if (item == null)
			{
				continue;
		...
		final var playerIU = Config.FORCE_INVENTORY_UPDATE ? null : new InventoryUpdate();
		for (final var item : attachments.getItems())
		{
			if (item == null)
			{
			...
			final var newItem = attachments.transferItem(attachments.getName(), item.getObjectId(), item.getCount(), activeChar.getInventory(), activeChar, null);
			if (newItem == null)
			{
				return;
			}
			...
			final var sm = SystemMessage.getSystemMessage(SystemMessageId.YOU_ACQUIRED_S2_S1);
			sm.addItemName(item.getId());
			sm.addItemNumber(count);
			activeChar.sendPacket(sm);
			Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> receive mail from -> " + msg.getSenderName() + " with item ->", item.getName(null), item.getCount(), item.getEnchantLevel(), null);
		}
		...
		SystemMessage sm;
		final var sender = GameObjectsStorage.getPlayer(msg.getSenderId());
		if (adena > 0)
		{
			if (sender != null)
			{
			...
			{
				final var paidAdena = ItemsParser.getInstance().createItem("PayMail", ADENA_ID, adena, activeChar, null);
