...
import gameserver.model.actor.templates.items.LifeStone;
import gameserver.model.base.ShortcutType;
import gameserver.model.items.instance.ItemInstance;
import gameserver.network.SystemMessageId;
import gameserver.network.serverpackets.ExVariationResult;
...
public final class RequestRefine extends AbstractRefinePacket
{
	...
	protected void runImpl()
	{
		...
		if (equipped)
		{
			final var unequiped = activeChar.getInventory().unEquipItemInSlotAndRecord(targetItem.getLocationSlot(), true);
		...
		Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> destroy ->", refinerItem.getName(null), refinerItem.getCount(), 0, null);
		Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> destroy ->", gemStoneItem.getName(null), _gemStoneCount, 0, null);
