...
import gameserver.model.items.buylist.Product;
		...
		for (final ItemHolder i : _items)
		{
			...
			if (product.hasLimitedStock())
			{
				if (product.decreaseCount(i.getCount()))
				{
					...
					Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> buy ->", Util.getItemName(i.getId()), i.getCount(), 0, " -> from BuyList[" + _listId + ".xml]");
				}
			}
			else
			{
