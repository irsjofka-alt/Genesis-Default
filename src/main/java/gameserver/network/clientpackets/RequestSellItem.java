...
import static gameserver.model.actor.Npc.INTERACTION_DISTANCE;
...
import gameserver.model.items.buylist.ProductList;
...
public final class RequestSellItem extends GameClientPacket
{
	...
	protected void processSell()
	{
		...
		for (final ItemHolder i : _items)
		{
			...
			if (merchant != null)
			{
				Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> sell ->", Util.getItemName(i.getId()), i.getCount(), 0, "-> by " + merchant.getName(null));
			}
			else
			{
				Log.addLogItem(getClass().getSimpleName(), player.getName(null) + " -> sell ->", Util.getItemName(i.getId()), i.getCount(), 0, null);
			}
		}
