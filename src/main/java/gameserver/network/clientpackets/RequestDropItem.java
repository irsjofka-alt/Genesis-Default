...
public final class RequestDropItem extends GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.isActionsDisabled() || activeChar.inObserverMode())
		{
		...
		if (item.isEquipped())
		{
			final var unequiped = activeChar.getInventory().unEquipItemInSlotAndRecord(item.getLocationSlot(), true);
		...
		if (dropedItem != null)
		{
			Log.addLogItem(getClass().getSimpleName(), activeChar.getName(null) + " -> has dropped ->", dropedItem.getName(null), dropedItem.getCount(), dropedItem.getEnchantLevel(), "at Location[" + _x + ", " + _y + ", " + _z + "]");
		}
	}
