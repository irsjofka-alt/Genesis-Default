...
public final class RequestGiveItemToPet extends GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		final var item = player.getInventory().getItemByObjectId(_objectId);
		if (item == null)
		{
			return;
		}
		...
		if (item.isHeroItem() || (Config.ALLOW_BLOCK_TRADE_ITEMS && !item.getItem().isPetItems()) || (!Config.ALLOW_BLOCK_TRADE_ITEMS && (!item.isDropable() || !item.isDestroyable() || !item.isTradeable())))
		{
			player.sendPacket(SystemMessageId.ITEM_NOT_FOR_PETS);
			return;
		}

		final var pet = (PetInstance) player.getSummon();
		if (pet.isDead())
		{
		...
		final var petItem = pet.getControlItem();
