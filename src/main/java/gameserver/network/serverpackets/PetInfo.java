...
public class PetInfo extends GameServerPacket
{
	...
	{
		_summon = summon;
		_name = !summon.isTargetable() ? "" : summon.getName(null);
		_title = !summon.isTargetable() ? "" : summon.getOwner().isInFightEvent() ? summon.getOwner().getFightEvent().getVisibleTitle(summon.getOwner(), summon.getOwner(), summon.getTitle(null), false) : summon.getTitle(null);
		...
		_runSpd = (int) Math.round(_summon.getRunSpeed() / _moveMultiplier);
		_walkSpd = (int) Math.round(_summon.getWalkSpeed() / _moveMultiplier);
		_swimRunSpd = (int) Math.round(_summon.getSwimRunSpeed() / _moveMultiplier);
