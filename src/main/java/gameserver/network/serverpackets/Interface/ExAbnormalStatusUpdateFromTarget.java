...
public class ExAbnormalStatusUpdateFromTarget extends GameServerPacket
{
	...
	public ExAbnormalStatusUpdateFromTarget(Creature creature)
	{
		...
		if (creature.isRaid())
		{
			_effects = Arrays.asList(creature.getEffectList().getAllEffects()).stream().filter(Objects::nonNull).filter(Effect::isInUse).filter(b -> b.getAbnormalTime() > 1 || b.getTotalTickCount() > 1).collect(Collectors.toList());
		}
		else
		{
			_effects = Arrays.asList(creature.getEffectList().getAllEffects()).stream().filter(Objects::nonNull).filter(Effect::isInUse).filter(b -> b.getSkill().hasDebuffEffects() && (b.getAbnormalTime() > 1 || b.getTotalTickCount() > 1)).collect(Collectors.toList());
		}
	}
