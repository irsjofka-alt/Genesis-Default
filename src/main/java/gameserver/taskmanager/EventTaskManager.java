...
public class EventTaskManager extends LoggerObject
{
	...
	public void recalcTime()
	{
		...
		try
		{
			...
			final var sorted = _eventTasks.entrySet().stream().sorted(Map.Entry.comparingByValue()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, _) -> e1, LinkedHashMap::new));
			if (!sorted.isEmpty())
			{
