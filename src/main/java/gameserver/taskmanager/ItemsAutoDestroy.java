...
public class ItemsAutoDestroy extends LoggerObject
{
	...
	{
		_items.clear();
		if (Config.RATE_CHANCE_DROP_HERBS > 0)
		{
	...
	private void spawnHerbs(int count, Location loc, long destroyTime)
	{
		if (Rnd.getChance(HerbsDropParser.getInstance().getChance()))
		{
			for (int i = 0; i < count; i++)
			{
				...
				item.setCount(1);
				final var pos = Location.findPointToStay(loc, loc.getHeading(), true);
	...
	private void recalcTime()
	{
		try
		{
			...
			final var sorted = _items.entrySet().stream().sorted(Map.Entry.comparingByValue()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, _) -> e1, LinkedHashMap::new));
			if (!sorted.isEmpty())
			{
	...
	private class CheckItemsForDestroy implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				if (Config.RATE_CHANCE_DROP_HERBS > 0 && _herbRecal <= curtime)
				{
					final var herbs = HerbsDropParser.getInstance();
					if (herbs.getHerbSpawns().size() > 0)
					{
