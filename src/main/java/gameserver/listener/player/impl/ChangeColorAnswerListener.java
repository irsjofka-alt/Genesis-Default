...
public class ChangeColorAnswerListener implements OnAnswerListener
{
	...
	private final String _colorh;
	private final int _days;
	private final int _type;
		
	public ChangeColorAnswerListener(Player player, String colorh, String color, int days, int type)
	{
		_player = player;
		_colorh = colorh;
		_color = color;
					...
					if (found)
					{
						if (itemId != 0)
						{
							if (_player.getInventory().getItemByItemId(itemId) == null)
							{
								...
								if (cmd != null)
								{
									cmd.onBypassCommand("_bbs_service;colornamelist", _player);
								}
								return;
							}
						...
						final int curColor = Integer.decode("0x" + _colorh);
					...
					if (found)
					{
						if (itemId != 0)
						{
							if (_player.getInventory().getItemByItemId(itemId) == null)
							{
								...
								if (cmd != null)
								{
									cmd.onBypassCommand("_bbs_service;colortitlelist", _player);
								}
								return;
							}
							if (_player.getInventory().getItemByItemId(itemId).getCount() < amount)
						...
						final int curColor1 = Integer.decode("0x" + _colorh);
