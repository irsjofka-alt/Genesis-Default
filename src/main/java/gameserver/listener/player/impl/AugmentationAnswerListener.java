...
public class AugmentationAnswerListener implements OnAnswerListener
{
	...
	public void sayYes()
	{
		if (_player != null && _player.isOnline())
		{
			...
			_player.getInventory().unEquipItemInSlot(Inventory.PAPERDOLL_RHAND, true);
			...
			_player.getInventory().equipItem(targetItem, true, true);
	...
	private void unAugment(Player player, ItemInstance item)
	{
		...
		if (equipped)
		{
			player.getInventory().unEquipItemInSlot(Inventory.PAPERDOLL_RHAND, true);
		}
		item.removeAugmentation();
		
		if (equipped)
		{
			player.getInventory().equipItem(item, true, true);
		}
