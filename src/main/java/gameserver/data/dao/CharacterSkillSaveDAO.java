...
public class CharacterSkillSaveDAO extends LoggerObject
{
	...
	public void store(Player player, boolean isAcumulative, boolean storeEffects)
	{
		...
		try
		{
			...
			final Map<Integer, Integer> storedSkills = new ConcurrentHashMap<>();
			...
			final var effects = player.getAllEffects();
			if (storeEffects && effects.length > 0)
			{
				isUpdate = true;
				for (final var effect : effects)
				{
					if (effect == null || effect.getSkill().isToggle() || effect.getSkill().isBlockSave())
					{
						continue;
					}
			...
			int hash;
			TimeStamp t;
			final var skillReuse = player.getSkillReuseTimeStamps();
			if (skillReuse.size() > 0)
			{
				isUpdate = true;
				for (final var ts : skillReuse.entrySet())
				{
					hash = ts.getKey();
