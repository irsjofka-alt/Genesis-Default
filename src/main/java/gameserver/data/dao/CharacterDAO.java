...
public class CharacterDAO extends LoggerObject
{
	...
	public boolean deleteAllSubClass(Player player)
	{
		var removed = true;
		Connection con = null;
		PreparedStatement statement = null;
		try
		{
			...
			for (final var sub : player.getSubClasses().values())
			{
				try
				{
					statement = con.prepareStatement("DELETE FROM character_hennas WHERE charId=? AND class_index=?");
					statement.setInt(1, player.getObjectId());
					statement.setInt(2, sub.getClassIndex());
					statement.execute();
					statement.close();
				...
				{
					warn("Could not delete sub class for " + player.getName(null) + " to class index " + sub.getClassIndex() + ": " + e.getMessage(), e);
					removed = false;
				}
			}
