...
import gameserver.model.entity.events.model.template.FightEventItem;
...
public final class FightEventParser extends DocumentParser
{
	...
	private final Map<Integer, Map<Integer, FightEventSet>> _eventItems = new HashMap<>();
			...
			if ("list".equalsIgnoreCase(c.getNodeName()))
			{
				for (Node events = c.getFirstChild(); events != null; events = events.getNextSibling())
				{
					if ("event".equalsIgnoreCase(events.getNodeName()))
					{
						...
						try
						{
							eventClass = (Class<AbstractFightEvent>) Class.forName("gameserver.model.entity.events.model.impl." + impl + "Event");
						}
						...
						final Map<Integer, FightEventSet> eventSets = new HashMap<>();
						set.set("id", id);
						...
						set.set("eventClass", "gameserver.model.entity.events.model.impl." + impl + "Event");
						...
						for (Node par = events.getFirstChild(); par != null; par = par.getNextSibling())
						{
							...
							else if ("eventItems".equalsIgnoreCase(par.getNodeName()))
							{
								for (Node d1 = par.getFirstChild(); d1 != null; d1 = d1.getNextSibling())
								{
									...
									{
										attrs = d1.getAttributes();
										final MultiValueSet<String> setItems = new MultiValueSet<>();
										final int setId = Integer.parseInt(attrs.getNamedItem("id").getNodeValue());
										for (final String lang : Config.MULTILANG_ALLOWED)
										{
											if (lang != null)
											{
												final String name = "name" + lang.substring(0, 1).toUpperCase() + lang.substring(1);
												setItems.set(name, attrs.getNamedItem(name) != null ? attrs.getNamedItem(name).getNodeValue() : attrs.getNamedItem("nameEn") != null ? attrs.getNamedItem("nameEn").getNodeValue() : "");
											}
										}
										setItems.set("id", setId);
										...
										eventSets.put(setId, new FightEventSet(setItems, items));
									}
								}
							}
						}
						...
						_eventItems.put(event.getId(), eventSets);
					}
				}
			}
		}
	...
	public Map<Integer, FightEventSet> getEventItems(int eventId)
	{
