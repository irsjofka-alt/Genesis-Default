...
import gameserver.model.ProductItem;
import gameserver.model.actor.Player;
import gameserver.model.actor.templates.ProductItemTemplate;
import gameserver.model.actor.templates.items.Item;
import gameserver.network.serverpackets.ExBrBuyProduct;
import gameserver.network.serverpackets.ExBrGamePoint;
import gameserver.network.serverpackets.ExBrRecentProductList;
...
public class ProductItemParser extends DocumentParser
{
	...
	public void requestBuyItem(Player player, int productId, int count)
	{
		...
		for (final ProductItemTemplate comp : product.getComponents())
		{
			...
			Log.addLogItem("RequestBrBuyProduct", player.getName(null) + " -> buy ->", Util.getItemName(comp.getId()), comp.getCount() * count, 0, null);
		}
