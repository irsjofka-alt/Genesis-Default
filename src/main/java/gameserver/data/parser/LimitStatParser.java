...
public class LimitStatParser extends DocumentParser
{
	...
	public double getStatLimit(Creature actor, Stats stat, double limit)
	{
		if (actor.isPlayer() || actor.isSummon())
		{
			...
			{
				final var limits = player.isInOlympiadMode() ? actor.isSummon() ? _olyStatsList.get(LimitType.SUMMON) : _olyStatsList.get(LimitType.PLAYER) : actor.isSummon() ? _statsList.get(LimitType.SUMMON) : _statsList.get(LimitType.PLAYER);
				if (limits != null && !limits.isEmpty())
				{
					final var classId = actor.isSummon() ? -1 : actor.getActingPlayer().getBaseClass();
					var stats = limits.get(classId);
