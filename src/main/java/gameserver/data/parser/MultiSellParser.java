...
import gameserver.model.items.multisell.ListContainer;
...
import gameserver.network.serverpackets.MultiSellList;
...
public class MultiSellParser extends DocumentParser
{
	...
	{
		_entries.clear();
		parseDirectory("data/stats/npcs/multisell", true, false);
		verify();
		...
		try
		{
			...
			for (Node n = getCurrentDocument().getFirstChild(); n != null; n = n.getNextSibling())
			{
				if ("list".equalsIgnoreCase(n.getNodeName()))
				{
					...
					att = n.getAttributes().getNamedItem("isSkillProducts");
					list.setIsSkillProducts((att != null) && Boolean.parseBoolean(att.getNodeValue()));
					...
					for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						if ("item".equalsIgnoreCase(d.getNodeName()))
						{
							...
							if (e.isChanceMultiSell())
							{
								list.setIsChanceMultiSell(true);
							}
						}
	...
	public final void separateAndSend(int listId, Player player, Npc npc, boolean inventoryOnly, double productMultiplier, double ingredientMultiplier)
	{
		...
		{
			player.sendPacket(new ExDynamicPacket(list));
		}
		else
		{
			int index = 0;
	...
	public static final void addSpecialProduct(int id, long amount, Player player)
	{
		...
		Log.addLogItem("RequestMultiSellChoose", player.getName(null) + " -> destroy ->", Util.getItemName(id), amount, 0, null);
	}
