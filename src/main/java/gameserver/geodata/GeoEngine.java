...
import l2e.commons.geometry.LinePointIterator2D;
...
import gameserver.geodata.loader.GeoLoader;
import gameserver.geodata.pathfinding.PathFinding;
import gameserver.geodata.util.GeoUtils;
...
{
	private static final Logger _log = LogManager.getLogger(GeoEngine.class);
	
	private static final int ELEVATED_SEE_OVER_DISTANCE = 2;
	private static final int MAX_SEE_OVER_HEIGHT = 48;
	private static final int SPAWN_Z_DELTA_LIMIT = 100;
	
	public static float LOW_WEIGHT = Config.LOW_WEIGHT;
	public static float MEDIUM_WEIGHT = Config.MEDIUM_WEIGHT;
	public static float HIGH_WEIGHT = Config.HIGH_WEIGHT;
	public static float DIAGONAL_WEIGHT = Config.DIAGONAL_WEIGHT;
	...
	public Location moveCheck(Creature creature, int x, int y, int z, int tx, int ty, int tz, Reflection ref, boolean returnPrev)
	{
		...
		final var pointIter = new LinePointIterator2D(geoX, geoY, tGeoX, tGeoY);
		pointIter.next();
		int prevX = pointIter.x();
		int prevY = pointIter.y();
		int prevZ = z;
		...
		int i = 1;
		final Location[] path = new Location[2];
		path[0] = new Location(prevX, prevY, prevZ);
		while (pointIter.next())
		{
			final int curX = pointIter.x();
			final int curY = pointIter.y();
			...
			if (hasGeoPos(prevX, prevY))
			{
				...
				{
					return new Location(getWorldX(path[0].getX()), getWorldY(path[0].getY()), path[0].getZ());
				}
				prevX = curX;
				prevY = curY;
				prevZ = curZ;
				path[i % 2 == 0 ? 1 : 0] = new Location(prevX, prevY, prevZ);
			}
		}
		
		if (returnPrev && hasGeoPos(prevX, prevY) && (prevZ != tz) || (prevZ != tz && tz > prevZ))
		{
			if (prevZ != tz && tz > prevZ)
			{
	...
	public Location moveDoorCheck(Creature creature, int tx, int ty, int tz, Reflection ref)
	{
		...
		while (pointIter.next())
		{
			...
			if (hasGeoPos(prevX, prevY))
			{
				i++;
				final int nswe = GeoUtils.computeNswe(prevX, prevY, curX, curY);
				final var isBlockDoor = creature.isInFrontDoor(DoorParser.getInstance().getRegionDoors(creature.getLocation(), ref), getWorldX(prevX), getWorldY(prevY), prevZ, getWorldX(curX), getWorldY(curY), curZ, ref);
	...
	private Location moveWaterCheck(int x, int y, int z, int tx, int ty, int tz, Reflection ref, int minZ, int maxZ)
	{
		...
		final Location[] path = new Location[2];
		path[0] = new Location(next_x, next_y, next_z);
		...
		if (dx >= dy)
		{
			...
			for (int i = 0; i < dx; i++)
			{
				...
				final int nswe = GeoUtils.computeNswe(x, y, next_x, next_y);
				...
				path[i % 2 != 0 ? 1 : 0] = new Location(next_x, next_y, next_z);
			}
		}
		else
		{
		...
		return new Location(getWorldX(next_x), getWorldY(next_y), next_z);
	}

	public Location moveCheck(Creature creature, Location startLoc, Location endLoc, Reflection ref, boolean returnPrev)
	{
		return moveCheck(creature, startLoc.getX(), startLoc.getY(), startLoc.getZ(), endLoc.getX(), endLoc.getY(), endLoc.getZ(), ref, returnPrev);
	}
	...
	{
		return hasGeo(loc.getX(), loc.getY()) ? isNSWEAll(loc.getX(), loc.getY(), loc.getZ(), distance) : false;
	}
	
	public boolean isNSWEAll(int worldX, int worldY, int worldZ)
	{
		return hasGeo(worldX, worldY) ? isNSWEAll(worldX, worldY, worldZ, 40) : false;
	}
