...
import fake.ai.FallbackAI;
import fake.ai.addon.TraderAI;
...
import gameserver.data.parser.FakeArmorParser;
import gameserver.data.parser.FakeClassesParser;
...
import gameserver.model.actor.templates.player.FakeTraderTemplate;
...
import gameserver.model.base.Sex;
...
public class FakeSupport
{
	...
	public static FakePlayer createRandomFakePlayer(FakeLocTemplate loc)
	{
		...
		if (rndLevel > 75)
		{
			...
			{
				classId = ClassId.getClassId(loc.getClasses().get(Rnd.get(0, loc.getClasses().size() - 1)));
			}
			else
			{
	...
	public static FakePlayer createRandomPassiveTrader(FakeTraderTemplate tpl)
	{
		...
		if (rndLevel > 75)
		{
			if (tpl.getClassId() >= 0)
			{
				classId = ClassId.getClassId(tpl.getClassId());
			}
			else
			{
				classId = FakeClassesParser.getInstance().getThirdClasses().get(Rnd.get(0, FakeClassesParser.getInstance().getThirdClasses().size() - 1));
		...
		if (items != null)
		{
			for (final PcItemTemplate ie : items)
			{
				...
				if (item.isEquipable())
				{
					...
					player.getInventory().equipItem(item, true, false);
				}
			}
