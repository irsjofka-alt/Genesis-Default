...
import fake.FakePoolManager;
import fake.PassiveFakeTaskManager;
...
public abstract class FakePlayerAI implements Runnable
{
	...
	public FakePlayerAI(FakePlayer character, boolean isPassive)
	{
		...
		if (!isPassive)
		{
			checkFakeLocation();
			applyDefaultBuffs();
			if (_fakePlayer.getClassId() == ClassId.SHILLEN_TEMPLAR)
			{
				selfCubicBuffs();
			}
			startActionTask();
	...
	protected void checkFakeLocation()
	{
	...
	public void run()
	{
		if (_lock || _fakePlayer == null || _fakePlayer.getFakeAi().isBusyThinking())
		{
			return;
		}
	...
	protected void applyDefaultBuffs()
	{
		if (_buffTime <= System.currentTimeMillis())
		{
			if (_fakePlayer.getLevel() > 8)
			{
				...
				{
					final Map<Integer, Effect> activeEffects = Arrays.stream(_fakePlayer.getEffectList().getAllEffects()).filter(x -> x.getEffectType() == EffectType.BUFF).collect(Collectors.toMap(x -> x.getSkill().getId(), x -> x));
					...
					if (skill != null)
					{
						if (!activeEffects.containsKey(buff[0]))
						{
							...
							{
								skill.getEffects(_fakePlayer, _fakePlayer, false, false);
							}
						}
						else
						{
	...
	protected void teleportToLocation(int x, int y, int z)
	{
	...
	protected GameObject tryTargetRandomCreatureByTypeInRadius(int radius, Function<Creature, Boolean> condition)
	{
		...
		if (target == null)
		{
			...
			for (final Creature obj : World.getAroundCharacters(_fakePlayer, radius, 300))
			{
				if (obj.isPlayer())
				{
	...
	protected void toVillageOnDeath()
	{
		...
		try
		{
			...
			_fakePlayer.getFakeAi().teleportToLocation(location.getX(), location.getY(), location.getZ());
			checkFakeLocation();
		}
		finally
		{
	...
	public synchronized void stopActionTask()
	{
