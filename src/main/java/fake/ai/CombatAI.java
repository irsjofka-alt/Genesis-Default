...
import gameserver.model.actor.instance.ClanHallDoormenInstance;
...
public abstract class CombatAI extends FakePlayerAI
{
	...
	{
		if (_sleepTime > System.currentTimeMillis())
		{
			return;
		}
	...
	{
		if (_fakePlayer.getFakeAi().isTownZone() && _waitTime > System.currentTimeMillis())
		{
			townActions();
		}
		else
		{
			if (_fakePlayer.getFakeAi().isWantToFarm())
			{
				...
				if (target != null && target instanceof TeleporterInstance)
				{
					...
					if (distance <= 150)
					{
						_sleepTime = System.currentTimeMillis() + (Rnd.get(2, 3) * 1000L);
						_fakePlayer.getFakeAi().setWantToFarm(false);
						final Location loc = _fakePlayer.getFakeLocation().getLocation();
						_fakePlayer.getFakeAi().teleportToLocation(loc.getX(), loc.getY(), loc.getZ());
						_fakePlayer.getFakeAi().checkFakeLocation();
					}
					else
					{
						...
						{
							final var path = PathFinding.getInstance().findPath(_fakePlayer, _fakePlayer.getX(), _fakePlayer.getY(), _fakePlayer.getZ(), target.getX(), target.getY(), target.getZ(), _fakePlayer.getReflection(), true, false);
			...
			{
				if (_waitTime > System.currentTimeMillis())
				{
					townActions();
				}
				else
	...
	private void townActions()
	{
		...
		if (target != null && _fakePlayer.getFakeAi().isTargetLock())
		{
			...
			if (distance <= 150)
			{
				_sleepTime = System.currentTimeMillis() + (Rnd.get(4, 6) * 1000L);
		...
		else
		{
			...
			if (_sitTime <= System.currentTimeMillis())
			{
				...
				if (newTarget != null)
				{
					...
					if (!GeoEngine.getInstance().canSeeTarget(_fakePlayer, target))
					{
						final var path = PathFinding.getInstance().findPath(_fakePlayer, _fakePlayer.getX(), _fakePlayer.getY(), _fakePlayer.getZ(), newTarget.getX(), newTarget.getY(), newTarget.getZ(), _fakePlayer.getReflection(), true, false);
						if (path == null || path.size() < 2)
						{
					...
					_idleTime = System.currentTimeMillis() + (15 * 1000);
				}
			}
		}
	}
	...
	private void sitDown(FakePlayer fakePlayer)
	{
		...
		{
			fakePlayer.sitDown();
			_sitTime = System.currentTimeMillis() + (Rnd.get(10, 30) * 1000);
		}
	}
	...
	private Creature searchTownNpc(int range)
	{
		...
		for (final Npc target : World.getAroundNpc(_fakePlayer, range, 400))
		{
	...
	private void areaBehavior(boolean isMage)
	{
		...
		else
		{
			...
			if (!_fakePlayer.isInsideRadius(tmpl.getLocation().getX(), tmpl.getLocation().getY(), tmpl.getLocation().getZ(), tmpl.getDistance(), true, true))
			{
				...
				{
					final var path = PathFinding.getInstance().findPath(_fakePlayer, _fakePlayer.getX(), _fakePlayer.getY(), _fakePlayer.getZ(), tmpl.getLocation().getX(), tmpl.getLocation().getY(), tmpl.getLocation().getZ(), _fakePlayer.getReflection(), true, false);
					if (path == null || path.size() < 2)
					{
						_sleepTime = System.currentTimeMillis() + (Rnd.get(3, 5) * 1000L);
			...
			{
				_sleepTime = System.currentTimeMillis() + (Rnd.get(1, 2) * 1000L);
			}
		}
	}
	...
	protected void selfSupportBuffs()
	{
		...
		final List<Integer> activeEffects = Arrays.stream(_fakePlayer.getEffectList().getAllEffects()).map(x -> x.getSkill().getId()).collect(Collectors.toList());
